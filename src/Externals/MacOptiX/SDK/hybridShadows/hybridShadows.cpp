...
optix::Context        rtContext = 0;
optix::Buffer         rtShadowBuffer = 0;
optix::TextureSampler rtWorldSpaceTexture = 0;
...
void init_gl()
{
  ...
  cgSetParameter1i(cgDoPCFParam, 0);

  // create PBO's
  glGenBuffers(1, &shadowMapPBO);
  glBindBuffer(GL_PIXEL_UNPACK_BUFFER, shadowMapPBO);
  glBufferData(GL_PIXEL_UNPACK_BUFFER, 32*32*sizeof(unsigned char), 0, GL_STREAM_READ);
  ...
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE8, 32, 32, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, NULL);
  glBindTexture(GL_TEXTURE_2D, 0);
...
{
  return std::string(sutilSamplesPtxDir()) + "/hybridShadows_generated_" + base + ".ptx";
}
...
void init_optix()
{
  try {
    ...
    rtContext["scene_epsilon"]->setFloat(1e-4f);
    ...
    rtShadowBuffer->setFormat(RT_FORMAT_UNSIGNED_BYTE);
    rtContext["shadow_buffer"]->setBuffer(rtShadowBuffer);

    rtContext->setRayGenerationProgram( 0, rtContext->createProgramFromPTXFile( ptxpath("shadow_request.cu"), "shadow_request" ) );
    rtContext->setExceptionProgram( 0, rtContext->createProgramFromPTXFile( ptxpath("shadow_request.cu"), "exception" ) );

    optix::Material opaque = rtContext->createMaterial();
    opaque->setAnyHitProgram(0, rtContext->createProgramFromPTXFile( ptxpath("opaque.cu"), "any_hit_shadow") );
...
void draw_ground(CGtechnique& tech)
{
  ...
  while(pass) {
    ...
    glNormal3f(0,1,0);
    glTexCoord2f(0,0); glVertex3f(-dim, modelBBMin.y-diag*.05f, -dim);
    glTexCoord2f(0,1); glVertex3f(-dim, modelBBMin.y-diag*.05f, dim);
    glTexCoord2f(1,0); glVertex3f(dim, modelBBMin.y-diag*.05f, -dim);

    glTexCoord2f(0,1); glVertex3f(-dim, modelBBMin.y-diag*.05f, dim);
    glTexCoord2f(1,0); glVertex3f(dim, modelBBMin.y-diag*.05f, -dim);
    glTexCoord2f(1,1); glVertex3f(dim, modelBBMin.y-diag*.05f, dim);
...
void display()
{
  sutilFrameBenchmark("Hybrid Shadows", warmup_frames, timed_frames);
...
void resize(int w, int h)
{
  ...
  try {
    ...
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, shadowMapPBO);
    glBufferData(GL_PIXEL_UNPACK_BUFFER, traceWidth*traceHeight*sizeof(unsigned char), 0, GL_STREAM_READ);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
  ...
  // resize textures
  glBindTexture(GL_TEXTURE_2D, shadowMapTex);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE8, traceWidth, traceHeight, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0);
  glBindTexture(GL_TEXTURE_2D, 0);
...
void printUsageAndExit( const std::string& argv0, bool doExit = true )
{
  ...
  std::cerr
    ...
    << "  b Toggle Bilateral PCF blurring\n"
    << std::endl;
...
{
  // Allow glut to handle its args first
  glutInit(&argc, argv);
  ...
  if(cgfx_path.empty()) {
    cgfx_path = std::string( sutilSamplesDir() ) + "/hybridShadows/hybridShadows.cgfx";
  }

  CGFX_PATH = cgfx_path;
  GROUNDTEX_PATH = std::string( sutilSamplesDir() ) + "/hybridShadows/ground.ppm";

  glutInitWindowSize(initialWindowWidth, initialWindowHeight);
  glutInitWindowPosition(100, 100);
  glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH);
  glutCreateWindow("hybridShadows");
