...
{
//  if(x < 0.25f || y < 0.25f || x + y > 0.75f) 
//    return 0.0f;
//  else return DISPLACE_MAX_D;
...
RT_PROGRAM void mesh_intersect( int primIdx )
{
  ...
  float3 n0 = normalize(normal_buffer[ n_idx.x ]);
  float3 n1 = normalize(normal_buffer[ n_idx.y ]);
  float3 n2 = normalize(normal_buffer[ n_idx.z ]);
