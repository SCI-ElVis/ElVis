              ...
              unsigned int ntris, unsigned int* indices )
{
  ...
  *tris = (float*)malloc( buffer_size );
  ...
  for(tri_index = 0; tri_index < ntris; ++tri_index)
  {
    ...
    for(i = 0; i < 3; ++i)
    {
      unsigned int index = tri_index*3+i;
      unsigned int vert_index = indices[index];
      soup[index].x = vertices[vert_index].x;
      soup[index].y = vertices[vert_index].y;
      soup[index].z = vertices[vert_index].z;
    }
  }
}
                  ...
                  unsigned int width, unsigned int height)
{
  ...
  for( j = height; j > 0 && written < max; j--) {
    for ( i = 0; i < width && written < max; i++) {
      unsigned int index = (j-1) * width + i;
      if (index >= num_rays)
        continue;
      if( results[index].prim_id != -1 )
        fprintf( outfile, "%i\n", (int)(results[index].t / max_val * 255.0f) );
      else
        fprintf( outfile, "%i\n", 0 );
      written++;
    }
