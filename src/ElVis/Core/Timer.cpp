...
namespace ElVis
{
  Timer::Timer() : m_start(), m_end(), m_resolution() {}

  Timer::~Timer() {}

  Timer::Timer(const Timer& rhs)
    : m_start(rhs.m_start), m_end(rhs.m_end), m_resolution(rhs.m_resolution)
  {
  }
  ...
  double Timer::TimePerTest(unsigned int n)
  {
    ...
    return Elapsed().QuadPart / static_cast<double>(n) * 1.0 /
           frequency.QuadPart;
#elif defined(__APPLE__)
