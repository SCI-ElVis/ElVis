...
namespace ElVis
{
  namespace
  {
    const std::string VALUE_KEY_NAME("Values");
  }
  ...
  public:
    ...
    Point(const std::string& pointValues) { fromString(pointValues, *this); }
    ...
    Point(const Point<DataType, dim, space>& rhs)
      : OnPointChanged(), OnXChanged(), OnYChanged(), OnZChanged()
    {
      for (unsigned int i = 0; i < dim::Value; ++i)
      {
    ...
    Point<DataType, dim, space>& operator=(
    ...
    void SetValue(unsigned int i, const DataType& value) { m_data[i] = value; }
    ...
    const DataType* GetPtr() const { return &m_data[0]; }
  ...
  Point<DataType, dim, space> operator+(const Point<DataType, dim, space>& lhs,
                                        const Point<DataType, dim, space>& rhs)
  {
  ...
  template <typename DataType, typename dim, typename space>
  Point<DataType, dim, space> operator+(const DataType& lhs,
                                        const Point<DataType, dim, space>& rhs)
  {
  ...
  template <typename DataType,
            ...
            typename space,
            typename ScalarType>
  Point<DataType, dim, space> operator*(const ScalarType& lhs,
                                        const Point<DataType, dim, space>& rhs)
  {
    Point<DataType, dim, space> result(rhs);
    result *= lhs;
  ...
  std::ostream& operator<<(std::ostream& os,
                           const Point<DataType, dim, space>& p)
  {
    os << p.AsString();
    return os;
  }
  ...
  template <typename DataType, typename dim, typename space>
  bool operator<(const Point<DataType, dim, space>& lhs,
                 const Point<DataType, dim, space>& rhs)
  {
    for (unsigned int i = 0; i < dim::Value; ++i)
    {
  ...
  template <typename DataType, typename dim, typename space>
  Point<DataType, dim, space> CalcMin(const Point<DataType, dim, space>& lhs,
                                      const Point<DataType, dim, space>& rhs)
  {
  ...
  template <typename DataType, typename dim, typename space>
  Point<DataType, dim, space> CalcMax(const Point<DataType, dim, space>& lhs,
                                      const Point<DataType, dim, space>& rhs)
  {
    Point<DataType, dim, space> result;
    for (unsigned int i = 0; i < dim::Value; ++i)
