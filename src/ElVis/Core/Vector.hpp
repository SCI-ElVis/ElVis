...
namespace ElVis
{
    ...
    template<typename DataType, typename T, typename space>
    Point<DataType, ThreeD, space> findPointAlongVector(const Vector<DataType, space>& lhs, const T& t);
    ...
    class Vector
    {
        ...
        public:
            ...
            explicit Vector(const ElVisFloat3& values) :
                m_data(3)
            {
                m_data[0] = values.x;
                m_data[1] = values.y;
            ...
            explicit Vector(const ElVisFloat4& values) :
                m_data(4)
            {
                m_data[0] = values.x;
                m_data[1] = values.y;
            ...
            DataType Magnitude() const { return ElVis::Magnitude(*this); }
        
            DataType Dot(const Vector<DataType, space>& rhs) const { return ElVis::Dot(*this, rhs); }
        
            Vector<DataType, space> Cross(const Vector<DataType, space>& rhs) const
            {
                return ElVis::Cross(*this, rhs);
            }
        
            std::string AsString() const { return ElVis::AsString(*this); }

            // Norms
            DataType L1Norm() const { return ElVis::L1Norm(*this); }
            DataType L2Norm() const { return ElVis::L2Norm(*this); }
            DataType InfinityNorm() const { return ElVis::InfinityNorm(*this); }
