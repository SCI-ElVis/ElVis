...
/// \brief If a cut surface is located outside the volume, then we do not want
/// it to be considered for the
...
RT_PROGRAM void IgnoreCutSurfacesOutOfVolume()
{
  ...
  {
    // ELVIS_PRINTF("IgnoreCutSurfacesOutOfVolume: Ignoring intersection at
    // %f\n", closest_t);
    rtIgnoreIntersection();
  }
}

/// \brief When a cut-surface is encountered, this closest hit program is
...
RT_PROGRAM void SamplerVolumeClosestHit()
{
  ...
  ElementFinderPayload findElementPayload =
    FindElementFromFace(intersectionPoint);
  ...
  if (findElementPayload.elementId >= 0)
  {
    ...
    payload.scalarValue = EvaluateFieldOptiX(
      findElementPayload.elementId, findElementPayload.elementType, FieldId,
      intersectionPoint, findElementPayload.ReferencePointType,
      findElementPayload.ReferenceIntersectionPoint);
...
//    const ElVisFloat3 intersectionPoint = MakeFloat3(ray.origin) + closest_t *
//    MakeFloat3(ray.direction);
//    ElementFinderPayload findElementPayload = FindElement(intersectionPoint);

//    // Now use the extension interface to sample the field.
//    if( findElementPayload.elementId != -1 )
//    {
//        ELVIS_PRINTF("SamplerFaceClosestHit: Element id is %d and face id is
//        %d\n", findElementPayload.elementId, intersectedFaceGlobalIdx);
//        payload.scalarValue = EvaluateFieldOptiX(findElementPayload.elementId,
//        findElementPayload.elementType, FieldId, intersectionPoint,
//        ElVis::eReferencePointIsValid,
//        findElementPayload.ReferenceIntersectionPoint);
//        payload.isValid = true;
...
//        payload.Normal = MakeFloat3(MAKE_FLOAT(0.0), MAKE_FLOAT(0.0),
//        MAKE_FLOAT(0.0));
//        payload.IntersectionPoint = MakeFloat3(MAKE_FLOAT(0.0),
...
RT_PROGRAM void SamplerFaceClosestHit()
{
  ...
  //    ELVIS_PRINTF("SamplerFaceClosestHit Origin (%2.15f, %2.15f, %2.15f),
  //    closest t %2.15f, direction (%2.15f, %2.15f, %2.15f)\n",
  //                 ray.origin.x, ray.origin.y, ray.origin.z,
  //                 closest_t,
  //                 ray.direction.x, ray.direction.y, ray.direction.z);
  const ElVisFloat3 intersectionPoint =
    MakeFloat3(ray.origin) + closest_t * MakeFloat3(ray.direction);
  //    ELVIS_PRINTF("SamplerFaceClosestHit Intersection point (%2.15f, %2.15f,
  //    %2.15f)\n",
  ...
  if (faceIntersectionReferencePointIsValid)
  {
    ...
    id =
      FindElement(testPoint, intersectionPoint, faceIntersectionReferencePoint,
                  intersectedFaceGlobalIdx, n);
  }
  else
  {
  ...
  // ELVIS_PRINTF("SamplerFaceClosestHit: Element Id %d, Element Type %d,
  // FieldID %d, Normal (%f, %f, %f)\n", id.Id, id.Type, FieldId, n.x, n.y,
  // n.z);
  payload.scalarValue =
    EvaluateFieldOptiX(id.Id, id.Type, FieldId, intersectionPoint);
...
RT_PROGRAM void SampleOntoNrrd()
{
  ...
  if (findElementPayload.elementId != -1)
  {
    SampledOntoNrrdSamples[launch_index] = EvaluateFieldOptiX(
      findElementPayload.elementId, findElementPayload.elementType, FieldId,
      intersectionPoint, findElementPayload.ReferencePointType,
      findElementPayload.ReferenceIntersectionPoint);
  }
