...
/*-------------------------------------------------------------------*
 *   Dump a 2d .fld file with the information about the              *
 *    INFLOW boundary condition                                      *
 *-------------------------------------------------------------------*/

static int surf_inflow_number=0;
static int init2=1;

void surf_inflow(Domain *omega, int step, double time){
  register int i;
  int      eid, qa, qb, face;
  int  surfid;
  Bndry   *B = omega->Ubc;
  ...
  double  **D = dmatrix(0,3,0,QGmax*QGmax*QGmax-1);
  double  *influ = D[0];
  double  *inflv = D[1];
  double  *inflw = D[2], *p = D[3];
  ...
  FILE    *fout = omega->inflow_file;
  Element *eU, *eV, *eW, *eP;


  static int srf=option("SurInflow");
  ...
  if(init2){
    ...
    if(srf&&!Surf[0]){ // setup surface structure
	  Surf[0] = setup_surflist(omega,B,'V'); 

      if(Surf[0]){
	for(i = 0; i < Surf[0]->nel; ++i)
	  Surf[0]->flist[i]->type = 'u';

	Surf[1] = Surf[0]->gen_aux_field('v');
	Surf[2] = Surf[0]->gen_aux_field('w');
	Surf[3] = Surf[0]->gen_aux_field('p');
  ...
  surfid=0;
	...
	dsmul(qa,wb[i],w+i*qa,1,w+i*qa,1);
     ...
     eP->GetFace(**eP->h_3d,face,wk); 
     eP->InterpToFace1(face,wk,p);
	 eU->GetFace(**eU->h_3d,face,wk); 
     eU->InterpToFace1(face,wk,influ);
	 eV->GetFace(**eV->h_3d,face,wk); 
     eV->InterpToFace1(face,wk,inflv);
	 eW->GetFace(**eW->h_3d,face,wk); 
     eW->InterpToFace1(face,wk,inflw);
      
	  
      if(srf){
        dcopy(qa*qb,p,1,Surf[3]->flist[surfid]->h[0],1);
        dcopy(qa*qb,influ,1,Surf[0]->flist[surfid]->h[0],1);
        dcopy(qa*qb,inflv,1,Surf[1]->flist[surfid]->h[0],1);
        dcopy(qa*qb,inflw,1,Surf[2]->flist[surfid]->h[0],1);
      }
