...
#define  SEC_INFLOW   sections[12]            /* Inflow BC from file        */
#define  SEC_INFLOW2D sections[13]            /* Inflow BC from 2D file     */

static int   checklist   (const char *name, const char *list[]);
static char *findSection (const char *name, char *buf, FILE *fp);
...
static const char *sections[] = { 
  ...
  "DRIVE FUNCT", "TIME DEPENDENT", "INFLOW FILE", "INFLOW2D FILE"};
...
{
	int  n;
	char buf[BUFSIZ], value[100], name[25], c;
	double val, dt;

	static const char *dspecial[] = { "KINVIS", "LAMBDA", "IOTIME", "KC", "LZ",
	                            "FRNDX","FRNDY","FRNDZ" , "PECLET", 0 };
	static const char *ispecial[] = { "TEQN", "EQTYPE", "CFLSTEP", "HISSTEP",
	                            "SLVTYPE", 0 };
	...
	if (sscanf(buf, "%d", &n) != 1)
	{
		fputs("ReadParams: can't read # of parameters", stderr);
		exit(-1);
	}
	...
	{
		fgets (buf, BUFSIZ, fp);
		if(sscanf(buf, "%100s%25s", value, name) == 2)
		{
	...
	if ((dt=dparam("DELT")) == 0.)
	{
		...
		{
			fputs("ReadParams: no time step specified", stderr);
			exit(-1);
		}
	}
	else if (dparam("DT") == 0.)
	...
	if(n = option("NORDER.REQ"))
		iparam_set("MODES",n);
	if(!iparam("MODES")        )
		iparam_set("MODES",iparam("NORDER"));
...
void ReadPscals (FILE *fp)
{
	...
	if (sscanf(buf,"%d", &n) != 1)
	{
		fputs("ReadPscals: no passive scalar data", stderr);
		exit(-1);
	}
...
Element_List *ReadMesh (FILE *fp, char* session_name)
{
	...
	for(k = 0; k < nel; k++)
	{
		...
		for (i = 0; i < new_E[k]->Nverts; i++)
		{
			...
			if(xscal)
				new_E[k]->vert[i].x *= xscal;
			if(xmove)
				new_E[k]->vert[i].x += xmove;
		}
		...
		for (i = 0; i < new_E[k]->Nverts; i++)
		{
			...
			if(yscal)
				new_E[k]->vert[i].y *= yscal;
			if(ymove)
				new_E[k]->vert[i].y += ymove;
		}
		...
		if(new_E[k]->dim() == 3)
		{
                    ...
                    for (i = 0; i < new_E[k]->Nverts; i++)
			{
				...
				if(zscal)
					new_E[k]->vert[i].z *= zscal;
				if(zmove)
					new_E[k]->vert[i].z += zmove;
			}
			fgets(buf_a, BUFSIZ, fp);  /* finish the line */
		}
	}
	...
	if(strstr(buf,"type"))
	{
		...
		if(ntags)
		{
			...
			for(i = 0; i < ntags; ++i)
			{
				...
				else if(strstr(buf,"Cir")||strstr(buf,"cir"))
				{ /* circle */
					fgets  (buf, BUFSIZ, fp);
					sscanf (buf, "%*lf%*lf%lf%1s",&(ctmp[i].info.arc.radius),
							tagid+i);
					ctmp[i].type = T_Arc;
				}
				...
				else if(strstr(buf,"Cyl")||strstr(buf,"cyl"))
				{ /* cylinder */
					fgets (buf, BUFSIZ, fp);
				...
				else if(strstr(buf,"Con")||strstr(buf,"con"))
				{  /* cone */
					fgets (buf, BUFSIZ, fp);
					sscanf (buf, "%lf%lf%lf%lf%lf%lf%lf%1s",
				...
				else if(strstr(buf,"Spi")||strstr(buf,"spi"))
				{ /* cylinder */
					fgets (buf, BUFSIZ, fp);
				...
				else if(strstr(buf,"Tau")||strstr(buf,"tau"))
				{ /* Taurus ?? */
					fgets (buf, BUFSIZ, fp);
				...
				else if(strstr(buf,"Sph")||strstr(buf,"sph"))
				{ /* sphere */
					fgets (buf, BUFSIZ, fp);
					sscanf (buf, "%lf%lf%lf%lf%1s",
					        &(ctmp[i].info.sph.xc), &(ctmp[i].info.sph.yc),
					        &(ctmp[i].info.sph.zc), &(ctmp[i].info.sph.radius),
							tagid+i);
					ctmp[i].type = T_Sphere;
				}
				else if(strstr(buf,"Fil")||strstr(buf,"fil"))
				{ /* Felisa file input */
					char file[BUFSIZ];
					fgets  (buf, BUFSIZ, fp);
					sscanf (buf, "%s",file);
			...
			if (k)
			{
				double d_eid;
				if(!ntags)
					error_msg(No curved type information specified);
				...
				/* for some reason Linux doesn't read in first integer
				   properly so am using a double read followed by a type cast */
					...
					{
						fprintf(stderr,"ReadMesh -- unable to read curved "
									   "information\n");
						exit(-1);
					}	
					...
					if(j == ntags)
					{
						fprintf(stderr,"can not find tag type `%c\' called in "
								"element %d face %d\n" ,type,eid+1,face);
						exit(-1);
					}
 ...
 * ----------------------------------------------------------------------- */
static void Restart (const char *name, int nfields, Element_List *U[]);
...
void ReadICs (FILE *fp, Domain *omega)
{
	...
	static  const char *ictypes[] = { "PrepOnly", "Restart", "Default", "Given",
	                            "Hardwired", "Minion"};
	...
	switch (type)
	{
		...
		case 3:
			{                          /* Function(x,y,z) Initial conditions */
				char    *s;
				...
				{
					fprintf(stdout,"Setting up initial field as exact "
							"Womersley solution at t = %lf \n",
							dparam("STARTIME"));
				...
				else
				{
					...
					for (i = 0; i < nfields && n--; i++)
					{
						...
						{
							fprintf(stderr,"ReadICs: the following function is "
									"invalid %s",s);
							exit(-1);
						}
						while (isspace(*++s))
	...
	while (n--&&n>0)
		fgets(buf, BUFSIZ, fp); /* read to the end of the section */
...
static void Restart (const char *name, int nfields, Element_List **U)
{
	...
	{
		fputs("ReadICs: couldn't read the restart file name", stderr);
		exit(-1);
	}

	if ((fp = fopen(fname,"r")) == (FILE *) NULL)
	{
		fprintf(stderr,"ReadICs: restart file %s not found",fname);
		exit(-1);
	}
	...
	while (readField (fp, &f))
		dump++;
	if (!dump)
	{
		fputs("Restart: no dumps read from restart file", stderr);
		exit(-1);
	}

	if (f.dim != U[0]->fhead->dim())
	{
		fputs("Restart: file if wrong dimension", stderr);
		exit(-1);
	}

	if (f.nel != nel)
	{
		fputs("Restart: file is the wrong size", stderr);
		exit(-1);
	}
...
void ReadDF(FILE *fp, int nforces, ...)
{
	...
	int  nlines;
	char buf[BUFSIZ], *p;
	static const char *ftypes[] = { "FFX", "FFY", "FFZ" };

	if (!findSection (SEC_DF, buf, fp))
	{
		fputs("ReadDF: section not found", stderr);
		exit(-1);
	}
	if (sscanf(fgets(buf, BUFSIZ, fp), "%d", &nlines) != 1)
	{
		fputs("ReadDF: can't read the number of lines", stderr);
		exit(-1);
	}
 ...
 *    N      Wall Normal   Vnormal		  Vnormal        Vnormal            *
 *			 velocity														*
 *																			*
 * NOTES:                                                                   *
 *                                                                          *
...
Bndry *ReadMeshBCs (FILE *fp, Element_List *Mesh)
{
	...
	for(E=Mesh->fhead;E;E=E->next)
	{
		...
		for (n = 0; n < ntot; ++n)
		{
			...
			{
				sprintf(buf, "Mismatched element/side -- got %d,%d, expected "
						"%d,%d", iel, iside, E->id+1, n+1);
			...
			switch (bc)
			{
				case 'Z':
					...
					else if(f[2])
					{
						...
						fprintf(stderr,"Error: Symmetry boundary in elmt %d, "
								"face %d does not have a direction "
								"prescribed\n",iel,iside);
					break;

				case 'N': // NEW: Wall normal velocity Boundary condition
				case 'W':
				case 'V':
				case 't':
				case 'w': 
...
Bndry *ReadBCs (FILE *fp, Element *U, AdvVel *vel)
{
	register int   n,i;
	char       bc, buf[BUFSIZ], *p;
	...
	for(k =0; (k < pllinfo.gnel) && (E); ++k)
	{
		...
		{ /* skip to next element */
			...
			for(n = 0; n < ntot; ++n)
			{
				...
				if(!p)
				{
					sprintf(buf,"Found end of file whilst reading element %d, "
							"side %d\n", E->id+1, n+1);
					exit(-1);
				}
			}
			...
			for (n = 0; n < ntot; ++n)
			{
				...
				if (iel - pllinfo.eloop[E->id] != 1 || iside - n != 1)
				{
					sprintf(buf, "Mismatched element/side -- got %d,%d, "
							"expected %d,%d", iel, iside,
							pllinfo.eloop[E->id]+1, n+1);
					fprintf(stderr,"ReadBCs : %s\n", buf);
					exit(-1);
				}
				...
				switch (bc)
				{
					...
					case 'Z': /* symmetry boundary condition */
								/* set dirichlet condition depending on
								   definition of f[fnum] */
						if(f[fnum])
						{
							E->set_solve(n,off);
					...
					case 'N': 
					if(fnum==0) option_set("NORMAL0",1);
					else if(fnum==1) {option_set("NORMAL0",0); option_set("NORMAL1",1);}
					else if(fnum==2) {option_set("NORMAL1",0);  option_set("NORMAL2",1);}
					else 
						fprintf(stderr,"Error: Wall normal velocity boundary in elmt %d, "
								"face %d : no normal velocity prescribed "
								"prescribed\n",iel,iside);
					...
					case 't':
								case 'w': // flag for Womersley boundary
                                case 'v': 
						E->set_solve(n,off);
					case 'f':
					case 'r':
						{
							...
							for (i = 0; i <= fnum; i++)
								fgets(buf, BUFSIZ, fp);
							//if (iparam("INFLOW")||iparam("INFLOW2D"))
							if (vel!= NULL)
								new_bndry = E->gen_bndry(vel, bc, n, buf);
								else{
							new_bndry = E->gen_bndry(bc, n, buf);
                                                        ...
                                                        new_bndry->usrtype = bc; 
                                                        // treat Womersley 
                                                        // as standard velocity condition
                                                       if(bc == 'w') 
                                                        {
                                                            new_bndry->type = 'V';
                                                        }
							...
							do	// Search through the following lines for the   
							{	// first one without an '=' (function 
								fgetpos (fp, &pos); 	//   specification)
								fgets   (buf, BUFSIZ, fp);  
							}
							while (strchr (buf, '='));  
					...
					case 'P':
						...
						break;
					default:
						fputs("ReadBCs: read a strange boundary condition",
							  stderr);
						exit(-1);
						break;
				}
...
Bndry *bsort(Bndry *bndry_list, int nbcs)
{
	...
	for(i = 0 ; i < nbcs; i++, bedg = bedg->next)
		tmp[i] = bedg;
...
error_msg(ReadBCs: do
			          not see any boundary conditions);
...
void ReadSetLink(FILE *fp, Element_List *U)
{
	...
	if (!strstr(buf, SEC_BCS))
	{
		fprintf(stderr,"ReadSetLink: do not see any boundary conditions\n");
		exit(-1);
	}
	fgets(buf,BUFSIZ,fp);
			...
			for(i=0;i<E->Nfaces;++i)
				if(E->face[i].link)
					if(E->face[i].link->link->eid != E->id ||
						...
						fprintf(stderr,"Error in ReadSetLink: Elmt %d, Face %d "
									   "not matched\n", E->id+1, i+1);
...
static void set_edge_con(Edge *edg1, Edge *edg2, int trip)
{
	...
	if(trip)
	{ /* set two edges to different connectivity */
	...
	{  /* set both edges to same connectivity */
...
static void set_link(Element_List *U, int eid1, int id1, int eid2, int id2)
{
	...
	else
	{
		...
		if(eid1 > eid2)
			return; /* just do first connection in list */
		...
		if(nfv1 != nfv2)
		{
			fprintf(stderr, "Set_link: Error in connectivity, two unlike faces "
					"connected\n");
			exit(-1);
		}

		if(nfv1 == 3)
		{  // triangle meets triangle

			if(con_test_a[id1] == con_test_b[id2])
			{
				E->face[id1].con = 1;

				set_edge_con(E->edge+E->ednum(id1,0),
							 F->edge+F->ednum1(id2,0), 1);
				edge_link   (E->edge+E->ednum(id1,0), F->edge+F->ednum1(id2,0));

				for(i = 1; i < nfv1; ++i)
				{
					set_edge_con(E->edge+E->ednum(id1,i),
								 F->edge+F->ednum1(id2,i),0);
					edge_link   (E->edge+E->ednum(id1,i),
								 F->edge+F->ednum1(id2,i));
				}
			}
			else
			{
				...
				{
					set_edge_con(E->edge+E->ednum(id1,i),
								 F->edge+F->ednum(id2,i),0);
					edge_link   (E->edge+E->ednum(id1,i),
								 F->edge+F->ednum(id2,i));
				}
			}
		...
		{   // quad. face meets quad. face
			...
			if(i==nfv2)
			{
				fprintf(stderr,"Set_link: elmt:%d face:%d and elmt:%d face:%d "
						"do not align", eid1, id1, eid2, id2);
				exit(-1);
			}

			int con[4] = {0,0,0,0};

			if(map[1] == (map[0]+1)%nfv2)
			{ // same orientation
				switch(map[0])
				{
					case 0:
			...
			{                         // mirror orientation
				switch(map[0])
				{
					...
					case 1:
						...
						con[0] = con[2] = 1;  // negate 'a' edges
						break;
					case 2:					// negate 'a' and 'b' and transpose
						E->face[id1].con = 3+4; 
						con[0] = con[1] = con[2] = con[3] = 1;
						break;
					case 3:
...
error_msg(CheckOrderFIle: Illegal function definition);
			...
			for(k=0;k<nel;++k)
			{
				...
				f[F->Nverts] =(F->identify()==Nek_Tri ||
							   F->identify()==Nek_Nodal_Tri) ? max_f-1.0:max_f;

				if(n=option("aposterr"))
					for(i = 0; i < F->Nedges+1; ++i)
						size[i] = (int)max(f[i],0.0) + n;
				else
					for(i = 0; i < F->Nedges+1; ++i)
						size[i] = (int)max(f[i],0.0);
#else
				...
				for(i = 0; i < 4; ++i)
				{
					f[i+6] = max(max(f[ednum[i][0]],f[ednum[i][1]]),
								 f[ednum[i][2]])-1;
					f[10]  = max(f[10],f[i+6]);
				}
				--f[10];
...
void ReadHisData (FILE *fp, Domain *omega)
{
	...
	{
		fputs("ReadHisData: Section not found", stderr);
		exit(-1);
	}
	if (sscanf(fgets(buf, BUFSIZ, fp), "%d", &npts) != 1)
	{
		fputs("ReadHisData: can't read the number of points", stderr);
		exit(-1);
	}
	...
	while (npts--)
	{
		...
		{
			if ((--hp->i) > omega->U->flist[atoi(p)-1]->Nedges ||
				(hp->id--) > nel)
			{
				free(hp);
				continue;
	...
	int myhis, ghis;
	myhis = (his_list) ? 1 : 0;
	MPI_Reduce(&myhis, &ghis, 1, MPI_INT, MPI_LOR, 0, MPI_COMM_WORLD);
	ROOTONLY
	{
		if (ghis) puts("History points:");
	}
		...
		DO_PARALLEL
		{
		    for (hp = his_list, cnt = 0; hp; hp = hp->next)
	    	{
			    if(hp->mode == TransVert)
				    printf ("%2d: Proc %d, vertex = %2d, elmt = %3d, fields = "
							"%s\n", ++cnt,pllinfo.procid, hp->i + 1,
							pllinfo.eloop[hp->id] + 1, hp->flags);
			    else if(hp->mode == TransEdge)
				    printf ("%2d: Proc %d, edge   = %2d, elmt = %3d, fields = "
							"%s\n", ++cnt,pllinfo.procid, hp->i + 1,
						    pllinfo.eloop[hp->id] + 1, hp->flags);
		    }
		}
...
void ReadDFunc(FILE *fp, Domain *Omega)
{
	...
	if(strstr(p = fgets(buf, BUFSIZ, fp),"File"))
	{
		...
		UL[2]->fhead->type = 'j';
		Restart(buf, 3, UL);
		UL[0]->Trans(UL[0], J_to_Q);
		UL[0]->fhead->type = 'u';
		UL[1]->Trans(UL[1], J_to_Q);
		UL[1]->fhead->type = 'v';
		UL[2]->Trans(UL[2], J_to_Q);
		UL[2]->fhead->type = 'w';
		;
	...
	if(findSection(SEC_TIME,buf,fp))
	{
		...
		if(strstr(buf,"Str")||strstr(buf,"str"))
		{
			...
			for(i = 0; i < n; ++i)
			{
				...
				if((p = strchr(p,'=')) == (char *) NULL)
				{
					fprintf(stderr,"ReadTimeDepend: have not defined all time "
							"functions\n");
					exit(-1);
				}
...
/* ----------------------------------------------------------------------- *
 * ReadInflow() - Read Inflow BC (Dirichlet BC)                            *
 *                                                                         *
 *      This function reads the inflow BC from a 3D fld file specified     *
 *      in the .rea file.                                                  *
 *                                                                         *
 *      2 INFLOW FILE ****                                                   *
 *      Restart                                                            *
...
static void Restart_Inflow(const char *name, int nfields, Domain *omega,
                        Element_List *Mesh);

void ReadInflow (FILE *fp, Domain *omega, Element_List *Mesh)
{

	Element_List *U = omega->Usys->vel->advU;
	Element_List *V = omega->Usys->vel->advV;
	Element_List *W = omega->Usys->vel->advW;
	double scalefactor = dparam("ScaleFactor");

	if (fabs(scalefactor) < 1e-14)
	{
		scalefactor = 1.0;
	}
	...
	if (!findSection (SEC_INFLOW, buf, fp))
	{
		fprintf(stderr,"ReadInflow: no advection specified "
		        "(last line read below) %s", buf);
		exit(-1);
	}
	if (sscanf(buf, "%d", &n) != 1)
	{
		fprintf(stderr,"ReadInflow: # component of velocity field is not specified "
		        "(last line read below) %s", buf);
		exit(-1);
	}
	...
	switch (type)
	{
		...
		case 1:                             /* Restart from a file */
		{
			fgets   (buf, BUFSIZ, fp);
			n--;
			Restart_Inflow (buf,nfields, omega, Mesh);
			break;
		}
		case 2:                             /* Default Velocity Field */
		{
		...
		case 3:                            /* Function(x,y,z) Velocity field */
		{
			char    *s;

			ROOTONLY printf("Inflow BC   :");

			/*Read x component of velocity*/
			if (!(s = strchr(fgets(buf,BUFSIZ,fp),'=')))
			{
				fprintf(stderr,"ReadInflow: the following function is invalid %s",s);
				exit(-1);
			}
			...
			ROOTONLY
			{
				printf ("                     %c = %s", U->fhead->type, s);
			}

			/*Read y component of velocity*/
			if (!(s = strchr(fgets(buf,BUFSIZ,fp),'=')))
			{
				fprintf(stderr,"ReadInflow: the following function is invalid %s",s);
				exit(-1);
			...
			ROOTONLY
			{
				printf ("                     %c = %s", V->fhead->type, s);
			}

			/*Read z component of velocity*/
			if (dim==3)
			{
				if (!(s = strchr(fgets(buf,BUFSIZ,fp),'=')))
				{
		...
		default:
			fputs("ReadInflow: invalid advection field", stderr);
			exit(-1);
			break;
	}

	dscal(U->hjtot,scalefactor,U->base_hj,1);
	dscal(V->hjtot,scalefactor,V->base_hj,1);
	if (dim == 3)
		dscal(W->hjtot,scalefactor,W->base_hj,1);
                        ...
                        Element_List *Mesh)
{
	...
	if (sscanf(name, "%s", fname) != 1)
	{
		fputs("ReadInflow: couldn't read the restart file name", stderr);
		exit(-1);
	}

	if ((fp = fopen(fname,"r")) == (FILE *) NULL)
	{
		fprintf(stderr,"ReadInflow: restart file %s not found",fname);
		exit(-1);
	}
	...
	ROOTONLY
	{
		if (dparam("ReverseAdvection"))
			printf("Velocity field     : %s at t = %g negated", fname, f.time);
		else
			printf("Velocity field     : %s at t = %g", fname, f.time);
		putc ('\n', stdout);
	}

	/* Copy x component of velocity field into advU */
	if (!(p = strchr (f.type, U->fhead->type)))
	{
		sprintf (buf, "variable %c is not on file", U->fhead->type);
		fprintf(stderr,"Restart: %s", buf);
		exit(-1);
	}
	pos = (int) (p - f.type);
	copyfield (&f,pos,U->fhead);
	if (dparam("ReverseAdvection"))
	{
		dneg(U->hjtot,U->base_hj,1);
	}

	/* Copy y component of velocity field into advV */
	if (!(p = strchr (f.type, V->fhead->type)))
	{
		sprintf (buf, "variable %c is not on file", V->fhead->type);
		fprintf(stderr,"Restart: %s", buf);
		exit(-1);
	}
	pos = (int) (p - f.type);
	copyfield (&f,pos,V->fhead);
	
	
	if (dparam("ReverseAdvection"))
	{
		dneg(V->hjtot,V->base_hj,1);
	}

	/* Copy z component of velocity field into advW */
	if (dim==3)
	{
		if (!(p = strchr (f.type, W->fhead->type)))
		{
			sprintf (buf, "variable %c is not on file", W->fhead->type);
			fprintf(stderr,"Restart: %s", buf);
			exit(-1);
		}
		pos = (int) (p - f.type);
		copyfield (&f,pos,W->fhead);

		if (dparam("ReverseAdvection"))
		{
			dneg(W->hjtot,W->base_hj,1);
		}
...
/* ----------------------------------------------------------------------- *
 * ReadInflow2D() - Read Inflow BC (Dirichlet BC)                            *
 *                                                                         *
 *      This function reads the inflow BC from a 2D fld file specified     *
 *      in the .rea file.                                                  *
 *                                                                         *
 *      2 INFLOW2D FILE ****                                                      *
 *      Restart                                                            *
...
static void Restart_Inflow2D(const char *name, int nfields, Domain *omega,
                        Bndry *Meshbc);
void ReadInflow2D (FILE *fp, Domain *omega, Bndry *Meshbc)
{
	...
	if (!findSection (SEC_INFLOW2D, buf, fp))
	{
		fprintf(stderr,"ReadInflow2D: no advection specified "
		        "(last line read below) %s", buf);
		exit(-1);
	}
	if (sscanf(buf, "%d", &n) != 1)
	{
		fprintf(stderr,"ReadInflow2D: # component of velocity field is not specified "
		        "(last line read below) %s", buf);
		exit(-1);
	}
	...
	switch (type)
	{
		...
		case 1:                             /* Restart from a file */
		{
			fgets   (buf, BUFSIZ, fp);
			n--;
			Restart_Inflow2D (buf,nfields, omega, Meshbc);
			break;
		}
		case 2:                             /* Default Velocity Field */
		{

			fputs("ReadInflow2D: Default 2D Inflow Boundary Condition not implemented", stderr);
			exit(-1);
			break;
		}
		...
		case 3:                            /* Function(x,y,z) Velocity field */
		{
			...
			fputs("ReadInflow2D: Functional 2D Inflow Boundary Condition not implemented", stderr);
			exit(-1);
			break;
		...
		default:
			fputs("ReadInflow2D: invalid inflow field", stderr);
			exit(-1);
			break;
	}
...
/* initialize the Element pointers */
	...
	U[0] = omega->Usys->vel->advU;
	U[1] = omega->Usys->vel->advV;
	U[2] = omega->Usys->vel->advW;
	...
	char    *p, fname[FILENAME_MAX], buf[BUFSIZ];
	register int i, pos, dump, nel;
	int dim = U[0]->fhead->dim();

	if (sscanf(name, "%s", fname) != 1)
	{
		fputs("ReadInflow2D: couldn't read the restart file name", stderr);
		exit(-1);
	}

	if ((fp = fopen(fname,"r")) == (FILE *) NULL)
	{
		fprintf(stderr,"ReadInflow2D: restart file %s not found",fname);
		exit(-1);
	}
