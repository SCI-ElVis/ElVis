...
class Tet: public Element
{
	...
	public:
		...
		// Error functions
		void Set_field(const char *sTetng);               // set field to function
		void Error(const char *sTetng);                   // compare with function
		double L2_error_elmt(const char *sTetng);
		double H1_error_elmt(const char *sTetng);
		void Verror(double *u, const char *sTetng);
		double Int_error( const char *sTetng);
		double Norm_li();
		double Norm_l2();
