...
class Pyr: public Element
{
	...
	public:
		...
		Pyr& operator*=(Pyr&);					// multiplying operator
		Pyr& operator*=(double&);               // multiplying operator
		...
		// Error functions
		void Set_field(const char *sPyrng);				// set field to function
		void Error(const char *sPyrng);					// compare with function
		double L2_error_elmt(const char *sPyrng);
		double H1_error_elmt(const char *sPyrng);
		void Verror(double *u, const char *sPyrng);
		double Int_error(const char *sPyrng);
		double Norm_li();
		double Norm_l2();
