...
class Quad: public Element
{
	...
	public:
		...
		Quad& operator*=(Quad&);				// multiplying operator
		Quad& operator*=(double&);				// multiplying operator
		...
		void Trans(Element *, Nek_Trans_Type);		// Transform to Quad
		void Iprod(Element *);						// Inner product to Quad
		void Iprod_d(Element *,Basis *, Basis *);	// Inner product to Quad
		void Sign_Change();
		...
		// Error functions
		void Set_field(const char *string);			// set field to function
		void Error(const char *string);				// compare with function
		double L2_error_elmt(const char *string);
		double H1_error_elmt(const char *string);
		void Verror(double *u, const char *string);
		double Int_error(const  char *string);
		double Norm_li();
		double Norm_l1();
		double Norm_l2();
		...
		double     get_1diag_massmat(int id);  /* get diagonal component of
