...
namespace NekBlkMat
{
	...
	class BlkMat
	{
		public:
			...
			{ // set up _offset if not already done at init
				...
				if(!_offset)
					_offset = new int [_max_colblk+1];
				vmath::zero(_max_colblk+1,_offset,1);

				for(int i = 0; i < _max_rowblk; ++i)
					for(unsigned int j = 0; j < _Row[i]._Blk.size(); ++j)
						update_offset(_Row[i]._Blk[j]->get_id(),
						              _Row[i]._Blk[j]->get_cols());
			}
    ...
    void PrintBlks(FILE *out){

      if(!_offset) setup_offset();
				   
      for(int i = 0; i < _max_rowblk; ++i)
	_Row[i].PrintBlks(_offset,out);
			...
			BlkMat& geMxM (MatStorage formA, MatStorage formB,
			               const double& alpha, const BlkMat& A,
			               const BlkMat& b, const double& beta);
			...
			double* geMxv (MatStorage form, const double& alpha,
			               const double *v, const double& beta, double *y);
