...
namespace NekBlkMat
{
	...
	class BlkVec
	{
		...
		public:
			...
			~BlkVec()
			{
				for(unsigned int i = 0; i < _Blk.size(); ++i)
					delete _Blk[i];
				delete[] _entries;
			}
     ...
     void PrintBlks(const int *offset,FILE *out){
       int i,j,k,id,cols;
       double *mat;
	 ...
	 fprintf(out," -- Empty row -- \n");
			...
			BlkVec& operator = (const BlkVec &a)
			{
				if(this != &a)
				{
					...
					for(unsigned int i = 0; i < a._Blk.size(); ++i)
					{
