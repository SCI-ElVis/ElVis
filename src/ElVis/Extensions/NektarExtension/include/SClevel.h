...
namespace MatSolve
{
	...
	class SClevel
	{
		...
		private:
			...
			/// This is an alternative if BlkMat not defined - assumes that
			/// All Blk Matrices are defined within.
			...
			double *_signchange;  ///< signchange vector for local to global
			///< mapping
		...
		public:
			...
			{ ///< default destructor
				if(_eidmap)
					delete[] _eidmap;
				// currently do not delete signchange as not directly
				// attached to class
				if(_Am)
					delete _Am;
				if(_Bm)
			...
			*/
			...
			void Condense()
			{
				...
				if(_state != Filled)
					NekError::error(fatal,"SClev::Condense()",
					                "Matrices not filled");
			...
			/**
			Calculate (this){^-1} by Matrix and assuming that matrix is
			...
			**/
			...
			void CondenseV(double *rhs)
			{
				...
				if(_signchange)
					vmath::vmul(_Asize,_signchange,1,rhs,1,rhs,1);
			}

			/**  Pack all boundary elements in array \a in to array \a out
			 according to the mapping given by \a (*this)->locmap.
			...
			*/
			...
			/**  UnPack all boundary elements in array \a in to array \a out
			 according to the mapping given by \a (*this)->_locmap.
			...
			*/
			void UnPackV(const double *in, double *out)
			{
				...
				for(i = 0; i < _Asize; ++i)
					if((id = _locmap[i])+1)
						out[i] = in[id];
					else				// need to be zero for correct
						out[i] = 0.0; 	// interior solve
			}
			...
			*/
			void SolveVd(double *V)
			{
				...
				if(_signchange)
					vmath::vmul(_Asize,_signchange,1,V,1,V,1);
			...
			*/
			...
			void Assemble(SClevel& NewSClev, const int *noffsetA,
			              const int *noffsetC)
			{
				...
				// number of Blks in (this) SClevel sys
				nrow   = _Am->get_mrowblk();
				offset = _Am->get_offset();

				// max number of A entries
				max_A = noffsetA[NewSClev._Am->get_mrowblk()];
				...
				for(p = 0; p < nrow; ++p) // assumes system is square
					for(q = 0; q < nrow; ++q)
						if( (Asub = _Am->get_mat(p,q,nr,nc)) )
						{
							...
							if(_signchange )
							{
								// signchange necessary on first level
								if(p == q)
							...
							for(i =0; i < nr; ++i)
								for(j = 0; j < nc; ++j)
								{
									...
									if((id1+1)&&(id2+1))
									{
										if((id1 < max_A)&&(id2 < max_A))
										{ // project to _A
											id1 -= noffsetA[neidr];
											id2 -= noffsetA[neidc];
											NewSClev._Am->GenBlk(neidr,neidc,
											  noffsetA[neidr+1]-noffsetA[neidr],
											  noffsetA[neidc+1]-noffsetA[neidc],
											  id1,id2,val);
										}
										else if((id1 < max_A)&&(id2 >= max_A))
										{ // project to _B
											id1 -= noffsetA[neidr];
											id2 -= noffsetC[neidc];

											NewSClev._Bm->GenBlk(neidr,neidc,
											  noffsetA[neidr+1]-noffsetA[neidr],
											  noffsetC[neidc+1]-noffsetC[neidc],
											  id1,id2,val);
										}
										else if((id1 >= max_A)&&(id2 >= max_A))
										{ // project to _C
											...
											if(fabs(val) > ASS_TOL)
											{
												NewSClev._Cm->GenBlk(neidr,
										neidc,noffsetC[neidr+1]-noffsetC[neidr],
										noffsetC[neidc+1]-noffsetC[neidc],
										id1,id2,val);
												if(neidr != neidc)
													NekError::error(warning,
											"SClevel::Assemble",
										...
										else if (!_symmetric)
										{
											...
											{ // project to _D^T
												...
												NewSClev._Dm->GenBlk(neidc,
										neidr,noffsetA[neidr+1]-noffsetA[neidr],
										noffsetC[neidc+1]-noffsetC[neidc],
										id2,id1,val);
											}
										}
