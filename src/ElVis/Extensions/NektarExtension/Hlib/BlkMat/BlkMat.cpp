...
namespace NekBlkMat
{
	                    ...
	                    const double *mat)
	{
		...
		if((en = _entries[id])+1)
		{// entry already exists so add matrix
	                    ...
	                    const int id1, const int id2, const double val)
	{
		...
		{// entry already exists so add matrix
			if((_Blk[en]->get_rows() != rows)||(_Blk[en]->get_cols() != cols))
				NekError::error(warning,"BlkVec::GenBlk","add value to Blk "
				"which does not have the same dimensions as calling argument");
		...
		(const BlkVec& a, const BlkVec& b)
	{
		...
		{ // need to to b-matrix first
			...
			for(i = 0; i < a._Blk.size(); ++i)
			{
				...
				{ // need to declare memory and copy  in a matrix
		...
		else
		{
			...
			// check over 'b' vector entries for any independent entries not in
			// 'a'
			...
			//If original row is larger than new contributions then delete extra
			//terms
			if(_Blk.size() != cnt)
			{
	...
	BlkVec& BlkVec::sub(const BlkVec& a, const BlkVec& b)
	{
		...
		else
		{
			...
			// If original row is larger than new contributions then delete
			// extra terms
			if(_Blk.size() != cnt)
			{
				for(i = 0; i < _Blk.size(); ++i)
	...
	BlkVec& BlkVec::MxMpM(const BlkSubMat& a, const BlkVec& b)
	{
		...
		for(i = 0; i < b._Blk.size(); ++i)
		{
			if(b._Blk[i]->get_rows() != a.get_cols())
				NekError::error(warning,"BlkVec::MxMpM",
								"cols and rows do not match");
	...
	BlkVec& BlkVec::MtxMpM(const BlkSubMat& a, const BlkVec& b)
	{
		...
		for(i = 0; i < b._Blk.size(); ++i)
		{
			if(b._Blk[i]->get_rows() != a.get_rows())
				NekError::error(warning,"BlkVec::axbpy",
								"cols and rows do not match");
	...
	BlkMat& BlkMat::axpy(const double alpha, const BlkMat& A)
	{
		...
		if((A._max_rowblk != _max_rowblk)||(A._max_colblk != _max_colblk))
			NekError::error(warning,"BlkMat::axMpM",
							"Matrix rows/cols not compatible");
	...
	BlkMat& BlkMat::diagMxy(const BlkMat& A)
	{
		...
		for(i=0; i < _max_rowblk; ++i)
		{
			...
			if(A._Row[i]._Blk[0]->get_rows() !=
				...
				NekError::error(fatal,"Blkmat::diagMxy",
	...
	// (this) = alpha* A * B + beta (this)
	BlkMat& BlkMat::geMxM(MatStorage formA, MatStorage formB,
						  const double& alpha, const BlkMat& A, const BlkMat& B,
						  const double& beta)
	{
		...
		switch(formB)
		{
			case RowMajor:
				...
				{
					case RowMajor:
						// multiply every entry in row of B by A matrix and 
						// put in C[i]
						for(i=0; i < A._max_rowblk; ++i)
							for(jj=0;jj < A._Row[i]._Blk.size(); ++jj)
							{
								id = A._Row[i]._Blk[jj]->get_id();
								_Row[i].geMxM(formA,formB,alpha,
										 A._Row[i]._Blk[jj][0],B._Row[id],beta);
					...
					case ColMajor:
						...
						for(i=0; i < A._max_rowblk; ++i)
							...
							{
								id = A._Row[i]._Blk[jj]->get_id();
								_Row[id].geMxM(formA,formB,alpha,
										A._Row[i]._Blk[jj][0],B._Row[i],beta);
							}
						break;
					default:
			...
			case ColMajor:
				...
				{
					case RowMajor:
						// Can not use BlkVec since Blk row ordering is not
						// consistent
						for(i=0; i < A._max_rowblk; ++i)
								...
								for(k = 0; k < A._max_colblk; ++k)
								{
									if(((ida=A._Row[i]._entries[k])+1)
										...
										if((id = _Row[i]._entries[j])+1) 
											...
											_Row[i]._Blk[id]->geMxM(formA,formB,
												alpha,A._Row[i]._Blk[ida][0],
												B._Row[j]._Blk[idb][0],beta);
										else
										{ // declare and put in matrix
											Btmp = new BlkSubMat(j,
											   A._Row[i]._Blk[ida]->get_rows(),
											   B._Row[j]._Blk[idb]->get_rows());
											Btmp->geMxM(formA,formB,alpha,
												   A._Row[i]._Blk[ida][0],
											       B._Row[j]._Blk[idb][0],beta);
								...
								for(k = 0; k < A._max_rowblk; ++k)
								{
									if(((ida=A._Row[k]._entries[i])+1)
										...
										if((id = _Row[i]._entries[j])+1) 
											// add local contribution
											_Row[i]._Blk[id]->geMxM(formA,formB,
												alpha,A._Row[k]._Blk[ida][0],
												B._Row[j]._Blk[idb][0],beta);
										else
										{ // declare and put in matrix
											Btmp = new BlkSubMat(j,
											   A._Row[k]._Blk[ida]->get_cols(),
											   B._Row[j]._Blk[idb]->get_rows());
											Btmp->geMxM(formA,formB,alpha,
												   A._Row[k]._Blk[ida][0],
											       B._Row[j]._Blk[idb][0],beta);
	...
	BlkMat& BlkMat::invert_diag()
	{
		...
		for(i = 0; i < _max_rowblk; ++i)
		{
			...
			if((id = _Row[i]._entries[i])+1)
				...
				NekError::error(warning,"BlkMat::invert",
