...
namespace MatSolve
{
	...
	void NekMat::SetupSC(Element_List& U,Bsystem *B,Bndry *Ubc,SolveType Stype)
	{
		...
		if(B->lambda->wave)
			option_set("Oseen",1);
			// required for mat_mem to allocate correct storage
				...
				case Helm:
					...
					for(Bc=Ubc;Bc;Bc=Bc->next)
						if(Bc->type == 'R' && Bc->elmt->id == E->id)
							E->Add_robin_matrix(Mat, Bc, B->vel);
		...
		{
			SClev[lev]._locmap = new int[ SClev[lev]._Am->
										  get_tot_row_entries()];
		...
		if(SlvType == Direct)
		{
			...
			// assemble into global matrix system taking account of signchange
			// if nec.
			Ainv->Assemble(SClev[lev]._Am[0],SClev[lev]._locmap,
			               SClev[lev]._signchange);

			// release memory associated with row matrix storage but set up
			// offset !
			SClev[lev]._Am->setup_offset();
			SClev[lev]._Am->Reset_Rows();
	...
	*/
	                   ...
	                   Bsystem& Ubsys, SolveType Stype)
	{
		if(U.base_hj == Uf.base_hj)
			cerr << "Warning: in NekMat::Solve U must be independent of Uf" 
				 << endl;
