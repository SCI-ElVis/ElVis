...
namespace MatSolve
{
	...
	*/
	void Gmat::Factor()
	{
		...
		switch(mat_form)
		{
			...
			case General_Banded:
				_ipiv = new int[_lda];
				lapack::dgbtrf(_lda,_lda,_ldiag,_bwidth-1,_Am,2*_ldiag+_bwidth,
							   _ipiv,info);
				assert(info==0);
				break;
			case General_Full:
	...
	*/
									...
									for(i = 0; i < n; ++i)
										if((idi=map[offset[p]+i])+1)
											if(idj <= idi)
											{
												sign = (signchg)?
												       signchg[offset[q]+j]*
													   signchg[offset[p]+i] : 1;
												_Am[idi+idj*(2*_lda-idj-1)/2] +=
															   sign*Asub[i*n+j];
											}
				break;
			case Symmetric_Positive_Banded:
									...
									for(i = 0; i < n; ++i)
										if((idi=map[offset[p]+i])+1)
										{
											sign = (signchg)?
											       signchg[offset[q]+j]*
												   signchg[offset[p]+i] : 1;
											_Am[idj*(2*_ldiag+_bwidth)+
												_ldiag+_bwidth-1+(idi-idj)] +=
															  sign*Asub[i*n+j];
										}
				break;
			case General_Full:
									...
									for(i = 0; i < n; ++i)
										if((idi=map[offset[p]+i])<nsolve)
										{
											sign = (signchg)?
											       signchg[offset[q]+j]*
												   signchg[offset[p]+i] : 1;
											_Am[idj*(2*_ldiag+_bwidth)+_ldiag+
												_bwidth-1+(idi-idj)] += 
															   sign*Asub[i*n+j];
										}
				break;
	...
	double Gmat::L2ConditionNo()
	{
		...
		if(min < 1e-11)
		{ // if min < 1e-11 find second smallest ev
	...
	void Gmat::Spectrum(double *er, double *ei, double *evecs)
	{
		...
		switch(mat_form)
		{
			...
			case Symmetric_Positive_Banded:
				{
					double *work  = new double [3*_lda];
					vmath::zero(_lda,ei,1);
					lapack::dsbev('N','L',_lda,_bwidth-1,_Am,_bwidth,er,&dum,1,
								  work,info);
					assert(info==0);
					delete[] work;
					break;
			...
			case General_Banded:
				NekError::error(fatal,"GMatrix::Spectrum","Eigenvalue "
						"evaluation for genaral baneded matrix needs coding");
				break;
			case General_Full:
				{
	...
	void Gmat::plotmatrix()
	{
		...
		switch(mat_form)
		{
			case Symmetric_Positive:
			case Symmetric:
				NekError::error(warning,"Gmat:showmatrix",
								"Not set up to show matrix");
				break;
			case Symmetric_Positive_Banded:
				NekError::error(warning,"Gmat:showmatrix",
								"Not set up to show matrix");
	...
	void Gmat::WriteMatrix(FILE *fp)
	{
		...
		switch(mat_form)
		{
			case Symmetric_Positive:
			case Symmetric:
				NekError::error(warning,"Gmat:WriteMatrix",
								"Not set up to show matrix");
				break;
			case Symmetric_Positive_Banded:
				NekError::error(warning,"Gmat:WriteMatrix",
