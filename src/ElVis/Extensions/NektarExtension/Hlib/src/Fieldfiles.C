...
#include <math.h>
    ...
    defined(__linux__) || defined (__APPLE_INTEL__)
...
static const char *hdr_fmt_comp[] = {
    "%-25s "            "Session\n",
    "%-25s "            "Created\n",
...
static int checkfmt    (const char *format);
...
int gettypes (char *t, char *s)
{
    ...
    if (!p)
    {
        fprintf(stderr,"invalid \"Fields\" string");
        exit(-1);
    }
...
#if defined(__sgi) || defined(cm5) || defined(_AIX) || defined (__hpux) || \
    defined (__sparc) || defined (_CRAYMPP) ||                          \
    (defined(__APPLE__) && !defined(__APPLE_INTEL__))
#define ieeeb
#endif
...
static int checkfmt (const char *arch)
{
    const char        **p;
...
int writeField (FILE *fp, Field *f, Element *E)
{
    ...
    case 'b':
#ifdef _CRAY
        // int on Crays is 64b, short 32b -
        // use short for portable fieldfiles
        ...
        for (n = 0; n < nfields; n++)
            if (fwrite (f->data[n], sizeof(double), ntot, fp) != 
                (unsigned int)ntot)
            {
...
int readHeader (FILE *fp, Field *f, int *shuff)
{
    ...
    if (fscanf (fp, "%s", buf) == EOF)
        return 0;
    f->name    = (char *)strdup (buf);		// session name
    fgets (buf, BUFSIZ, fp);
    fgets (buf, DESCRIP, fp);
    f->created = (char *)strdup (buf);		// creation date
    fgets (buf, BUFSIZ, fp);
    ...
    //ALE modification - check to see if it is an ALE simulation
    if (strstr(buf,"ALE"))
        f->isALE = true;
    ...
    {
        fscanf(fp,"%d%d%d%d",&f->nel,&f->dim,&f->lmax,&f->nz);
        fgets(buf,BUFSIZ,fp);
        Fourier = 1;
    }
    else
    {
        fscanf(fp,"%d%d%d",&f->nel,&f->dim,&f->lmax);
        fgets(buf,BUFSIZ,fp);
        f->nz = 1;
    }
    ...
    case 'b':
        {
            ...
            { // old hybrid format
                if(shuffled)
                {
                    ...
                    if(fread(semap, sizeof(short), (size_t) f->nel, fp) !=
                       f->nel)
                    ...
                    if(fread(f->emap, sizeof(int), (size_t) f->nel,fp) !=
                       (unsigned int)f->nel)
                        fprintf (stderr, "error reading emap");
                    if(Bswap)
                        Ibrev(f->nel,f->emap);
...
void ReadDataNormal(FILE *fp, Field *f)
{
    ...
    { /* read in relevant elements */
        ...
        { /* old 'c' code format */
            ...
            for(i = 0; i < pllinfo.nloop; ++i)
            {
                icopy(lskip,gsize + pllinfo.eloop[i]*lskip,1,
                      f->size + i*lskip,1);
                lnfacet[i] = lskip;
            }
        }
                    ...
                    for(i = 0; i < f->nel; ++i)
                    {
                        ...
                        if((eid < pllinfo.nloop)&&(i == pllinfo.eloop[eid]))
                        {
                            if(fread(f->data[n]+dskip,sizeof(double),
                                     (size_t)ntot,fp) != (unsigned int)ntot)
                            {
                                fprintf (stderr, "error reading field %c",
                                         f->type[n]);
                                exit(-1);
                            ...
                            if(Bswap)
                                Dbrev(ntot,f->data[n]+dskip);
                            dskip += ntot;
                            ++eid;
                        }
                        else
                        {
                            if (fread (tmp, sizeof(double), (size_t) ntot, fp)
                                != (unsigned int)ntot)
                            {
                                fprintf (stderr, "error reading field %c",
                                         f->type[n]);
    ...
    { /* standard input routine */
        ...
        case 'a':
            ...
            for (i = 0; i < ntot; i++)
            {
                for (n = 0; n < nfields; n++)
                    ...
                    {
                        fprintf(stderr,"Error: reading field %c, point "
                                "%d\n", f->type[n],i+1);
                        exit(-1);
                    }
            }
        ...
        case 'b':
            {			
                ...
                for (n = 0; n < nfields; n++)
                {
                    if (fread (f->data[n], sizeof(double), (size_t) ntot, fp) 
                        != (unsigned int)ntot)
                    {
...
void ReadDataShuffled(FILE *fp, Field *f)
{
    ...
    DO_PARALLEL
    { /* just read in relevant elements of shuffled data   */
        int j;
        int *gsize, *gnfacet, eid, skip;
        int *lsize, *lnfacet;
            ...
            {    
                ...
                for(i = 0; i < f->nel; ++i)
                {
                    if((cnt < pllinfo.nloop)&&(i == peid[cnt]))
                    {
                        for(j = 0; j < len[i]; ++j)
                            for(n = 0; n < nfields; ++n)
                                if (fscanf (fp, "%lf", f->data[n]+dskip[cnt]+j) 
                                    != 1)
                                {
                                    fprintf(stderr,"Error: reading field %c, "
                                            "element %d\n", f->type[n],i+1);
        ...
        case 'b':
            {   
                ...
                for(i = 0; i < f->nel; ++i)
                {
                    if((cnt < pllinfo.nloop)&&(i == peid[cnt]))
                    {
                        for(n = 0; n < nfields; ++n)
                        {
                            if(fread (f->data[n]+dskip[cnt],sizeof(double),
                                      (size_t)len[i],fp) != (unsigned int)len[i])
                            {
                                fprintf(stderr, "error reading field %c, "
                                        "element %d", f->type[n],i+1);
                                exit(-1);
                            }
                            if(Bswap)
                                Dbrev(len[i],f->data[n]+dskip[cnt]);
                        }
                        ++cnt;
                    }
                    else
                    {
                        for(n = 0; n < nfields; ++n)
                            if(fread (tmp,sizeof(double),(size_t)len[i],fp) 
                               != (unsigned int)len[i])
                            {
                                fprintf(stderr, "error reading field %c, "
                                        "element %d", f->type[n],i+1);
        ...
        case 'a':
            ...
            for (i = 0; i < f->nel; ++i)
                for (j = 0; j < len[f->emap[i]]; ++j)
                    for (n = 0; n < nfields; ++n)
                        if (fscanf (fp, "%lf", f->data[n] + dskip[i]+j) 
                            != 1)
                        {
                            fprintf(stderr,"Error: reading field %c, "
                                    "element %d\n", f->type[n],i+1);
                ...
                for (i = 0; i < f->nel; ++i)
                {
                    for (n = 0; n < nfields; ++n)
                    {
                        if(fread (f->data[n]+dskip[i], sizeof(double),
                                  (size_t)len[f->emap[i]],fp) !=
                           (unsigned int)len[f->emap[i]])
                        {
                            fprintf(stderr, "Error reading field %c, "
                                    "element %d\n", f->type[n],i+1);
                            exit(-1);
                        ...
                        if(Bswap)
                            Dbrev(len[f->emap[i]],f->data[n]+dskip[i]);
                    }
                }
                break;
            }	
...
void copyelfield(Field fld, int nfields, int eid, Element_List **U)
{
	...
	{ /* restart from transformed field at any order */
		...
		for(f=0;f<nfields;++f)
			data[f] += data_len(E->id,size,nfacet,E->dim());
		size += isum(E->id,nfacet,1);
		for(f=0;f<nfields;++f)
			U[f]->flist[eid]->Copy_field(data[f], size);
...
static const char *p_hdr_fmt_exp[] = {
	"Session\n",
	"Created\n",
...
void Writefld(FILE **fp, const char *name, int step, double t,
              int nfields, Element_List *UL[])
{
	...
	f.time      =  t;
	f.time_step =  dparam("DT");
	f.kinvis    =  (fabs(dparam("KINVIS-ORIG")) > FPTOL)?
	...
	// Checking if it is an ALE simulation
	f.isALE = false;
	if (nfields == f.dim+5 && strstr(f.type,"xymn"))
		f.isALE = true;
	...
	if(fp[0])
		...
		fprintf(stderr,"No header file pointer\n");
	if(fp[1])
		writeData  (fp[1],&f);
	else
		fprintf(stderr,"No data file pointer\n");
...
void Writefld(FILE **fp, const char *name, int step, double t,
              int nfields, Element_List *UL, double **data, const char *type)
{
	register int n,i,j;
	time_t   tp;
	char     buf[128],state;
...
void writeHeader(FILE *fp, Field *f)
{
	...
	if (f->isALE == false)
	{
		if(f->nz > 1) /* fourier case */
			sprintf (buf,"%s   Fourier (Hybrid) %s",
					 p_hdr_fmt[2],p_hdr_fmt_exp[2]);
		else
		{
			DO_PARALLEL
			{
				sprintf (buf,"%s  Shuffled (Hybrid) %s",
						 p_hdr_fmt[2],p_hdr_fmt_exp[2]);
			}
			else
				sprintf (buf,"%s           (Hybrid) %s",
						 p_hdr_fmt[2],p_hdr_fmt_exp[2]);
		}
	}
	else
	{
		if(f->nz > 1) /* fourier case */
			sprintf(buf,"%s  ALE Fourier (Hybrid) %s",
					p_hdr_fmt[2],p_hdr_fmt_exp[2]);
		else
		{
			DO_PARALLEL
			{
				sprintf(buf,"%s  ALE Shuffled (Hybrid) %s",
				        p_hdr_fmt[2],p_hdr_fmt_exp[2]);
			}
			else
				sprintf(buf,"%s  ALE    (Hybrid) %s",
						p_hdr_fmt[2],p_hdr_fmt_exp[2]);
		}
	}
		...
		case 'b':
			...
			{ 
			    ...
			    for(i = 0,skip=0; i < f->nel; ++i)
			    {
				    ...
				    for (n = 0; n < nfields; n++)
					{
					    if (fwrite (f->data[n]+dskip, sizeof(double), ntot, fp)
								!= (unsigned int)ntot)
					    {
...
{
	register int i, j, n, nfields, ntot;
	...
	DO_PARALLEL
	{
	    ...
	    {
		    Fourier = 1;
		    fscanf(fp,"%d%d%d%d",&gnel,&f->dim,&f->lmax,&f->nz);
			fgets(buf,BUFSIZ,fp);
		}
		else
		{
			fscanf(fp,"%d%d%d",&gnel,&f->dim,&f->lmax);
			fgets(buf,BUFSIZ,fp);
			f->nz = 1;
		}
	    f->nel = pllinfo.nloop;
	    ...
	    switch (tolower(*f->format))
	    {
		    case 'a':
			    ...
			    if(f->state == 't')
				{
				    ...
				    {
					    el = pllinfo.eloop[i];
					    icopy(pllinfo.efacets[el], gsize + cfs[el], 1,
							  f->size + skip,1);
				    ...
				    for(i = 0; i < gnel; ++i)
				    {
					    ...
					    if((cnt < pllinfo.nloop)&&(i == pllinfo.eloop[cnt]))
					    {
						    for(j = 0; j < ntot; ++j, ++dskip)
							    for(n = 0; n < nfields; ++n)
								    if (fscanf (fp, "%lf", f->data[n] + dskip)
										 != 1)
								    {
		    ...
		    case 'b':
				...
				if(f->state == 't')
			    {
				    ...
				    if(fread(sgsize, sizeof(short), (size_t) cfs[gnel], fp) 
							!= (unsigned int)cfs[gnel])
				    {
				    ...
				    if(fread(gsize, sizeof(int),(size_t)cfs[gnel],fp) 
							!= (unsigned int)cfs[gnel])
				    {
					    fprintf (stderr, "error reading size of field ");
					    exit(-1);
...
int readHeaderP(FILE* fp, Field *f, Element_List *Mesh)
{
	...
	if (fscanf (fp, "%s", buf) == EOF)
		return 0;            /* session name */

	f->name    = (char *)strdup (buf);
	fgets (buf, BUFSIZ, fp);
	fgets (buf, DESCRIP, fp);                               /* creation date */
	f->created = (char *)strdup (buf);
	fgets (buf, BUFSIZ, fp);
	...
	if(strstr(buf,"Shuffled"))
		shuffled = 1; /*check for non-sequential setup*/
	fscanf(fp,"%d%d%d",&f->nel,&f->dim,&f->lmax);
	fgets(buf,BUFSIZ,fp);
	f->nz = 1;
	...
	switch (tolower(*f->format))
	{
		...
		{	
			...
			if(option("ALLTET"))
				fprintf(stderr, "Shuffled file not implemented on cray "
						 "routine\n");
			...
			switch (tolower(*Fld[i]->format))
			{
				case 'a':
					...
					for(k=0;k<Fld[i]->nel;++k)
					{
						...
						{
							for (n = 0; n < nfields; n++)
								if (fscanf(dat_fp,"%lf",GFld->data[n]+gskip+j)
										 != 1)
								{
									fprintf(stderr,"Error: reading field %c, "
											"point %d\n", Fld[i]->type[n],j+1);
				...
				case 'b':
					...
					for (n = 0; n < nfields; n++)
					{
						for(k=0;k<Fld[i]->nel;++k)
						{
							...
							ntot   = datalen[elmt];
							gskip  = cumdatalen[elmt];
							if(fread(GFld->data[n]+gskip, sizeof(double),
									(size_t) ntot, dat_fp)!= ntot)
							{
								fprintf (stderr, "error reading field %c",
										 GFld->type[n]);
								exit(-1);
...
Function Purpose:
 Copy field coefficients from field file format modal storage into element modal
 storage.
...
void Element::Copy_field(double *, int *)
{
	ERR;
}
...
/****************************************************************/
/*    copysurfinflow: Copies the information about the inflow   */
/*	  boundary condition contained in a 2d .fld file (Field *f) */
/*	  into the 3D Element_List *U                               */ 
/****************************************************************/

static void elmt_copy_inflow_field(Element *E,Bndry *Ubc,double *data,
                                 int *size);

void copysurfinflow(Field *f, int pos, Bndry *Ubc, Element_List *U)
{
	...
	else
	{
		...
		if((Ubc->type=='V')||(Ubc->type=='v'))
				{
				eid=Ubc->elmt->id;

				elmt_copy_inflow_field(U->flist[eid], Ubc, data, size);
                                 ...
                                 int *size)
{
	...
	for (i=0;i<nfs;i++)
	{
		...
		if ((l=EL->edge[id].l) !=0)
		{
			dzero(l,E->edge[id].hj,1);
		    cl=min(l, *size);
			dcopy(cl,data,1,E->edge[id].hj,1);
		}
	data+=*size;
	++size;
	}
	...
	if(nfs == 3)
	{
		if((l = E->face[Ubc->face].l) != 0)
		{
		    dzero(l*(l+1)/2.0, *E->face[Ubc->face].hj,1);
			cl = min(l,*size);
			for(i = 0; i < cl; ++i)
			{
				dcopy(cl-i,data,1,E->face[Ubc->face].hj[i],1);
	...
	else
	{
		if((l = E->face[Ubc->face].l) != 0)
		{
			dzero(l*l, *E->face[Ubc->face].hj,1);
			cl = min(l,*size);
			for(i = 0; i < cl; ++i)
			{
				dcopy(cl,data,1,E->face[Ubc->face].hj[i],1);
