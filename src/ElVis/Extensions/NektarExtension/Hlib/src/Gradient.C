...
void Element::Grad (Element *, Element *, Element *, char )
{
	ERR;
}//grad


void Tri::GradT(Element *d_dx, Element *d_dy, Element *d_dz, char)
{
...
Function Purpose:
Wrapper around Grad_h. Output will be the derivatives of the field stored in
this element.
 
Argument 1: double *ux
Purpose: 
Array for storing the 'x' derivative of the field held in the physical storage 
('h' (2d) or 'h_3d' (3d)) in this element.
...
void Element::Grad_d (double *, double *, double *, char )
{
	ERR;
} //grad
...
Argument 1: double *H
Purpose: 
Field (assumed to be given in normal quadrature storage format) to be 
differentiated.
...
void Tri::Grad_h(double *H, double *ux, double *uy, double *uz, char trip)
{
	...
	*-------------------------------------------------------------------*/
	...
	if(!curvX)
	{
		...
		{
			case 'a':
				if(fabs(sx) > FPTOL || fabs(sy) > FPTOL)
				{
	...
	if(!curvX)
	{
		switch(trip)
		{
			case 'a':
				...
				for(i = 0; i < qb*qc; ++i)
					dvmul(qa,fac,1,ua+i*qa,1,ux+i*qa,1);

				if(fabs(G->tx.d) > FPTOL)
				{
			...
			case 'x':
				...
				if(fabs(G->tx.d) > FPTOL)
				{
					for(i = 0; i < qc; ++i)
						dgemm('N','N',qa,qb,qb,1.0/v->c[i],u+i*qa*qb,qa,*db,qb,
						      0.0,ub+i*qa*qb,qa);
					dgemm('N','N',qa*qb,qc,qc,G->tx.d,u,qa*qb,*dc,qc,1.0,ux,
						  qa*qb);
			...
			case 'y':
				...
				if(fabs(G->ty.d) > FPTOL)
				{
					for(i = 0; i < qc; ++i)
						dgemm('N','N',qa,qb,qb,1.0/v->c[i],u+i*qa*qb,qa,*db,qb,
						      0.0,ub+i*qa*qb,qa);
					dgemm('N','N',qa*qb,qc,qc,G->ty.d,u,qa*qb,*dc,qc,1.0,uy,
						  qa*qb);
			...
			case 'z':
				...
				dsadd(qa,G->rz.d,fac,1,fac,1);
				for(i = 0; i < qb*qc; ++i)
					dvmul(qa,fac,1,ua+i*qa,1,uz+i*qa,1);
				...
				if(fabs(G->tz.d) > FPTOL)
				{
					for(i = 0; i < qc; ++i)
						dgemm('N','N',qa,qb,qb,1.0/v->c[i],u+i*qa*qb,qa,*db,qb,
						      0.0,ub+i*qa*qb,qa);
					dgemm('N','N',qa*qb,qc,qc,G->tz.d,u,qa*qb,*dc,qc,1.0,uz,
						  qa*qb);
	...
	{ /* curved face version */
		...
		switch(trip)
		{
			case 'a':
				...
				for(i = 0; i < qb*qc; ++i)
					dvmul(qa,v[1].a,1,fac+i*qa,1,fac+i*qa,1);
...
void Element::Grad_h (double *,double *, double *, double *, char )
{
	ERR;
}
...
Argument 1: double *H
Purpose: 
Field (assumed to be given in normal quadrature storage format)
...
static Basis *Tri_add_dbase(int L, int qa, int qb, int qc)
{
	...
	/* differentiate with respect to local co-ordinate independent modes */
...
static void Tri_diff_base(Basis *b, int L, int qa, int qb, int qc)
{
	...
	free(tmp);
	free_dmatrix(da,0,0);
	free_dmatrix(dat,0,0);
	free_dmatrix(db,0,0);
	free_dmatrix(dbt,0,0);
}
...
static void Pyr_diff_base(Basis *b, int L, int qa, int, int qc)
{
	register int i;
...
void Element::WeakDiff(Mode *, double *, double *,double *, int )
{
	ERR;
}
...
void Tri::fill_gradbase(Mode *gb, Mode *m, Mode *mb, Mode *fac)
{
	...
	/* d/dr term */
	dcopy(qa,mb->a,1,gb[0].a,1);			// dh_r/dr
	dvdiv(qb,m->b,1,fac->b,1,gb[0].b,1);	// h_s*2/(1-eta_2)

	/* d/ds terms  */
	dcopy(qa,m ->a,1,gb[1].a,1);			//h_r
	dcopy(qb,mb->b,1,gb[1].b,1);			//dh_s/ds
}


void Quad::fill_gradbase(Mode *gb, Mode *m, Mode *mb, Mode *)
{

	/* d/dr terms */
	dcopy(qa,mb->a,1,gb[0].a,1);	// dh_r/dr
	dcopy(qb,m->b ,1,gb[0].b,1);	// h_s
...
void Hex::fill_gradbase(Mode *, Mode *, Mode *, Mode *)
{
...
void Element::fill_gradbase(Mode *, Mode *, Mode *, Mode *)
{
	ERR;
}
