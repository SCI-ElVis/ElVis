...
static int X2A(Element *E, Coord *X, Coord *A)
{
	...
	A->x[0] = (fabs(1.0-s) > FPTOL) ? 2*(1.0+r)/(1.0-s) - 1.0: 1.0;
	A->y[0] = s;
...
int GetRS (Element *U, Coord *X, Coord *A)
{
	...
	free (hr);
	free (hs);
	free_dmatrix (gridx,0,0);
	free_dmatrix (gridy,0,0);
	...
	for(k = EID; k < E->nel; ++k)
	{
		...
		for(cnt = 0, j = 0; j < F->Nedges; ++j)
		{
			if(((X->x[0] - F->vert[j].x)*Nmals[k].n[j][0] +
			        (X->y[0] - F->vert[j].y)*Nmals[k].n[j][1]) >= 0)
				++cnt;
		}
		if(cnt == F->Nedges)
		{
...
double eval_field_at_pt(int qa, int qb, int qc, double *field, double *hr,
						double *hs, double *ht)
{
	int i,j;
	double d=0.;
	        ...
	        || E->identify() != NekTypeStore )
	{
		...
		switch(E->identify())
		{
			case Nek_Tri:
				...
				if(storage == 'm')
				{ /* use modal storage to order output */
					cnt = 0;
					order[0] = cnt++;
					order[qa-1] = cnt++;
			...
			case Nek_Quad:
				...
				if(storage == 'm')
				{
					fprintf(stderr,"Interp_symm not set up for modal storage "
							"in tets \n");
					exit(-1);
				}
