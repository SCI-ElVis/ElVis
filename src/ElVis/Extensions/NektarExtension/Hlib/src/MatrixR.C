...
void MemRecur(Bsystem *B, int level, char trip)
{
	...
	for(i = 0; i < rdata->npatch; ++i)
	{
		...
		{
			rdata->binvc[i] = dvector(0,rdata->patchlen_a[i]*
										rdata->patchlen_c[i]-1);
			dzero(rdata->patchlen_a[i]*rdata->patchlen_c[i],rdata->binvc[i],1);

			rslv->A.a[i] = dvector(0,rdata->patchlen_a[i]*
									 rdata->patchlen_a[i]-1);
			dzero(rdata->patchlen_a[i]*rdata->patchlen_a[i],rslv->A.a[i],1);
		}
		rdata->invc[i]  = dvector(0,rdata->patchlen_c[i]*
									rdata->patchlen_c[i]-1);
...
void PackMatrixV(double *a, int n, double *b, int bwidth, char trip)
{
	...
	{ /* banded symmetric lower triangular form */
		...
		if(trip == 'l')
		{
			...
			error_msg(banded upper form not set
				          up in PackMatrixV);
	}
	else
	{
...
void Rinvert_a(Bsystem *B, char trip)
{
		...
		if (B->singular)
		{
			...
			for(i = 0; i < asize; i++)
				if (i != gid)
					for(j = 0; j < asize; j++)
						if (j != gid)
							AddContSMat(a[i*asize + j],B->rslv->A.Spa,
										map[n][i],map[n][j]);
			AddContSMat(1.0,B->rslv->A.Spa,gid,gid);
		}
		else
		{
			for(i = 0; i < asize; i++)
	...
	if (B->rslv->A.Spa.status == OPEN)
		CloseSMat(B->rslv->A.Spa);
	...
	if(2*bwidth < nsolve)	// store as packed banded system
	{
		...
		for(n = 0; n < npatch; ++n)
		{
			...
			for(i = 0; i < asize; ++i)
				for(j  = i; j < asize; ++j)
					if(map[n][j] >= map[n][i])
						inva[map[n][i]*bwidth + (map[n][j]-map[n][i])] +=
								 a[i*asize+ j];
					else
						inva[map[n][j]*bwidth + (map[n][i]-map[n][j])] +=
								 a[i*asize+ j];
		}
	...
	else					// store as symmetric system
	{
		...
		for(n = 0; n < npatch; ++n)
		{
			...
			for(i = 0; i < asize; ++i)
				for(j  = i; j < asize; ++j)
					if(map[n][j] <= map[n][i])
						inva[(map[n][j]*(map[n][j]+1))/2 +
							 (nsolve-map[n][j])*map[n][j]
						     + (map[n][i]-map[n][j])] += a[i*asize+j];
					else
						inva[(map[n][i]*(map[n][i]+1))/2 +
							 (nsolve-map[n][i])*map[n][i]
	...
	void **asymb = &B->rslv->A.Spa.symbolic;
	void **anumer = &B->rslv->A.Spa.numeric;
...
static void SetupRecur_Precon(Bsystem *B)
{
	...
	if (!B->Pmat)
	{
		B->Pmat = (MatPre *)malloc(sizeof(MatPre));
		if (B->Precon == Pre_SSOR)
			B->Pmat->info.ssor.tmp = NULL;
	}
	
	M = B->Pmat;
	
	/* at present just store diagonal of whole matrix in M->invert */
	switch(B->Precon)
	{
				...
				for(i = 0; i < npatch; ++i)
				{
					...
					for(j =0,pos = 0; j < Blk.ngv[i]; ++j, pos += al--)
					{
						...
						for(k = j+1; k < Blk.ngv[i]; ++k)
						{
							...
							if(gid < gid1)
								...
								M->ivert[(gid*(gid+1))/2 + gid*(nvs-gid) +
										 gid1-gid] += a[i][pos+k-j];
							
							else if(gid == gid1) /* special case of periodic
												    element */
								...
								M->ivert[(gid1*(gid1+1))/2 + gid1*(nvs-gid1) +
										 gid-gid1] += a[i][pos+k-j];
						}
					}
					...
					for(j = 0; j < Blk.nle[i]; ++j)
					{
						for(k = 0,n=0; k < Blk.edglen[i][j];pos += al--,
							...
							dvadd(Blk.edglen[i][j]-k,a[i]+pos,1,
		...
		case Pre_None: /* no preconditioner */
			break;
		case Pre_LEnergy:
			fprintf(stderr,"\nLow Energy Block Preconditioner not set up for "
					"recursive static condensation. Exiting to system...\n");
			exit(-1);
			break;
		case Pre_SSOR:
			if (!M->info.ssor.tmp)
			{
				Recur *rdata = B->rslv->rdata + B->rslv->nrecur - 1;
				M->info.ssor.tmp = dvector(0,rdata->cstart-1);
			}
			break;
		default:
...
#ifdef 1
{
	...
	else
	{
		...
		for(i = 0; i < r[nrecur-1].npatch; ++i)
			mem_iter += r[nrecur-1].patchlen_a[i]*
						(r[nrecur-1].patchlen_a[i]+1)/2;
		memory += mem_iter;
