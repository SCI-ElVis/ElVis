...
#define jacobz(n,z,alpha,beta) JacZeros(n,z,alpha,beta)
	...
	*/
	...
	/**
	  \brief  Gauss-Lobatto-Jacobi zeros and weights with end point at 
	  \a z=-1,\a 1
	 
	 
	  \li Generate \a np Gauss-Lobatto-Jacobi points, \a z, and weights, 
	  \a w, associated with polynomial \f$ (1-z)(1+z)
	   P^{\alpha+1,\beta+1}_{np-2}(z) \f$
		...
		else
		{
			...
			for (i = 0; i < np; i++)
				for (j = 0; j < np; j++)
				{
					...
					{
						if      (i == 0)
							D[i*np+j] = (alpha - (np-1)*(np + alpha + beta))/
										(two*(beta+ two));
						else if (i == np-1)
	...
	*/
	...
	/**
	    \brief Compute the value of the 
		\a i th Lagrangian interpolant through the
	...
	*/
	...
	double hgrjm (int i, double z, double *zgrj, int np, double alpha,
				  double beta)
	{
	...
	*/
	...
	double hgrjp (int i, double z, double *zgrj, int np, double alpha,
				  double beta)
	{
...
*/
	...
	double hglj (int i, double z, double *zglj, int np, double alpha,
				 double beta)
	{
		double one = 1., two = 2.;
		double zi, dz, p, pd, h;
...
*/
	...
	/**
	    ...
	    NOT USED IN THIS VERSION! In case you modify the code and include a
		call to this function, please remove this comment.
