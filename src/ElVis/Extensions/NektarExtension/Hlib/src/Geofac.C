			...
			case 0:
				...
				{ /* straight sided case */
					for(j = 0; j < qedg; ++j)
					{
						...
						e->norm->y[j] = ny;
						if(invjac)
							e->jac[j] = (0.5/geom->jac.d)*
							            sqrt(((v[1].x-v[0].x)*(v[1].x-v[0].x)+
...
void Prism::set_edge_geofac(int)
{
	fprintf(stderr,"set_edge_geofac with innjac not set up\n");
	exit(1);
}
...
void Hex::set_edge_geofac()
{
	...
	for(i=0;i<Nfaces;++i)
	{
		...
		if(curvX)
		{
			...
			Interp2d(*ima, *imb,  Bc->nx.p, qa, qb, f->n->x, f->qface,
					 f->qface);
			Interp2d(*ima, *imb,  Bc->ny.p, qa, qb, f->n->y, f->qface,
					 f->qface);
			Interp2d(*ima, *imb,  Bc->nz.p, qa, qb, f->n->z, f->qface,
					 f->qface);
...
void Element::set_edge_geofac()
{
	ERR;
}
void Element::set_edge_geofac(int)
{
	ERR;
}
...
void Tri::set_geofac()
{
	...
	{           /* check link list */
		...
		for(Tri_Ginf = Tri_Gbase; Tri_Ginf; Tri_Ginf = Tri_Ginf->next)
		{
			for(i = 1; i < Nverts; ++i)
			{
				if((fabs((v[i].x - v[0].x) - Tri_Ginf->dx[i-1]) > FPTOL)||
				   (fabs((v[i].y - v[0].y) - Tri_Ginf->dy[i-1]) > FPTOL))
					break;
			}
...
void Tet::set_geofac()
{
	...
	{           /* check link list */
		...
		for(Tet_Ginf = Tet_Gbase; Tet_Ginf; Tet_Ginf = Tet_Ginf->next)
		{
			for(i = 1; i < Nverts; ++i)
			{
				if((fabs((v[i].x - v[0].x) - Tet_Ginf->dx[i-1]) > FPTOL) ||
				   (fabs((v[i].y - v[0].y) - Tet_Ginf->dy[i-1]) > FPTOL) ||
				   (fabs((v[i].z - v[0].z) - Tet_Ginf->dz[i-1]) > FPTOL))
					break;
			}
...
void Pyr::set_geofac()
{
	...
	{           /* check link list */
		...
		for(Pyr_Ginf = Pyr_Gbase; Pyr_Ginf; Pyr_Ginf = Pyr_Ginf->next)
		{
			...
			{
				if((fabs((v[i].x - v[0].x) - Pyr_Ginf->dx[i-1]) > FPTOL) ||
				   (fabs((v[i].y - v[0].y) - Pyr_Ginf->dy[i-1]) > FPTOL) ||
				   (fabs((v[i].z - v[0].z) - Pyr_Ginf->dz[i-1]) > FPTOL))
					break;
			}
...
void Prism::set_geofac()
{
	...
	{           /* check link list */
		...
		for(Prism_Ginf = Prism_Gbase; Prism_Ginf; Prism_Ginf = Prism_Ginf->next)
		{
			...
			{
				if((fabs((v[i].x - v[0].x) - Prism_Ginf->dx[i-1]) > FPTOL) ||
				   (fabs((v[i].y - v[0].y) - Prism_Ginf->dy[i-1]) > FPTOL) ||
				   (fabs((v[i].z - v[0].z) - Prism_Ginf->dz[i-1]) > FPTOL))
					break;
			}
...
void Hex::set_geofac()
{
	...
	{           /* check link list */
		...
		for(Hex_Ginf = Hex_Gbase; Hex_Ginf; Hex_Ginf = Hex_Ginf->next)
		{
			...
			{
				if((fabs((v[i].x - v[0].x) - Hex_Ginf->dx[i-1]) > FPTOL) ||
				   (fabs((v[i].y - v[0].y) - Hex_Ginf->dy[i-1]) > FPTOL) ||
				   (fabs((v[i].z - v[0].z) - Hex_Ginf->dz[i-1]) > FPTOL))
					break;
			}
...
static void Tri_DerX(Tri *E, double **DR)
{
	...
	for(i = 0; i < qb; ++i)
		dscal(qa,1/v->b[i],DR[0]+i*qa,1);

	/* calculate dx/ds */
	for(i = 0; i < qb; ++i)
		dvmul(qa,v[1].a,1,DR[0]+i*qa,1,DR[1]+i*qa,1);
	dgemm('N','N',qa,qb,qb,1.0,X.x,qa,*db,qb,1.0,DR[1],qa);

	/* calculate dy/dr */
	dgemm('T','N',qa,qb,qa,1.0,*da,qa,X.y,qa,0.0,DR[2],qa);
	for(i = 0; i < qb; ++i)
		dscal(qa,1/v->b[i],DR[2]+i*qa,1);

	/* calculate dy/ds */
	for(i = 0; i < qb; ++i)
		dvmul(qa,v[1].a,1,DR[2]+i*qa,1,DR[3]+i*qa,1);
...
static void Tet_DerX(Element *E, double **DR)
{
	...
	/* calc dx/dr*(1+a)/2 */
	for(i = 0; i < qb*qc; ++i)
		dvmul(qa,v[1].a,1,DR[0]+i*qa,1,fac+i*qa,1);
	...
	/* calc dy/dr*(1+a)/2 */
	for(i = 0; i < qb*qc; ++i)
		dvmul(qa,v[1].a,1,DR[3]+i*qa,1,fac+i*qa,1);
	...
	/* calc dz/dr*(1+a)/2 */
	for(i = 0; i < qb*qc; ++i)
		dvmul(qa,v[1].a,1,DR[6]+i*qa,1,fac+i*qa,1);
...
static int Tri_check_L(Tri *E, Geom *G)
{
	...
	for(i = 0; i < E->Nedges; ++i)
		if(E->edge[i].l != U->edge[i].l)
			trip = 0;

	if(E->face->l != U->face->l)
		trip = 0;
...
static int Tet_check_L(Element *E, Geom *G)
{
	...
	for(i = 0; i < NTet_faces; ++i)
		if(E->face[i].l != U->face[i].l)
			trip = 0;

	if(E->interior_l != U->interior_l)
		trip = 0;
