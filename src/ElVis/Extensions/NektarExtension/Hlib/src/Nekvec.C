...
double ***dtarray(int nwl, int nwh, int nrl, int nrh, int ncl, int nch)
{
	...
	***m3;
	...
	if(!m1)
		error_handler("allocation failure 1 in dtarray()");

	m2 = (double **) malloc((unsigned)(nwh-nwl+1)*skipr*sizeof(double*));
	if(!m2)
		error_handler("allocation failure 2 in dtarray()");

	m3 = (double ***) malloc((unsigned)(nwh-nwl+1)*sizeof(double*));
	if(!m3)
		error_handler("allocation failure 3 in dtarray()");
...
Mode *mvector(int nl, int nu)
{
	...
	if(!m)
		fprintf(stderr,"allocation failure in mvector()");
	return m-nl;
}
...
void Tri_mvmul2d(int qa, int qb, int, Mode *x, Mode *y, Mode *z)
{
