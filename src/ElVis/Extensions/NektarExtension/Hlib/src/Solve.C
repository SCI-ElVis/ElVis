           ...
           SolveType Stype)
{
	...
	/*-----------------------------*/
	setupRHS(U,Uf,RHS,U0,Ubc,Ubsys,Stype);

	/*-----------------------------*/
	/* Solve the boundary system   */
	/*-----------------------------*/
	solve_boundary(U,Uf,RHS,U0,Ubsys);

	/*-----------------------------*/
	/* solve the Interior system   */
	/*-----------------------------*/
	solve_interior(U,Ubsys);
...
static void Set_tolerance(Element_List *Ef_L)
{
                      ...
                      Bndry *Ubc, Bsystem *B, SolveType Stype)
{
	...
	if(B->nsolve)
	{               /* subtract off interior coupling          */
	...
	if(B->smeth == iterative&&!B->rslv)
	{
		...
		if(B->Precon == Pre_LEnergy)
		{
			...
			Set_tolerance(Uf); /* set solver tolerances */
		...
		else
		{
			...
			// Lift solution (subtract Dirichlet bcs from rhs)
			for(k = 0; k < nel; ++k)
			{
	...
	else
	{
		...
		else
		{
			...
			for(k = 0; k < nel; ++k)
			{
				...
				dcopy(N,Uf->flist[k]->vert->hj+nbl,1,
				      U->flist[k]->vert->hj+nbl,1);
			}
			GathrBndry(U,rhs,B);
			dzero(B->nglobal-B->nsolve, rhs + B->nsolve, 1);
		}
                    ...
                    double *rhs, double *u0, Bsystem *B)
{
	...
	{
		if(B->rslv)
		{ /* recursive Static condensation solver */
			...
			{
				if(B->singular)
					rhs[B->singular-1] = 0.0;
				
				if(aslv)
				{
#ifdef UMFPACKSLV		// UMFPACK implementation
					double *null = (double*)NULL;
					void *numer = R->A.Spa.numeric;
					int *rpt = R->A.Spa.rpt;
					int *ija = R->A.Spa.ija;
					double *sa = R->A.Spa.sa;
					double *tmprhs = R->A.Spa.tmprhs;
					
					dcopy(aslv, rhs, 1, tmprhs, 1);
					umfpack_di_solve(UMFPACK_At, rpt, ija, sa, rhs, tmprhs,
									 numer, null, null);
		...
		else
		{
			...
			else
			{
				...
				double *null = (double*)NULL;
				void *numer = B->Gmat->Spa.numeric;
				int *rpt = B->Gmat->Spa.rpt;
				int *ija = B->Gmat->Spa.ija;
				double *sa = B->Gmat->Spa.sa;
				double *tmprhs = B->Gmat->Spa.tmprhs;
					
				dcopy(nsolve, rhs, 1, tmprhs, 1);
				umfpack_di_solve(UMFPACK_At, rpt, ija, sa, rhs, tmprhs, numer,
								 null, null);
...
void SetBCs(Element_List *EL, Bndry *Ubc, Bsystem *Ubsys)
{
	...
	for(;Ubc;Ubc= Ubc->next)
	{
		switch(Ubc->type)
		{
			case 'V':		// Velocity Dirichlet conditions
			case 'M':
			case 'o':		// Pressure at the outflow is set to 0
				scal = 1.0;
				eid  = Ubc->elmt->id;
				face = Ubc->face;
...
static void Precon   (Element_List *, Bsystem *,  double *,  double *);
                      ...
                      Bsystem *B, double *p)
{
	...
	DO_PARALLEL
	{
	    ...
	    ROOTONLY
	    {
		    ...
		    if (info)
		        error_msg("Lanczos Method, dsterf  -- info is not zero!!");
	    ...
	    if (iter > MAX_ITERATIONS)
	    {
		    ...
		    ROOTONLY printf("\tField %c: %3d iterations, error = %#14.6g %lg "
	...
	else	// Serial run
	{
		...
		while ((eps > tolcg ||iter < MIN_LANCZOS_ITER) &&
				iter++ < MAX_ITERATIONS)
#else
		while (eps > tolcg && iter++ < MAX_ITERATIONS )
#endif
		...
		{
			Precon(U,B,r,z);					// Solve Mz(i-1) = r(i-1)
			
			rtz  = ddot (nsolve, r, 1, z, 1);	// rho(i-1)=r(i-1)*z(i-1)

			if (iter > 1)
			{         /* Update search direction */
				beta = rtz / rtz_old;			// beta(i-1) = rho(i-1)/rho(i-2)
#ifdef Lanczos
			...
			A_fast(U,Uf,B,p,w);					// w(i) = A*p(i)
			
#endif
			
			if(B->singular && B->Precon == Pre_LEnergy)
				w[B->singular-1] = 0.0;
											// alpha(i) = rho(i-1)/(p(i)*w(i))
			...
			if (iter <= MAX_lanc_iter)
				alpha_l[iter] = alpha;
#endif
												// u(i) = u(i-1) + alpha(i)*p(i)
			daxpy(nsolve, alpha, p, 1, u, 1);	/* Update solution...   */
			daxpy(nsolve,-alpha, w, 1, r, 1);	/* ...and residual      */
												// r(i) = r(i-1) - alpha(i)*w(i)
			rtz_old = rtz;
...
/* Start to set up the Lanczos matrix, T, which is a tria-diagonal
   ...
   eigenvalues of a tria-diagonal matrix */
		...
		{
			iter = MAX_lanc_iter;
			printf("Lanczos: Conj.Gradient not fully converged, stop at 990 "
					"iter.\n");
		}
	...
	if(B->Precon == Pre_LEnergy)
	{ /* tranform back solution to original basis*/
		double **Rv, **Rvi, **Re;
		int Ne, Nf;
		Element *E;
...
static void Precon(Element_List *U, Bsystem *B, double *r, double *z)
{
	switch(B->Precon)
	{
		case Pre_Diag:
		...
		case Pre_LEnergy: /* Low Energy */
		{
			...
			if(nvs&&doblock)
			{
				...
				DO_PARALLEL
				{
				    ...
				    for(i = 0; i < nv_lpsolve; ++i)
					    ztmp[B->pll->solvemap[i]] -= ddot(csize,
								     *M->info.lenergy.ivert_B+i*csize,1,fint,1);
					...
					if(info)
					    fprintf(stderr,"Error solving  matrix in InvtPrecon\n");
				    ...
				    if(csize)
				    {
					    ...
					    dpptrs('L',csize,1,*M->info.lenergy.ivert_C,fint,csize,
								info);
					    if(info)
						    fprintf(stderr,"Error solving matrix in"
										            " InvtPrecon\n");

					    // substract inv(c) B^t contribution
					    for(i = 0; i < csize; ++i)
						    z[nv_lpsolve+i] = fint[i] - ddot(nv_lpsolve,
										M->info.lenergy.ivert_B[0]+i,csize,z,1);
				    }
					...
					for(E = U->fhead; E; E = E->next)
				    {
					    ...
					    for(j = 0; j < E->Nverts; ++j)
						    if(B->bmap[E->id][j] < B->nv_solve)
							    ztmp[B->pll->solvemap[B->bmap[E->id][j]]] +=
														        E->vert->hj[j];
				    }
				    ...
				    if(2*bw < nvs)
					    dpbtrs('L',nvs,bw-1,1,*M->info.lenergy.ivert,bw,ztmp,
								nvs,info);
				    else
					    dpptrs('L',nvs,1,*M->info.lenergy.ivert,ztmp,nvs,info);
				...
				else 		// Serial run
				{
					...
					if(2*bw < nvs)
						dpbtrs('L',nvs,bw-1,1,*M->info.lenergy.ivert,bw,z,
								nvs,info);
					else
				...
				if(info)
					error_msg(error inverting coarse solve in precon);
			}
			...
			if(LGmax > 2)
			{
				...
				{
					if(B->pll->singular)
						tmp[B->pll->singular-1] = 0.0;
					else
						if(B->singular)
							tmp[B->singular-1] = 0.0;
				}
				...
				for(i = 0, s=tmp+nvs; i < nes; ++i)
				{
					...
					if(l)
						dpptrs('L',l,1,M->info.lenergy.iedge[i],s,l,info);
					s  +=l;
				}
				for(i = 0; i < nfs; ++i)
				{
					...
					if(l)
						dpptrs('L',l,1,M->info.lenergy.iface[i],s,l,info);
					s  +=l;
				}
		...
		case Pre_Overlap:
			//dvmul(B->Pmat->info.overlap.ndiag,B->Pmat->info.overlap.idiag,1,
			//r,1,z,1);
		...
		{
#ifdef SPARSEMAT
			// For an A matrix which is symmetric and could be represented by
			// L+D+L', where L is the lower triangular part (excluding the
			// diagonal), L' is the transpose of L, and D the diagonal, the pre-
			// conditioner matrix "M" for the SSOR scheme is [D+L]D^(-1)[D+L]'
			// This routine must solve Mz=r. This is done in two steps: first
			// the solution of [D+L]t=r is computed and next D^(-1)[D+L]z=t
			int i, j;
			int nlines = B->Gmat->Spa.nlines;
			double *sa = B->Gmat->Spa.sa;
			int *ija = B->Gmat->Spa.ija;
			...
			{
				// Solving [D+L] * tmp = r (back-substitution)
				for (i = 0; i < nlines; i++)
				{
				...
				// Solving D^(-1)[D+L]' * z = t (back-substitution using
				// [D+L]' * z = D * t)
...
#else
			fprintf(stderr,"SSOR preconditioner not implemented for elemental"
					"full matrix storage. Change the preconditioner or define"
					"SPARSEMAT when compiling Hlib. "
