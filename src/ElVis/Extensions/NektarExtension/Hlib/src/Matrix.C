...
* Function GenMat															   *
*																			   *
* Description: Generates the matricial system and sets up the solver		   *
*			   accordingly. The linear system can be solved using the following*
*			   procedures:													   *
*																			   *
*		1- Iterative solver (with or without preconditioners)				   *
*		2- Direct solver using static condensation							   *
*			2.1- Recursive static condensation								   *
*			2.2- Non-recursive - using a reverse Cuthill-McKee algorithm	   *
*																			   *
*******************************************************************************/
            ...
            SolveType Stype)
{
	...
	Bndry   *Bc;
	Element *E;
	static int SVV = (fabs(dparam("SVVfactor")) > FPTOL) ? 1:0;
	...
	if(regen)
	{ // only generate matrix if Ubsys not previously defined
#ifdef UMFPACKSLV
		...
		switch(Stype)
		{
			...
			case Helm:
				for(E = U->fhead; E;E=E->next)
				{
					...
					{
						if(E->identify()==Nek_Tri && !E->curvX &&
						        !lambda[E->id].p && !SVV)
							Tri_HelmMat(E, helm, lambda[E->id].d);
						else if (E->identify()==Nek_Quad && !lambda[E->id].p &&
								 !SVV)
						{
							// parallelogram
							if ((!E->curvX || E->curve->type == T_Straight) &&
								 (fabs((E->vert[1].x - E->vert[0].x) -
							 	  	(E->vert[2].x - E->vert[3].x)) < FPTOL) &&
								 (fabs((E->vert[1].y - E->vert[0].y) -
							 	  	(E->vert[2].y - E->vert[3].y)) < FPTOL))
								Prllgram_HelmMat(E, helm, lambda[E->id].d);
							else	// non-parallelogram
						...
						for(Bc=Ubc;Bc;Bc=Bc->next)
							if(Bc->type == 'R' && Bc->elmt->id == E->id){
								...
								E->Add_robin_matrix(helm, Bc,Ubsys->vel);}
				...
				DO_PARALLEL
				{
				    if((!lambda || (!lambda->p && (fabs(lambda->d) < FPTOL)))
					...
					{
						Ubsys->singular =
						    Ubsys->bmap[iparam("IESING")][iparam("IVSING")]+1;
				...
				// check to see if singular point is in inner boundary solve 
						if(Ubsys->rslv)
						{
							int i,k;
							int cstart =
							   Ubsys->rslv->rdata[Ubsys->rslv->nrecur-1].cstart;
...
finished:
	...
	{ /* recursive solver setup */
		...
		for(i = 0; i < Ubsys->rslv->nrecur-1; ++i)
		{
			...
			for(j = 0; j < rdata->npatch; ++j)
				Project_Recur(j,rdata->patchlen_a[j],
				              a[j],rdata->map[j],i+1,Ubsys);
		...
		{
#ifdef UMFPACKSLV
			// Initialize SMatrix structure
			rdata = Ubsys->rslv->rdata + Ubsys->rslv->nrecur-1;
			InitSMat(Ubsys->rslv->A.Spa, rdata->cstart);
			Ubsys->rslv->A.Spa.tmprhs = dvector(0, rdata->cstart-1);
#endif
			Rinvert_a(Ubsys,'p');
#ifdef UMFPACKSLV
			umfpack_di_free_symbolic(&Ubsys->rslv->A.Spa.symbolic);
#endif
		}
		else
		{
			...
			// Put matrix into sparse structure
			RBuildSMat(Ubsys->rslv);
#endif
		}
	}
	else // standard statically condensed set up
	{
		if(Ubsys->smeth == direct)
		{
			...
			invert_a  (Ubsys);
#ifdef UMFPACKSLV
			umfpack_di_free_symbolic(&Ubsys->Gmat->Spa.symbolic);
#endif			
		}
		else
		{
			...
			if(option("ReCalcPrecon"))
			{
				...
				if(Ubsys->Precon == Pre_LEnergy)	// transform basis for low
					Trans2LeBasis(U->fhead,Ubsys);	// energy preconditioner
					
#ifdef SPARSEMAT
			// Put matrix into sparse structure - has to be done after
			// Trans2LeBasis for Pre_LEnergy
			if (Ubsys->Precon == Pre_LEnergy)
				BuildSMat(Ubsys, U);
#endif
		}
...
static void invert_a(Bsystem *Ubsys)
{
...
#ifdef UMFPACKSLV	// UMFPACK implementation
	
	if (nsolve)
	{
		double *null = (double*)NULL;
		void **asymb = &Ubsys->Gmat->Spa.symbolic;
		void **anumer = &Ubsys->Gmat->Spa.numeric;
		int *rpt = Ubsys->Gmat->Spa.rpt;
		int *ija = Ubsys->Gmat->Spa.ija;
		double *sa = Ubsys->Gmat->Spa.sa;
		...
		if (Ubsys->singular)
		{
			...
			for (i = 0; i < nsolve; i++)
				for (j = rpt[i]; j < rpt[i+1] && ija[j] <= gid; j++)
					if (ija[j] == gid) 
						sa[j] == 0.0;
			for (j = rpt[gid]; j < rpt[gid+1]; j++)
			{
				if (ija[j] == gid)
					sa[j] = 1.0;
				else
		...
		if (*asymb == (void *)NULL)
			info=umfpack_di_symbolic(nsolve,nsolve,rpt,ija,sa,asymb,null,null);
		else
			info = UMFPACK_OK;
		
		if (info == UMFPACK_OK)
			info = umfpack_di_numeric(rpt,ija,sa,*asymb,anumer,null,null);
	}
	if (info != UMFPACK_OK)
		fprintf(stderr,"\nProblem in the UMFPACK factorization routines!\n");
	
#else				// LAPACK implementation
	...
	if(info)
		fprintf(stderr,"invertA: info not zero\n");
#endif
}
...
#ifdef SPARSEMAT			
			InitSMat(Ubsys->Gmat->Spa, Ubsys->nsolve);
#endif
		}
		else
		{
...
#ifdef UMFPACKSLV		// UMFPACK implementation
				InitSMat(Ubsys->Gmat->Spa, Ubsys->nsolve);
				Ubsys->Gmat->Spa.tmprhs = dvector(0, Ubsys->nsolve-1);
#else					// LAPACK implementation
				Ubsys->Gmat->bwidth_a = bandwidth(U,Ubsys);
				if(2*Ubsys->Gmat->bwidth_a < nsolve)	// banded solver
				{
					Ubsys->Gmat->inva =
					    dmatrix(0,nsolve-1,0,Ubsys->Gmat->bwidth_a-1);
...
void MemPrecon(Element *U, Bsystem *B)
{
	...
	switch(B->Precon)
	{
		...
		case Pre_Block:
			...
			if(U->dim() == 3)
			{
				M->info.block.iface = (double **) calloc((nfs)?nfs:1,
				                      sizeof(double *));
		...
		case Pre_LEnergy: /* Low Energy vertex and edge preconditioner */
		{
			...
			if(option("variable")) /* need mapping to be generalised */
				error_msg(Low Energy preconditioner not set
					          up for variable order)
						;
			...
			M->info.lenergy.iedge = (double **) calloc((nes)?nes:1,
			                        sizeof(double *));
			M->info.lenergy.Ledge = ivector(0,(nes)?nes-1:0);
			izero(nes, M->info.lenergy.Ledge, 1);

			/* declare memory */
			if((ll = M->info.lenergy.nvert) != 0)
			{
				if(doblock)
					if(2*M->info.lenergy.bw < ll)
					{
						M->info.lenergy.ivert =
						    dmatrix(0,ll-1,0,M->info.lenergy.bw-1);
						dzero(ll*(M->info.lenergy.bw),
						      M->info.lenergy.ivert[0],1);
					}
					else
					{
						ll = ll*(ll+1)/2;
			...
			M->info.lenergy.iface = (double **) calloc((nfs)?nfs:1,
														sizeof(double *));
		...
		case Pre_SSOR:
			DO_PARALLEL
			{
				fprintf(stderr,"SSOR preconditioner not implemented for "
						"parallel solver. Try another preconditioner...\n");
				exit(-1);
			}
			else	// Serial version
			{
				M->info.ssor.tmp = dvector(0,B->nsolve-1);
			}
			break;
		default:
			break;
...
static void ZeroPrecon(Bsystem *B, int dim)
{
	...
	{
		case Pre_Diag:
			dzero(M->info.diag.ndiag,M->info.diag.idiag,1);
			break;
		case Pre_Block:
			izero(nes, M->info.block.Ledge, 1);
			if(dim == 3)
		...
		case Pre_LEnergy: /* Low Energy vertex and edge preconditioner */
		{
			...
			if((ll = M->info.lenergy.nvert) != 0)
			{
				if(doblock)
					if(2*M->info.lenergy.bw < ll)
					{
						dzero(ll*(M->info.lenergy.bw),M->info.lenergy.ivert[0],
							  1);
					}
					else
					{
...
static void FillPrecon(Element *U, Bsystem *B)
{
	...
	switch(B->Precon)
	{
...
#ifdef SPARSEMAT
			for (i = 0; i < B->Gmat->Spa.nlines; i++)
				M->info.diag.idiag[i]= B->Gmat->Spa.sa[B->Gmat->Spa.rpt[i+1]-1];
#else			
			for(E=U;E;E=E->next)
			{
		...
		{	
			...
			nvs_ple = M->info.lenergy.nvert; //reset to global (parallel) number
				...
				DO_PARALLEL
				{
				    ...
				    for(E =U; E; E = E->next)
				    {
					    ...
					    for(i = 0,pos = 0; i < E->Nverts; ++i,pos+=nbl--)
					    {
						    gid = E->vert[i].gid;
						    if(gid < B->nv_solve)
						    { // project matrix to local matrices
							    if(gid < nv_lpsolve)
							    { // boundary-boundary
								    id = B->pll->solvemap[gid];
								    M->info.lenergy.ivert[0][(id*(id+1))/2+
												id*(nv_gpsolve-id)] += A[pos];
							    }
							    else
							    { // interior-interior
									M->info.lenergy.ivert_C[0][
										((gid-nv_lpsolve)*(gid-nv_lpsolve+1))/2+
										(gid-nv_lpsolve)*
							    ...
							    for(j = i+1; j < E->Nverts; ++j)
							    {
									...
									M->info.lenergy.ivert[0][(id*(id+1))/2+id*
									    (nv_gpsolve-id) + id1-id] += A[pos+j-i];
											
										    else if(id == id1)/* special case
														   of periodic element*/
												
								    M->info.lenergy.ivert[0][(id*(id+1))/2+
			                             id*(nv_gpsolve-id1)]  += 2*A[pos+j-i];
											
										    else
												
									M->info.lenergy.ivert[0][(id1*(id1+1))/2+
		                             id1*(nv_gpsolve-id1)+id-id1] += A[pos+j-i];
										    
										}  // interior-interior
									    else if ((gid >= nv_lpsolve)&&
												 (gid1 >= nv_lpsolve))
									    {
										    if(gid < gid1)
								...
								M->info.lenergy.ivert_C[0][((gid-nv_lpsolve)*
									  ...
									  (csize - (gid-nv_lpsolve))+gid1-gid] +=
																	A[pos+j-i];
											
											else if(gid == gid1)/* special case 
														   of periodic element*/
								...
								M->info.lenergy.ivert_C[0][((gid1-nv_lpsolve)*
									(gid1-nv_lpsolve+1))/2+(gid1-nv_lpsolve)*
								 	(csize-(gid1-nv_lpsolve))+gid-gid1] +=
																	 A[pos+j-i];
					...
					for(E =U; E; E = E->next)
					{
						...
						for(i = 0,pos = 0; i < E->Nverts; ++i,pos+=nbl--)
						{
							...
							if(gid < B->nv_solve)
							{
								...
								if(2*bw < nvs_ple) /* banded */
									M->info.lenergy.ivert[gid][0] += A[pos];
								else
									M->info.lenergy.ivert[0][(gid*(gid+1))/2+
											gid*(nvs_ple-gid)] += A[pos];
								...
								for(j = i+1; j < E->Nverts; ++j)
								{
									...
									if(gid1 < B->nv_solve)
									{
										...
										{ /* banded */
											if(gid < gid1)
												M->info.lenergy.ivert[gid][gid1-
															gid] += A[pos+j-i];
											else if(gid == gid1)/* special 
													case of periodic element*/
												M->info.lenergy.ivert[gid][0] +=
															 2*A[pos+j-i];
											else
												M->info.lenergy.ivert[gid1][gid-
														gid1]  += A[pos+j-i];
										}
										else
										{ /* symmetric */
									...
									M->info.lenergy.ivert[0][(gid*(gid+1))/2 +
								    gid*(nvs_ple-gid) + gid1-gid] += A[pos+j-i];
												
											else if(gid == gid1)/* special case
														   of periodic element*/
									
									M->info.lenergy.ivert[0][(gid*(gid+1))/2+
											gid*(nvs_ple-gid1)] += 2*A[pos+j-i];
											
											else
												
									M->info.lenergy.ivert[0][(gid1*(gid1+1))/2+
								 	 gid1*(nvs_ple-gid1)+gid-gid1]+= A[pos+j-i];
			...
			for(E=U;E;E = E->next)
			{
				...
				for(i = 0; i < E->Nedges; ++i)
				{
					...
					if(gid < B->ne_solve)
					{
						if(E->edge[i].con)
						{
							for(j=0, s=M->info.lenergy.iedge[gid];j<l; 
									s+=l-j,++j,pos+=nbl--)
							...
							if(E->face[i].con)
							{
								...
								for(j=0,s=M->info.lenergy.iface[gid];j < l; ++j)
									for(k = 0; k < l-j; ++k, pos+=nbl--)
									{
										...
										s1 = A + pos + l-j-k;
										s += l-j-k;
										for(h = j+1,sign=-1; h < l;
											  s+=l-h, s1+=l-h, ++h, sign=-sign)
											daxpy(l-h,sign,s1,1,s,1);
									}
							}
							else
								for(j=0,s=M->info.lenergy.iface[gid];
										j<l;s+=l-j,++j,pos+=nbl--)
									dvadd(l-j,A+pos,1,s,1,s,1);
						}
						else
						...
						if(gid < B->nf_solve)
						{
							switch(E->face[i].con)
							{
								case 0:
									...
									for(j=0,s=M->info.lenergy.iface[gid];j<l;
											s+=l-j,++j,pos+=nbl--)
										dvadd(l-j,A+pos,1,s,1,s,1);
									break;
										
								case 1:
								case 5:// negate in 'a' direction 
										
									l = E->face[i].l;
									for(j = 0, s=M->info.lenergy.iface[gid];
											j < l; ++j)
										for(k = 0, sign1=1; k < l;
												++k, pos+=nbl--, sign1=-sign1)
										{
											for(h = 0,sign=1; h < l-k;
													 ++h,sign=-sign)
												s[h] += sign*A[pos + h];
											s1 = A + pos + l-k;
											s += l-k;
											for(h = j+1; h < l; ++h,s+=l,s1+=l)
												for(h1 = 0,sign=sign1; h1 < l;
													 ++h1,sign=-sign)
													s[h1] += sign*s1[h1];
										}
										break;
								...
								case 6:// negate in 'b' direction
									...
									for(j = 0, s=M->info.lenergy.iface[gid];
										...
										for(k = 0; k < l; ++k, pos+=nbl--)
										{
											...
											s1 = A + pos + l-k;
											s += l-k;
											for(h = j+1,sign1=-1; h < l;
												 ++h,sign1=-sign1,s+=l,s1+=l)
												for(h1 = 0; h1 < l; ++h1)
													s[h1] += sign1*s1[h1];
										}
									break;
								...
								case 3:
								case 7:// negate in both  directions
										
									l = E->face[i].l;
									for(j = 0, s=M->info.lenergy.iface[gid];
											j < l; ++j)
										for(k = 0,sign2=1; k < l;
												++k, pos+=nbl--,sign2=-sign2)
										{
											for(h = 0,sign=1; h < l-k;
													 ++h,sign=-sign)
												s[h] += sign*A[pos + h];
...
#ifdef LE_VERT_OLD
						    tmp = dvector(0,
									   max(bw*nvs_ple,nvs_ple*(nvs_ple+1)/2)-1);

						    if(2*bw < nvs_ple)
						    {    /* banded */
							    gdsum(*B->Pmat->info.lenergy.ivert,bw*nvs_ple,
										tmp);
						    }
						    else
						    {
							    gdsum(*B->Pmat->info.lenergy.ivert,
									  nvs_ple*(nvs_ple+1)/2,tmp);
						    }
						    free(tmp);
#endif
		...
		{	
			...
			if((nvert = B->Pmat->info.lenergy.nvert)&&doblock)
			{
				...
				DO_PARALLEL
				{
				    ...
				    double *inva  = B->Pmat->info.lenergy.ivert[0];
				    ...
				    if(csize)
				    {
					    ...
					    if(info)
						    fprintf(stderr,"error in inv(c) b^T \n");
				    ...
				    //FacMatrix gather together global matrix system
				    ...
				    if(B->singular)
				    { /* zero out all but diagonal contribution */
					    register int k;
...

				{
					...
					{    /* banded */
						...
						dpbtrf('L',nvert,bwidth-1,
						       B->Pmat->info.lenergy.ivert[0],bwidth,info);
					}
					else
					{
						if(B->singular)
						{ /* zero out all but diagonal contribution */
							...
							for(k = 0; k < gid;
							        ++k, s += B->Pmat->info.lenergy.nvert-k)
								s[0] = 0.0;
					...
					if(info)
						fprintf(stderr,"InvtPrecon: info not zero\n");
				}
			}
			...
			{
				if((l = B->Pmat->info.lenergy.Ledge[i]) != 0)
				{
					dpptrf('L',l,B->Pmat->info.lenergy.iedge[i],info);
					if(info)
						fprintf(stderr,"info not zero\n");
				}
			}

			for(i = 0; i < B->Pmat->info.lenergy.nface; ++i)
				if((l = B->Pmat->info.lenergy.Lface[i]) != 0)
				{
...
void PackMatrix(double **a, int n, double *b, int bwidth)
{
	...
	if(n>2*bwidth)
	{ /* banded symmetric lower triangular form */
		double *s;
...
/* factor positive symmetric and banded matrices */
void FacMatrix(double *a, int n, int bwidth)
{
	...
	if(info)
	{
		fprintf(stderr, "FacMatrix: n=%d bw=%d info=%d\n",
		        n,bwidth,info);
                error_msg(FacMatrix: info not zero);
	}
}
...
void Bsystem_mem_free(Bsystem *Ubsys)
{
	const int nsolve = Ubsys->nsolve, nfam = Ubsys->families;
	int i,j;
	...
	if (Ubsys->rslv)	// Recursive solver
	{
		...
		// Free boundary-boundary matrix for iterative solver. For direct 
		// solver, this is done at the end of Rinvert_a (in MatrixR.C)
		if (Ubsys->smeth == iterative)
		{
			rdata = rslv->rdata + (rslv->nrecur-1);
			for (i = 0; i < rdata->npatch; i++)
				if (rdata->patchlen_a[i])
					free(rslv->A.a[i]);
			free(rslv->A.a);
...
static void  Seg_Add_robin_matrix(Element *E, LocMat *helm,
								  Bndry *rbc, AdvVel *vel);
static void Quad_Add_robin_matrix(Element *E, LocMat *helm,
								  Bndry *rbc, AdvVel *vel);
static void  Tri_Add_robin_matrix(Element *E, LocMat *helm,
                                  Bndry *rbc, AdvVel *vel);

void  Quad::Add_robin_matrix(LocMat *helm,Bndry *rbc, AdvVel *vel)
{
	Seg_Add_robin_matrix(this,helm,rbc,vel);
}

void  Tri::Add_robin_matrix(LocMat *helm, Bndry *rbc, AdvVel *vel)
{
	Seg_Add_robin_matrix(this,helm, rbc, vel);
}

void  Hex::Add_robin_matrix(LocMat *helm, Bndry *rbc, AdvVel *vel)
{
	Quad_Add_robin_matrix(this,helm,rbc,vel);
}

void  Tet::Add_robin_matrix(LocMat *helm, Bndry *rbc, AdvVel *vel)
{
	Tri_Add_robin_matrix(this,helm, rbc,vel);
}

void  Prism::Add_robin_matrix(LocMat *helm, Bndry *rbc, AdvVel *vel)
{
	switch(rbc->face)
	{
		case 0:
...
void  Pyr::Add_robin_matrix(LocMat *helm,  Bndry *rbc, AdvVel *vel)
{
	switch(rbc->face)
	{
		case 0:
...
static void Seg_Add_robin_matrix(Element *E, LocMat *helm, Bndry *rbc, AdvVel *vel)
{
	int      edge = rbc->face;
	int      qa   = E->qa;
	double   *tmp,*z,*w, d, *Vw;
	...
	//evaluate normal velocity along the edge
	if(dparam("ROBINBC_VEL"))
	{
		double *tmp1 = dvector(0,QGmax-1);
			    Vw = dvector(0,QGmax-1);

      /*Get velocity for the specific element*/
	  double *u = dvector(0,E->qtot);
	  double *v = dvector(0,E->qtot);
	  vel->Get_vel_element(E, u, v);
	  
		
	E->GetFace(u,rbc->face,tmp);
	
	E->InterpToFace1(rbc->face,tmp,Vw);
		

		if(E->curvX)
		{
			dvmul  (qa,rbc->nx.p,1,Vw,1,Vw,1);
		}
		else
		{
			dscal  (qa,rbc->nx.d,Vw,1);
		}
	
	
		E->GetFace(v,rbc->face,tmp);
	    E->InterpToFace1(rbc->face, tmp,tmp1);
		
		
		
		if(E->curvX)
		{
			dvvtvp  (qa,rbc->ny.p,1,tmp1,1,Vw,1,Vw,1);
		}
		else
		{
			daxpy  (qa,rbc->ny.d,tmp1,1,Vw,1);
		}
...
static void Tri_Add_robin_matrix(Element *E, LocMat *helm,
                                  Bndry *rbc, AdvVel *vel)
{
//This function is not working properly. Needs further checking
	int     i,j,k,l,lf, m, mf;
	int     totmodes,qa,qb,qc;
	int     cnt,cnt1,cnt2;
	double  **facemass;
	double  *wa, *wb,  *f, *tmp;
	Basis   *Base;
	int     *e_offset, f_offset, *offset;
	Tri      T;
	int      Nf_vert = 3;
	int      lmax = E->lmax;
	int      face  = rbc->face;
	...
	qa = E->qa;
	qb = E->qc; //Triangle face

	
	
    printf("Reading Tri_Add_robin_matrix \n");

	totmodes = E->lmax*(E->lmax+1)/2; // assumes faces are of same order

        offset = ivector(0,totmodes-1);

        // Set up offset array (vertices) 
        for(i = 0; i < Nf_vert; ++i)
        {
            offset[i] = E->vnum(face,i);
        }
        
	// work out edge expansion offset for full elemental matrix
	e_offset = ivector(0,E->Nedges);
	e_offset[0] = E->Nverts;
	for(i = 0; i < E->Nedges; ++i)
	{
            e_offset[i+1] = e_offset[i] + E->edge[i].l;
	}
        
        // Set up offset array (edges) 
        for(i = 0; i < Nf_vert; ++i)
        {
            cnt = e_offset[E->ednum(face,i)];
            for(j = 0; j < lmax-2; ++j)
            {
                offset[Nf_vert+i*(lmax-2)+j] = cnt + j;
            }
        }

	// work out face offset for full elemental  matrix
	f_offset = E->Nverts + E->Nedges*(lmax-2);
	for(i = 0; i < face; ++i)
	{
            if(E->Nfverts(i) == 3)
                f_offset += E->face[i].l*(E->face[i].l+1)/2;
            else
                f_offset += E->face[i].l*E->face[i].l;
	}

        // Set up offset array (edges) 
        for(i = 0; i < (lmax-3)*(lmax-2)/2; ++i)
        {
            offset[Nf_vert*(1+(lmax-2))+i] = f_offset + i;
        }
	...
	getzw(qa,&wa,&wa,'a');
	getzw(qb,&wb,&wb,'b'); //Check

	facemass = dmatrix(0,totmodes-1,0,totmodes-1);
    dzero(totmodes*totmodes, facemass[0], 1);
	f   = dvector(0,qa*qb-1);
	tmp = dvector(0,qb-1);


	//evaluate normal velocity along the edge
	double    *Vw = dvector(0,qa*qb-1);
	if(dparam("ROBINBC_VEL"))
	{
		double *tmp1 = dvector(0,qa*qb-1); //or QGmax*QGmax
		double *tmp2 = dvector(0,qa*qb-1);
		
      //Get velocity for the specific element
	  double *u = dvector(0,E->qtot);
	  double *v = dvector(0,E->qtot);
	  double *w = dvector(0,E->qtot);
	  vel->Get_vel_element(E, u, v, w);
	  
	  printf("qa = %d \n", qa);
	  printf("qb = %d \n", qb);	  
	  	
		  /*Check Get_vel_element*/
		  /*	  
	  for (k=0; k<qa*qb*qb; k++){
	  if(fabs(u[k])>0.01){
	  printf("u[%d] = %lf \n",k, u[k]);}
	  }
	for (k=0; k<qa*qb*qb; k++){
	  ...
	  printf("w[%d] = %lf \n",k, w[k]);}
		...
		E->GetFace(u,rbc->face,tmp1);
	
	
	for (k=0; k<qa*qb; k++){
	  if (fabs(tmp1[k])>0.0001){
	  printf("u_old = %e \n", tmp1[k]);}
	  }
	 
	

		E->InterpToFace1(rbc->face,tmp1,Vw);
		

	
		if(E->curvX)
		{
			dvmul  (qa*qb,rbc->nx.p,1,Vw,1,Vw,1);
		}
		else
		{
			dscal  (qa*qb,rbc->nx.d,Vw,1);
		}
	
		
		E->GetFace(v,rbc->face,tmp1);
	...
	for (k=0; k<qa*qb; k++){
	  ...
	  printf("v_old = %lf \n", tmp1[k]);}
		...
		E->InterpToFace1(rbc->face, tmp1,tmp2);
		
		
		
		if(E->curvX)
		{
			dvvtvp  (qa*qb,rbc->ny.p,1,tmp2,1,Vw,1,Vw,1);
		}
		else
		{
			daxpy  (qa*qb,rbc->ny.d,tmp2,1,Vw,1);
		}

		
	//	for (k=0; k<qa*qb; k++){
	 // if (fabs(Vw[k])>0.001){
	//  printf("Vw_intermedio = %lf \n", Vw[k]);}
 // }
		
		E->GetFace(w,rbc->face,tmp1);
		
		      
	//for (k=0; k<qa*qb; k++){
	//  if (fabs(tmp1[k])>0.001){
	//  printf("w_old[%d]= %lf \n",k, tmp1[k]);}
 // }
		...
		//for (k=0; k<qa*qb; k++){
	//  if (fabs(tmp2[k])>0.001){
	 // printf("w_interp[%d]= %lf \n",k, tmp2[k]);}
 // }
		...
		if(E->curvX)
		{
			dvvtvp  (qa*qb,rbc->nz.p,1,tmp2,1,Vw,1,Vw,1);
		}
		else
		{
			daxpy  (qa*qb,rbc->nz.d,tmp2,1,Vw,1);
		}
		...
		for (k=0; k<qa*qb; k++){
	  printf("Vw_FINAL[%d] = %lf \n",k, Vw[k]);}
	}



	// construct local face mass matrix
	/**********INNER PRODUCT VERTEX MODES********************/
	/* Inner product of vertex modes*/
	for(i = 0; i  < Nf_vert; ++i)
	{
            // fill modes into f
            for(j = 0; j < qb; ++j)
            {
                dvmul(qa, wa, 1, Base->vert[i].a, 1, f+j*qa, 1);
            }

            for(j = 0; j < qb; ++j)
            {
                dscal(qa, wb[j]*Base->vert[i].b[j], f+j*qa, 1);
            }
            
        if(dparam("ROBINBC_VEL"))
		    dvmul(qa*qb,Vw,1,f,1,f,1);
			 
		if(E->curvX)
            {
                dvmul(qa*qb,rbc->sjac.p,1,f,1,f,1);
            }
            else
            {
                dsmul(qa*qb,rbc->sjac.d,f,1,f,1);
            }
            
            
			for(k = 0; k < Nf_vert; ++k)
            {
                // evaluate inner product to get surface mass matrix
                for(j = 0; j < qb; ++j)
                {
                    tmp[j] = ddot(qa,Base->vert[k].a,1,f+qa*j,1);
                }
                facemass[i][k] = ddot(qb,Base->vert[k].b,1,tmp,1);
			}
        }
	...
	/**********INNER PRODUCT VERTEX-EDGES MODES********************/		
		/* Inner product of VERTEX AND EDGES MODES*/

printf("FACEMASS EDGES-VERTEX \n");
	for(i = 0; i  < Nf_vert; ++i)
	{
            // fill modes into f
            for(j = 0; j < qb; ++j)
	 ...
	 if(E->curvX)
            ...
            else
			{
				...
				for(l = 0; l< Nf_vert; ++l)
				{
					for (lf=0; lf<(lmax-2); ++lf)
					{
                // evaluate inner product to get surface mass matrix
						for(j = 0; j < qb; ++j)
						{
							tmp[j] = ddot(qa,Base->edge[l][lf].a,1,f+qa*j,1);
						}
					facemass[i][(Nf_vert + ((lmax-2)*l +lf))] = ddot(qb,Base->edge[l][lf].b,1,tmp,1);
					facemass[(Nf_vert + ((lmax-2)*l +lf))][i] = facemass[i][(Nf_vert + ((lmax-2)*l +lf))];
						}
	}
	}
	
     		
		/**********INNER PRODUCT EDGES-EDGES MODES********************/
		/* Inner product of EDGES AND EDGES MODES*/
	printf("FACEMASS EDGES-EDGES \n");
	for(l = 0; l < Nf_vert; ++l)
	{
		for(lf = 0; lf  < (lmax-2); ++lf)
		{
            ...
            for(j = 0; j < qb; ++j)
            {
                dvmul(qa, wa, 1, Base->edge[l][lf].a, 1, f+j*qa, 1);
            }

            for(j = 0; j < qb; ++j)
            {
                dscal(qa, wb[j]*Base->edge[l][lf].b[j], f+j*qa, 1);
            }
	  ...
	  if(E->curvX)
            ...
            else
            {
				...
				for(m = 0; m< Nf_vert; ++m)
				{
					for (mf=0; mf <(lmax-2); ++mf)
					{
                // evaluate inner product to get surface mass matrix
						for(j = 0; j < qb; ++j)
						{
							tmp[j] = ddot(qa,Base->edge[m][mf].a,1,f+qa*j,1);
						}
					facemass[Nf_vert + ((lmax-2)*l +lf)][Nf_vert + ((lmax-2)*m +mf)] = ddot(qb,Base->edge[m][mf].b,1,tmp,1);
					}
				}
			}	
	    }
		...
		for(i=0; i<Nf_vert; i++) 
	{
      ...
      if(E->curvX)
            ...
            else
            {
				...
				for(m = 0; m< lmax-3; ++m)
				{
					for (mf=0; mf< ((lmax-3)-m) ; ++mf)
					{
                // evaluate inner product to get surface mass matrix
						for(j = 0; j < qb; ++j)
						{
							tmp[j] = ddot(qa,Base->face[0][m][mf].a,1,f+j*qa,1);
						}
					facemass[i][Nf_vert*(1+(lmax-2))+(m +(lmax-3)*mf) - mf*(mf-1)/2] = ddot(qb,Base->face[0][m][mf].b,1,tmp,1);
					facemass[Nf_vert*(1+(lmax-2))+(m +(lmax-3)*mf) - mf*(mf-1)/2][i] =facemass[i][Nf_vert*(1+(lmax-2))+(m +(lmax-3)*mf) - mf*(mf-1)/2];
					}
				}
		...
		/**********INNER PRODUCT INTERIOR-INTERIOR MODES********************/
	
	printf("FACEMASS INTERIOR-INTERIOR \n");
	
		for(l = 0; l  < lmax-3; ++l) //Lface==(lmax-2)(lmax-2)
	{
		for(lf = 0; lf < (lmax-3)-l; ++lf)
		{
			// fill modes into f
            for(j = 0; j < qb; ++j)
            {
                dvmul(qa, wa, 1, Base->face[0][l][lf].a, 1, f+j*qa, 1);
				
            }

            for(j = 0; j < qb; ++j)
            {
                dscal(qa, wb[j]*Base->face[0][l][lf].b[j], f+j*qa, 1);
    ...
    if(E->curvX)
            ...
            else
            {
                ...
                // evaluate inner product to get surface mass matrix
						for(j = 0; j < qb; ++j)
						{
							tmp[j] = ddot(qa,Base->face[0][m][mf].a,1,f+qa*j,1);
						}
			facemass[Nf_vert*(1+lmax-2)+(l +(lmax-3)*lf) - lf*(lf-1)/2][Nf_vert*(1+lmax-2)+(m +(lmax-3)*mf) - mf*(mf-1)/2] = ddot(qb,Base->face[0][m][mf].b,1,tmp,1);
					}
				}
			}	
	    }
...
printf("FACEMASS EDGE-INTERIOR \n");
      ...
      if(E->curvX)
            ...
            else
            {
                ...
                // evaluate inner product to get surface mass matrix
					...
					facemass[Nf_vert + (lmax-2)*l+lf][Nf_vert*(1+lmax-2) +(m +(lmax-3)*mf) - mf*(mf-1)/2] = ddot(qb,Base->face[0][m][mf].b,1,tmp,1);
					facemass[Nf_vert*(1+lmax-2) +(m +(lmax-3)*mf) - mf*(mf-1)/2][Nf_vert + (lmax-2)*l+lf] = facemass[Nf_vert + (lmax-2)*l+lf][Nf_vert*(1+lmax-2) +(m +(lmax-3)*mf) - mf*(mf-1)/2];
					}
				}
			}	
	    }
	...
	//Print facemass matrix


// add matrix  TOTAL
	 printf("FACEMASS MATRIX \n");
        for(i = 0; i < totmodes; ++i)
        {
            for(j = 0; j <totmodes; ++j)
            {
                helm->a[offset[i]][offset[j]] += sigma*facemass[i][j];
               // printf("%lf \t", facemass[i][j]);             
			}
      //  printf("\n");
		}        
...
printf("Reading Quad_Add_robin_matrix \n");
printf("Elemento numero %d \n",E->id);

        offset = ivector(0,lmax*lmax-1);
        // Set up offset array (vertices) 
        for(i = 0; i < Nf_vert; ++i)
        {
            offset[i] = E->vnum(face,i);
           // printf("VERTEX  offset[%d] = %d \n", i, offset[i]);
		}
	...
	// work out edge expansion offset for full elemental matrix
	e_offset = ivector(0,E->Nedges);
	e_offset[0] = E->Nverts; //AnaPlata After the 8 vertex!!! BUG!!!!!!!!
	for(i = 0; i < E->Nedges; ++i)
	{
            e_offset[i+1] = e_offset[i] + E->edge[i].l;
		}
        ...
        for(i = 0; i < Nf_vert; ++i)
        {
            ...
            {
                offset[Nf_vert+i*(lmax-2)+j] = cnt + j;
           // printf("EDGE offset[%d]= %d \n", Nf_vert+i*(lmax-2)+j, offset[Nf_vert+i*(lmax-2)+j]);
			}
        }

	// work out face offset for full elemental  matrix
	
	f_offset = E->Nverts + E->Nedges*(lmax-2); //AnaPlata. After vertex and edges!!!
	for(i = 0; i < face; ++i)
	{
            if(E->Nfverts(i) == 3)
                f_offset += E->face[i].l*(E->face[i].l+1)/2;
               ...
               // f_offset += E->face[i].l*E->face[i].l;
				f_offset += (lmax-2)*(lmax-2);
	}


        // Set up offset array (edges) 
        for(i = 0; i < (lmax-2)*(lmax-2); ++i)
        {
            offset[Nf_vert*(1+(lmax-2))+i] = f_offset + i;
       // printf("INTERIOR offset[%d]= %d \n", Nf_vert*(1+(lmax-2))+i, offset[Nf_vert*(1+(lmax-2))+i]);
		}
...
printf("FACEMASS EDGES-VERTEX \n");
	for(i = 0; i  < Nf_vert; ++i)
	{
			...
			dvmul(qa*qb,rbc->sjac.p,1,f,1,f,1);
                ...
                // evaluate inner product to get surface mass matrix
					...
					facemass[i][(Nf_vert + ((lmax-2)*l +lf))] = ddot(qb,Base->edge[l][lf].b,1,tmp,1);
					facemass[(Nf_vert + ((lmax-2)*l +lf))][i] = facemass[i][(Nf_vert + ((lmax-2)*l +lf))];
				//printf ("%lf   " ,facemass[i][(Nf_vert + ((lmax-2)*l +lf))]);
			}
			}
      //  printf("\n");	
	}
	...
	for(l = 0; l < Nf_vert; ++l)
	{
		for(lf = 0; lf  < (lmax-2); ++lf)
		{
			...
			dvmul(qa*qb,rbc->sjac.p,1,f,1,f,1);
                ...
                // evaluate inner product to get surface mass matrix
					...
					facemass[Nf_vert + ((lmax-2)*l +lf)][Nf_vert + ((lmax-2)*m +mf)] = ddot(qb,Base->edge[m][mf].b,1,tmp,1);
					//printf ("%lf   " ,facemass[Nf_vert + ((lmax-2)*l +lf)][(Nf_vert + ((lmax-2)*m +mf))]);
			}
			}
        //printf("\n");				
			}	
		...
		/*************INNER PRODUCT VERTEX-INTERIOR*************/
		printf("FACEMASS VERTEX-INTERIOR \n");
		for(i=0; i<Nf_vert; i++) 
	{
            ...
            dvmul(qa*qb,rbc->sjac.p,1,f,1,f,1);
                ...
                // evaluate inner product to get surface mass matrix
					...
					facemass[i][Nf_vert*(1+(lmax-2))+(m +(lmax-2)*mf)] = ddot(qb,Base->face[0][m][mf].b,1,tmp,1);
					facemass[Nf_vert*(1+(lmax-2))+(m +(lmax-2)*mf)][i] =facemass[i][Nf_vert*(1+(lmax-2))+(m +(lmax-2)*mf)];
					//printf("%lf   ", facemass[i][Nf_vert*(1+(lmax-2))+((lmax-2)*m +mf)]);
					}
				}
        // printf ("\n");
	    }
	...
	printf("FACEMASS INTERIOR-INTERIOR \n");
		...
		for(l = 0; l  < lmax-2; ++l) //Lface==(lmax-2)(lmax-2)
	{
		for(lf = 0; lf < lmax-2; ++lf)
		{
			// fill modes into f
           ...
           if(dparam("ROBINBC_VEL"))
			...
			facemass[Nf_vert*(1+lmax-2)+(l+(lmax-2)*lf)][Nf_vert*(1+lmax-2)+(m +(lmax-2)*mf)] = ddot(qb,Base->face[0][m][mf].b,1,tmp,1);
								//printf(" %lf \t", facemass[Nf_vert*(1+lmax-2)+((lmax-2)*l+lf)][Nf_vert*(1+lmax-2)+((lmax-2)*m +mf)]);
					}
				}
			//printf ("\n");
			}	
           ...
           if(dparam("ROBINBC_VEL"))
		    ...
		    dvmul(qa*qb,rbc->sjac.p,1,f,1,f,1);
                ...
                // evaluate inner product to get surface mass matrix
					...
					facemass[Nf_vert + (lmax-2)*l+lf][Nf_vert*(1+lmax-2) +(m +(lmax-2)*mf)] = ddot(qb,Base->face[0][m][mf].b,1,tmp,1);
					facemass[Nf_vert*(1+lmax-2) +(m +(lmax-2)*mf)][Nf_vert + (lmax-2)*l+lf] = facemass[Nf_vert + (lmax-2)*l+lf][Nf_vert*(1+lmax-2) +(m +(lmax-2)*mf)];
					}
				}
			}	
	    }
...
void CorrectRobinFluxes(Bndry *B)
{
	...
	for(Ubc=B;Ubc;Ubc=Ubc->next)
		if(Ubc->type=='R')
		{
			...
			if(E->vert[vn1].solve && E->vert[vn2].solve)
				break;
...
// NOT USED IN THIS VERSION! In case you modify the code and include a call to
// this function, please remove this comment.
void Setup_Preconditioner(Element_List *U, Bsystem *Ubsys)
{
	...
	switch(Ubsys->Precon)
	{
		case Pre_Diag:
			...
			break;
		default:
			fprintf(stderr,"Preconditioner type not handled in "
...
// NOT USED IN THIS VERSION! In case you modify the code and include a call to
// this function, please remove this comment.
void UpdateMatList(Element_List *U, Bsystem *Ubsys, Metric *lambda, 
	...
	DO_PARALLEL 
	{
	            ...
	            && Ubsys->pll->nsolve == Ubsys->pll->nglobal)
    	{
		    ...
		    for(k = 0; k < U->nel; ++k)
			    for(i = 0; i < U->flist[k]->Nverts; ++i)
				    if((Ubsys->singular-1) ==
					    Ubsys->pll->solvemap[Ubsys->bmap[k][i]])
...
static void SetLowEnergyModes(Element *U, Bsystem *B)
{
	...
	if(verts[4]&&verts[6])
	{ /* if mixed element formulation then
		...
		if(TNf)
		{
			...
			for(i = 0; i < Nverts; ++i)
			{
				...
				{
					if(id1[i][j]+1)
						dcopy(ll,TRv[id2[i][j]] + cnt1 + id1[i][j]*ll,1,
								Rv[i] + cnt,1);
			...
			for(i = 0; i < Nedges; ++i)
				for(k= 0; k < Ep->edge[i].l; ++k)
				{
					...
					{
						if(id1[i][j]+1)
							dcopy(ll,TRe[le*id2[i][j]+k]+id1[i][j]*ll,1,
									Re[i*le+k]+cnt,1);
		...
		free_imatrix(id1,0,0);
		free_imatrix(id2,0,0);
					...
					switch(j)
					{
							...
							for(l = 0; l < qb; ++l)
								for(k = 0; k < qa; ++k)
									fprintf(fp,"%lf %lf %lf %lf\n",
									        X.x[l*qa + k]+
											nx*E->h_3d[0][0][l*qa+k],
									        X.y[l*qa + k]+
											ny*E->h_3d[0][0][l*qa+k],
									        X.z[l*qa + k]+
											nz*E->h_3d[0][0][l*qa+k],
											E->h_3d[0][0][l*qa+k]);
							break;
						...
						case 1:
							...
							for(l = 0; l < qc; ++l)
								for(k = 0; k < qa; ++k)
									fprintf(fp,"%lf %lf %lf %lf\n",
									        X.x[l*qa*qb+k]+
											nx*E->h_3d[0][0][l*qa*qb+k],
									        X.y[l*qa*qb+k]+
											ny*E->h_3d[0][0][l*qa*qb+k],
									        X.z[l*qa*qb+k]+
											nz*E->h_3d[0][0][l*qa*qb+k],
						...
						case 2:
							...
							for(l = 0; l < qc; ++l)
								for(k = 0; k < qb; ++k)
									fprintf(fp,"%lf %lf %lf %lf\n",
									        X.x[l*qa*qb+k*qa+qa-1]+
											nx*E->h_3d[0][0][l*qa*qb+k*qa+qa-1],
									        X.y[l*qa*qb+k*qa+qa-1]+
											ny*E->h_3d[0][0][l*qa*qb+k*qa+qa-1],
									        X.z[l*qa*qb+k*qa+qa-1]+
											nz*E->h_3d[0][0][l*qa*qb+k*qa+qa-1],
						...
						case 3:
							...
							for(l = 0; l < qc; ++l)
								for(k = 0; k < qb; ++k)
									fprintf(fp,"%lf %lf %lf %lf\n",
									        X.x[l*qa*qb+k*qa]+
											nx*E->h_3d[0][0][l*qa*qb+k*qa],
									        X.y[l*qa*qb+k*qa]+
											ny*E->h_3d[0][0][l*qa*qb+k*qa],
									        X.z[l*qa*qb+k*qa]+
											nz*E->h_3d[0][0][l*qa*qb+k*qa],
				...
				for(j = 0; j < E->Nfaces; ++j)
				{
					switch(j)
					{
						...
						case 4:
							...
							fprintf(fp,"ZONE T=\"Vert %d  face %d\" I=%d, "
									"J=%d, F=POINT\n",i+1,j+1,qb,qc);
							for(l = 0; l < qc; ++l)
								for(k = 0; k < qb; ++k)
									fprintf(fp,"%lf %lf %lf %lf\n",
						...
						default:
							...
							fprintf(stderr,"Please define geom in "
									"dump_le_modes\n");
							exit(-1);
							break;
					}
					...
					for(j = 0; j < E->Nfaces; ++j)
					{
						switch(j)
						{
							case 0:
								...
								fprintf(fp,"ZONE T=\"Edge %d mode %d face %d\" "
									"I=%d, J=%d, F=POINT\n",i+1,k1+1,j+1,qa,qb);
								for(l = 0; l < qb; ++l)
									for(k = 0; k < qa; ++k)
										fprintf(fp,"%lf %lf %lf %lf\n",
...
void Tri::LowEnergyModes(Bsystem *, int *, int *,
                         double ***, double ***, double ***)
{
	error_msg(LowEnergyModes not define in Matrix.C);
}

void Tri::MakeLowEnergyModes(Bsystem *, int *, int *,
                             double ***, double ***, double ***)
{}

void Quad::LowEnergyModes(Bsystem *, int *, int *,
                          double ***, double ***, double ***)
{
	error_msg(LowEnergyModes not define in Matrix.C);
}
void Quad::MakeLowEnergyModes(Bsystem *, int *, int *,
                              double ***, double ***, double ***)
{}
                         ...
                         double ***Rvi, double ***Re)
{
	...
	if(option("variable"))
		error_msg(LowEnergyModes not set
			          up for variable order)
				;

	// check link list
	for(Rvm = Rv_mat; Rvm; Rvm = Rvm->next)
		if(fabs(Rvm->lambda - B->lambda->d) < FPTOL)
		{
			Ne [0] = Rvm->Ne;
			Nf [0] = Rvm->Nf;
			Rv [0] = Rvm->Rv;
...
void Pyr::LowEnergyModes(Bsystem *, int *, int *,
                         double ***, double ***, double ***)
{
	error_msg(LowEnergyModes not define in Matrix.C);
}
...
void Pyr::MakeLowEnergyModes(Bsystem *, int *, int *,
                             double ***, double ***, double ***)
{}
                              ...
                              int *el, int *fl, Element *E)
{
	...
	switch(Nv)
	{
		case 4: /* Tet */
			...
			break;
		case 5: /* Pyramid */
			error_msg(LowEnergyVertices not set
				          up for pyramids)
					;
			break;
		case 6: /* Prism   */
                           ...
                           int *fl, Element *E)
{
	...
	if(flen)
	{
		...
		for(i = 0,n=0,cnt=0; i < 2; ++i)
		{
			...
			for(j = 0; j < l; ++j,++cnt)
			{
				dcopy(elen,A[Nv+el[Ne]+fl[face[i]]+j] + Nv + el[edge] ,1,*f+cnt,
					  flen);
				dcopy(l-j, A[Nv+el[Ne]+fl[face[i]]+j]+Nv+el[Ne]+fl[face[i]]+j,1,
				      a+n,1);
				n+=l-j;
				for(k = i+1; k < 2; ++k)
				{
...
void Element::LowEnergyModes(Bsystem *, int *, int *,
                             double ***, double ***, double ***)
{
	ERR;
}
void Element::MakeLowEnergyModes(Bsystem *, int *, int *,
                                 double ***, double ***, double ***)
{
	ERR;
}
...
void Element::MassMat (LocMat *)
{
	ERR;
}             // return mass-matri(ces)
...
{
	register int i,j,n;
	int nbl = Nbmodes, N = Nmodes - nbl, asize = mass->asize,
	                       csize = mass->csize;
	int      L;
	Basis   *B = getbasis();
	double **a = mass->a,
...
void  Quad::HelmMatC(LocMat *mass, Metric *lambda)
{
	...
	register int i,j,n;
	int nbl = Nbmodes, N = Nmodes - nbl,
	    csize = mass->csize,asize = mass->asize;
	int      L;
	Basis   *B = getbasis();
	double **a = mass->a,
	       **b = mass->b,
	...
	for(i = 0,n=0; i < Nverts; ++i,++n)
	{
		...
		dcopy(nbl,vert->hj,1,*a+n,asize);
			...
			if(lambda->wave&&N)					// For Olseen solver
			dcopy(N,vert->hj+nbl,1,d[n],1);
	}	
...
void Element::HelmMatC(LocMat *, Metric *)
{
	ERR;
}     // return Helmholtz op.
...
void  Tri::LapMat(LocMat *lap)
{
	...
	for(i = nbl; i < Nmodes; ++i)
		for(j = nbl; j < Nmodes; ++j)
			c[i-nbl][j-nbl] = c[j-nbl][i-nbl] = jac*
												iprodlap(gb[i],gb1[j],fac+1);
...
void Tri::project(LocMat *m, Bsystem *Ubsys)
{
	...
	if(Ubsys->rslv)
	{/* project a to first recursion */
	 /* close pack the a matrix to eliminate any extra zero for variable case */
		for(i = 1; i < asize; ++i)
	...
	{           
		...
		if((nsolve = Ubsys->nsolve) != 0)
		{
			for (i = 0; i < asize; i++)
				if ((pi=list[i]) < nsolve)
					for(j = 0; j < asize; j++)
						if ((pj=list[j]) < nsolve)
							AddContSMat(a[i][j], Ubsys->Gmat->Spa, pi, pj);
		}
...
void Element::project(LocMat *, Bsystem *)
{
	ERR;
}
...
Function Purpose: performs static condensation of the elemental matrices
 
Argument 1: LocMat *m
Purpose: Local (elemental) matrix
 
Argument 2: Bsystem *Ubsys
Purpose: Global matrix system
 
Argument 3: char pack
...
void Quad::condense(LocMat *m, Bsystem *Ubsys, char pack)
{
	...
	if(csize)
	{
		...
		/* calc inv(c)*b^T */
		if(csize > 2*bw[geom_id])
			dpbtrs('L',csize,bw[geom_id]-1,asize,invc,
			       bw[geom_id],binvc,csize,info);
		else
			dpptrs('L',csize,asize,invc,binvc,csize,info);
...
void Tet::condense(LocMat *m, Bsystem *Ubsys, char pack)
{
	...
	if(csize)
	{
		...
		/* calc inv(c)*b^T */
		if(csize > 2*bw[geomid])
			dpbtrs('L',csize,bw[geomid]-1,asize,invc,bw[geomid],binvc,csize,
					info);
		else
			dpptrs('L',csize,asize,invc,binvc,csize,info);
...
void Pyr::condense(LocMat *m, Bsystem *Ubsys, char pack)
{
	...
	if(csize)
	{
		...
		/* calc inv(c)*b^T */
		if(csize > 2*bw[geom_id])
			dpbtrs('L',csize,bw[geom_id]-1,asize,invc,bw[geom_id],binvc,csize,
					info);
		else
			dpptrs('L',csize,asize,invc,binvc,csize,info);
...
void Element::condense(LocMat *, Bsystem *, char )
{
	ERR;
}
...
void Element::fill_diag_massmat()
{
	ERR;
}
...
void Element::fill_diag_helmmat(Metric *)
{
	ERR;
}
...
static void Selec_Bsystem_mem (Bsystem *Ubsys, Element *U);

/*************  Functions for selective matrix regeneration ************/

/*--------------------------------------------------------------------------*
 *																			*
 * Function Selec_Bsystem_mem_free											*
 *																			*
 * Description: Frees memory allocated for bsystem in the positions 		*
 *              reserved for the deformable elements. 						*
 * Called by: Update_Bsystem in ALE.C (this file)							*
 * Parameters: Bsystem *Ubsys - Matrix system								*
 *			   Element_List *U - Element list containing variable to be		*
 *								 solved										*
 *  																		*
 *--------------------------------------------------------------------------*/
...
void Selec_Bsystem_mem_free(Bsystem *Ubsys, Element_List *U)
{
	...
	if(!Ubsys->rslv) /* if not a recursive solver */
	{
		// Free A matrix (boundary-boundary)
		if(Ubsys->smeth == iterative)
		{
#ifdef SPARSEMAT
			
			// For SparseMatrix implementation, only subtract contribution of
			// the degrees of freedom associated to deformable elements and
			...
			for(k=0; k<nfam; ++k)				// For ALE solver, nfam == nel
			{									// and geom->id == E->id
		...
		else
		{
			...
			{
#ifdef UMFPACKSLV		// UMFPACK implementation
				umfpack_di_free_numeric(&Ubsys->Gmat->Spa.numeric);
				Ubsys->Gmat->Spa.numeric = NULL;
#else					// LAPACK implementation
				Ubsys->Gmat->bwidth_a = 0;
				free_dmatrix(Ubsys->Gmat->inva,0,0);
#endif
	...
	// Freeing c matrices of deformable elements
	for(i=0;i<nfam;++i)
	{
		if (U->flist[i]->group < 0)
		{
			free(Ubsys->Gmat->invc[i]);
			free(Ubsys->Gmat->binvc[i]);
			Ubsys->Gmat->invc[i] = (double*)NULL;
			Ubsys->Gmat->binvc[i] = (double*)NULL;
		}
	}
	...
	if (Ubsys->rslv)	// Recursive solver
	{
		...
		{
#ifdef UMFPACKSLV		// UMFPACK implementation
			umfpack_di_free_numeric(&rslv->A.Spa.numeric);
			rslv->A.Spa.numeric = NULL;
#else					// LAPACK implementation
			free(rslv->A.inva);
			free(rslv->A.pivota);
#endif
...
{
	LocMat  *mass = 0, **helm;
	static LocMat ****HelmMats = (LocMat****)NULL;
	const   int nel = U->nel;
	Bndry   *Bc;
	Element *E;
	static int SVV = (fabs(dparam("SVVfactor")) > FPTOL) ? 1:0;
	static bool **initarr;
	static double **plambdaarr;
	bool *init;
	double *plambda;
	int mel = zel/2;
	...
	if (!HelmMats)
	{
		int nmodes = (option("NZ") == 1) ? 1 : option("NZ")/2;

		HelmMats = (LocMat****)malloc(sizeof(LocMat***)*nmodes);
		initarr = (bool**)malloc(sizeof(bool*)*nmodes);
		plambdaarr = (double**)malloc(sizeof(double*)*nmodes);
		for (n = 0; n < nmodes; n++)
		{
			if (option("REFLECT"))
			{
				HelmMats[n] = (LocMat***)malloc(sizeof(LocMat**)*3);
				initarr[n] = (bool*)malloc(sizeof(bool)*3);
				plambdaarr[n] = (double*)malloc(sizeof(double)*3);
				for (i = 0; i < 3; i++)
				{
					HelmMats[n][i] = (LocMat**)malloc(sizeof(LocMat*)*nel);
					initarr[n][i] = true;
				}
			}
			else
			{
				HelmMats[n] = (LocMat***)malloc(sizeof(LocMat**)*2);
				initarr[n] = (bool*)malloc(sizeof(bool)*2);
				plambdaarr[n] = (double*)malloc(sizeof(double)*2);
				for (i = 0; i < 2; i++)
				{
					HelmMats[n][i] = (LocMat**)malloc(sizeof(LocMat*)*nel);
					initarr[n][i] = true;
	...
	switch (U->fhead->type)
	{
		case 'p':
			init = &initarr[mel][0];
			helm = HelmMats[mel][0];
			plambda = &plambdaarr[mel][0];
			break;
		case 'u':
			init = &initarr[mel][1];
			helm = HelmMats[mel][1];
			plambda = &plambdaarr[mel][1];
			break;
		case 'v':
			init = &initarr[mel][2];
			helm = HelmMats[mel][2];
			plambda = &plambdaarr[mel][2];
			break;
		default:
			error_msg(Selec_GenMat: unrecognized type);
			break;
	}
	...
	{
		Selec_Bsystem_mem(Ubsys,U->fhead);

		if(Ubsys->Precon == Pre_LEnergy) // transform back to std basis
			TransFromLeBasis(U->fhead,Ubsys);	// Needs verification!!!
	}
	...
	if(Ubsys->smeth == iterative)
	{
		...
		{
			if(option("ReCalcPrecon") && *init)    // ALE -
				ZeroPrecon(Ubsys,U->fhead->dim()); //init stuff added to
		}										   //correct strange bug
	}
	...
	switch(Stype)
	{
		case Mass:
			...
			{
				mass = E->mat_mem ();
				if(Ubsys->smeth != iterative||!Ubsys->Gmat->invc[E->geom->id])
				{
					if(!E->curvX)
						E->MassMat(mass);
					else
		...
		case Helm:
			...
			{	// Selective regeneration
					...
					fabs(lambda[0].d - *plambda) > FPTOL)
				{
					        ...
					        !Ubsys->Gmat->invc[E->geom->id])
					{
						if(E->identify()==Nek_Tri && !E->curvX &&
											  !lambda[E->id].p && !SVV)
							Tri_HelmMat(E, helm[E->id], lambda[E->id].d);
						else if (E->identify()==Nek_Quad && !lambda[E->id].p &&
								 !SVV)
						{
							// parallelogram
							if ((!E->curvX || E->curve->type == T_Straight) &&
								 ...
								 (fabs((E->vert[1].y - E->vert[0].y) -
							 	  	(E->vert[2].y - E->vert[3].y)) < FPTOL))
								Prllgram_HelmMat(E,helm[E->id],lambda[E->id].d);
							else	// non-parallelogram
								E->HelmMatC (helm[E->id],lambda+E->id);
						}
						else
							E->HelmMatC (helm[E->id],lambda+E->id);
            ...
            E->Add_robin_matrix(helm[E->id], Bc,Ubsys->vel);




						E->condense(helm[E->id],Ubsys,'y');
					}
				}

				// Projection on the global system
				if(Ubsys->smeth == direct || Ubsys->rslv)
					E->project (helm[E->id],Ubsys);

				// Freeing memory
				if (E->group < 0) //free entire matrix for deformable elements
					E->mat_free (helm[E->id]);
				else
				{	// Maintain A matrix for rigid elements in case
					// projection is needed, otherwise free entire matrix
					if (*init)
					{	
						if(Ubsys->smeth == direct || Ubsys->rslv)
						{
							...
							if(helm[E->id]->csize)
							{
								free_dmatrix(helm[E->id]->b,0,0);
								free_dmatrix(helm[E->id]->c,0,0);
								if(helm[E->id]->d)
									free_dmatrix(helm[E->id]->d,0,0);
							}
						}
						else
						{
...
finished:
	...
	{ /* recursive solver setup */
		...
		if(Ubsys->smeth == direct)
		{
			...
			// Initialize SMatrix structure
			rdata = Ubsys->rslv->rdata + Ubsys->rslv->nrecur-1;
			if (Ubsys->rslv->A.Spa.nlines != rdata->cstart)
				InitSMat(Ubsys->rslv->A.Spa, rdata->cstart);
			if (Ubsys->rslv->A.Spa.tmprhs == NULL)
				Ubsys->rslv->A.Spa.tmprhs = dvector(0, rdata->cstart-1);
#endif
			Rinvert_a(Ubsys,'p');
		}
	...
	else /* standard statically condensed set up */
	{
		...
		else
		{
			...
			if (Ubsys->Precon != Pre_LEnergy)
			{
				if (*init || fabs(lambda[0].d - *plambda) > FPTOL)
					BuildSMat(Ubsys, U);
				else
					Selec_BuildSMat(Ubsys, U);
			}
#endif			
			...
			if(option("ReCalcPrecon"))
			{
				...
				if(Ubsys->Precon == Pre_LEnergy) 	// transform basis for
					Trans2LeBasis(U->fhead,Ubsys);	// low energy preconditioner
...
static void Selec_Bsystem_mem(Bsystem *Ubsys, Element *U)
{
	...
	if(!Ubsys->rslv && Ubsys->smeth == direct)
	{
		if(nsolve)
		{   
#ifndef UMFPACKSLV		// LAPACK implementation
			Ubsys->Gmat->bwidth_a = bandwidth(U,Ubsys);
			if(2*Ubsys->Gmat->bwidth_a < nsolve)	// banded solver
			{
				Ubsys->Gmat->inva =
