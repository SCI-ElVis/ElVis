...
/* Private functions */
static void setup_RHS (Element_List *U, Element_List *F, double *r, double *u0, 
...
static void A_cg(Element_List *U, Bsystem *B, double *p, double *w,
				 SolveType Stype);
...
static void solve_system_CG (Element_List *U, double *rhs, double *uo,
							 Bsystem *B, SolveType Stype);
              ...
              SolveType Stype)
{
	...
	solve_system_CG(U,RHS,U0,Ubsys,Stype);
...
static void Set_tolerance(Element_List *E_L, Element_List *Ef_L)
{
	...
	if(E->type != 'p')
	{
		...
		if(E->dim() == 2)
		{
			...
			full_toli = (full_toli+1)%2;
		}
		else
		{
			full_tol = sqrt(full_tolv[0]*full_tolv[0] +
			                full_tolv[1]*full_tolv[1] +
						    full_tolv[2]*full_tolv[2])/3.;
			full_toli = (full_toli+1)%3;
		}
	}
							 ...
							 Bsystem *B, SolveType Stype)
{
	...
	epsfac = (fabs(full_tol) > FPTOL) ? 1.0/full_tol : 1.0;
