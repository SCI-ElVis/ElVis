...
void Tri::split_edge(int edg, Element_List *EL, Bndry **Ubc, int nfields)
{
...
void Quad::split_edge(int edg, Element_List *EL, Bndry **Ubc, int nfields)
{
	int i, j, newid = EL->nel;
	int *link_eid  = ivector(0, Nedges-1);
	int *link_face = ivector(0, Nedges-1);
...
void Tet::split_edge(int, Element_List *, Bndry **, int)
{
...
void Pyr::split_edge(int, Element_List *, Bndry **, int)
{
...
void Prism::split_edge(int, Element_List *, Bndry **, int)
{
...
void Hex::split_edge(int, Element_List *, Bndry **, int)
{
	fprintf(stderr, "Hex::split_edge not implemented yet\n");
	return;
}
...
void Element::split_edge(int, Element_List *, Bndry **, int)
{
	ERR;
}
...
void Tri::split_element(Element_List *EL, Bndry **Ubc, int nfields, int *&flag)
{
	...
	//Assumed: edge order given in flag array is counter clockwise 
	//(ie 123, 312, etc..)

	//It is assumed that the flag array is declared dynamically. Once the
	//information is used, the array is deleted, and a new array is passed
	//back in its place.  The new array contains
	...
	{
		case 1:
			split_edge(flag[1], EL, Ubc, nfields);
			edge[1].base = NULL;
		...
		case 2:
			...
			newX.x[0] = centX.x[flag[1]];
			newX.y[0] = centX.y[flag[1]];
			newX.x[1] = origX.x[flag[2]];
			newX.y[1] = origX.y[flag[2]];
			newX.x[2] = centX.x[flag[2]];
			newX.y[2] = centX.y[flag[2]];
			...
			free(newX.y);
			 ...
			 //Nverts will be 3, so we are allocating for a quad
			newX.x = dvector(0, Nverts);
			newX.y = dvector(0, Nverts);

			// Element 2 //Quad element
			newX.x[0] = origX.x[flag[1]];
			newX.y[0] = origX.y[flag[1]];
			newX.x[1] = centX.x[flag[1]];
			newX.y[1] = centX.y[flag[1]];
			newX.x[2] = centX.x[flag[2]];
			newX.y[2] = centX.y[flag[2]];
			newX.x[3] = origX.x[(flag[2]+1)%Nverts];
			newX.y[3] = origX.y[(flag[2]+1)%Nverts];
			...
			EL->flist[EL->nel]->next = (Element*) 0; 
									//set the last elements next to NULL
			...
			//set inside edge connection (between tri and quad)
			set_link(EL, EL->nel,1,id,2); 
			edge[0].base = NULL;
			edge[1].base = NULL;
			...
			{	//connection to the outside element
				set_link(EL, EL->nel-1,3,link_eid[(flag[2]+1)%Nverts],
						 link_face[(flag[2]+1)%Nverts]); 
			}
			else
			{
				// check for bc on edge 0
		...
		default:
			...
			if(flag[0]!=3) //if flag[0]=3, we want no side splitting
				for(i=0;i<Nverts;++i)
					if(link_eid[i] != -1)
						EL->flist[link_eid[i]]->split_edge(link_face[i], EL,
													 		Ubc, nfields);
			...
			if(link_eid[0] != -1)
			{
				if(flag[0]!=3)
				{
					...
					nel += (EL->flist[link_eid[0]]->identify() == Nek_Tri) ?
							 1:2;
				}//endif
			}
			else
			...
			if(link_eid[1] != -1)
			{
				if(flag[0]!=3)
				{
					...
					nel += (EL->flist[link_eid[1]]->identify() == Nek_Tri) ?
							 1:2;
				}//endif
			}
			else
			...
			if(link_eid[2] != -1)
			{
				if(flag[0]!=3)
				{
					...
					nel += (EL->flist[link_eid[2]]->identify() == Nek_Tri) ?
							 1:2;
				}
			}
			else
...
void Quad::split_element(Element_List *EL, Bndry **Ubc, int nfields, int *&flag)
{
	...
	switch(flag[0])
	{
		...
		case 2:
			...
			// Element 2
			newX.x[0] =  centX.x[flag[2]];
			newX.y[0] = centX.y[flag[2]];
			newX.x[1] =  origX.x[(flag[2]+1)%Nverts];
			newX.y[1] = origX.y[(flag[2]+1)%Nverts];
			newX.x[2] =  origX.x[(flag[2]+2)%Nverts];
			newX.y[2] = origX.y[(flag[2]+2)%Nverts];
			newX.x[3] =  EcentX.x[0];
			newX.y[3] = EcentX.y[0];
			...
			// Element 3
			newX.x[0] =  origX.x[(flag[2]+2)%Nverts];
			newX.y[0] =  origX.y[(flag[2]+2)%Nverts];
			newX.x[1] =  origX.x[flag[1]];
			newX.y[1] =  origX.y[flag[1]];
			newX.x[2] =  centX.x[flag[1]];
			newX.y[2] =  centX.y[flag[1]];
			newX.x[3] =  EcentX.x[0];
			newX.y[3] =  EcentX.y[0];
			...
			set_link(EL, EL->nel, 1, link_eid[(flag[2]+1)%Nverts],
					 link_face[(flag[2]+1)%Nverts]);
			set_link(EL, EL->nel+1, 0, link_eid[(flag[2]+2)%Nverts],
					 link_face[(flag[2]+2)%Nverts]);
		...
		case 3:
			...
			// Element 1
			newX.x[0] = centX.x[flag[3]];
			newX.y[0] =  centX.y[flag[3]];
			newX.x[1] = origX.x[(flag[3]+1)%Nedges];
			newX.y[1] =  origX.y[(flag[3]+1)%Nedges];
			newX.x[2] = origX.x[(flag[3]+2)%Nedges];
			newX.y[2] =  origX.y[(flag[3]+2)%Nedges];
			newX.x[3] = centX.x[(flag[3]+2)%Nedges];
			newX.y[3] =  centX.y[(flag[3]+2)%Nedges];

			move_vertices(&newX);

			// Element 2
			newX.x[0] =  centX.x[(flag[3]+2)%Nedges];
			newX.y[0] =  centX.y[(flag[3]+2)%Nedges];
			newX.x[1] =  origX.x[flag[2]];
			newX.y[1] = origX.y[flag[2]];
			newX.x[2] =  centX.x[flag[2]];
			newX.y[2] = centX.y[flag[2]];
			...
			newX.x[0] =  centX.x[flag[2]];
			newX.y[0] = centX.y[flag[2]];
			newX.x[1] =  origX.x[flag[3]];
			newX.y[1] = origX.y[flag[3]];
			newX.x[2] =  centX.x[flag[3]];
			newX.y[2] = centX.y[flag[3]];
			...
			newX.x[0] =  centX.x[flag[2]];
			newX.y[0] = centX.y[flag[2]];
			newX.x[1] =  centX.x[flag[3]];
			newX.y[1] = centX.y[flag[3]];
			newX.x[2] =  centX.x[flag[1]];
			newX.y[2] = centX.y[flag[1]];
			...
			set_link(EL, id, 1, link_eid[(flag[3]+1)%Nverts],
					 link_face[(flag[3]+1)%Nverts]);
		...
		default:
			...
			if(link_eid[3] != -1)
			{
				if(flag[0]!=4)
				{
					...
					nel += (EL->flist[link_eid[3]]->identify() == Nek_Tri) ?
							 1:2;
				}
			}
			else
...
void Tet::split_element(Element_List *, Bndry **, int, int *&)
{
...
void Pyr::split_element(Element_List *, Bndry **, int, int *&)
{
	fprintf(stderr, "Hex::split_element not implemented yet\n");
	return;
}
...
void Prism::split_element(Element_List *, Bndry **, int, int *&)
{
	fprintf(stderr, "Hex::split_element not implemented yet\n");
	return;
}
...
void Hex::split_element(Element_List *, Bndry **, int, int *&)
{
	fprintf(stderr, "Hex::split_element not implemented yet\n");
	return;
}
...
void Element::split_element(Element_List *, Bndry **, int, int *&)
{
	ERR;
}
...
//------------------------------------------------------------------------
			...
			which_split = 0;
			if	((origX.x[flag[2]]-origX.x[(flag[2]+1)%Nverts])*
				 (origX.x[flag[2]]-origX.x[(flag[2]+1)%Nverts]) +
			     (origX.y[flag[2]]-origX.y[(flag[2]+1)%Nverts])*
				 (origX.y[flag[2]]-origX.y[(flag[2]+1)%Nverts]) >
			     (origX.x[flag[1]]-origX.x[flag[2]])*
				 (origX.x[flag[1]]-origX.x[flag[2]]) +
			     (origX.y[flag[1]]-origX.y[flag[2]])*
				 (origX.y[flag[1]]-origX.y[flag[2]]))
				which_split = 1;
			...
			switch(which_split)
			{
				case 1:
					...
					newX.x[0] = centX.x[flag[1]];
					newX.y[0] = centX.y[flag[1]];
					newX.x[1] = centX.x[flag[2]];
					newX.y[1] = centX.y[flag[2]];
					newX.x[2] = origX.x[flag[1]];
					newX.y[2] = origX.y[flag[1]];
					...
					set_link(EL, EL->nel-1, 1, link_eid[(flag[2]+1)%Nverts],
							 link_face[(flag[2]+1)%Nverts]);
					break;
				default:
					...
					newX.x[1] = centX.x[flag[2]];
					newX.y[1] = centX.y[flag[2]];
					newX.x[2] = origX.x[(flag[2]+1)%Nverts];
					newX.y[2] = origX.y[(flag[2]+1)%Nverts];
		...
		default:
			...
			if(flag[0]!=3)
			{ //if flag[0]=3, we want no side splitting
				for(i=0;i<Nverts;++i)
					if(link_eid[i] != -1)
					{
						foo1 = EL->flist[link_eid[i]]->edge[link_face[i]].l;
						foo2 = EL->flist[link_eid[i]]->edge[(link_face[i]+1)%
											  EL->flist[link_eid[i]]->Nedges].l;
						foo3 = EL->flist[link_eid[i]]->edge[(link_face[i]+2)%
											  EL->flist[link_eid[i]]->Nedges].l;
						EL->flist[link_eid[i]]->split_edge(link_face[i], EL,
													 		Ubc, nfields);
						...
						//foo3 is used to hold the minimum
						EL->flist[EL->nel-1]->edge[2].l = foo3;   
...
void Tet::close_split(Element_List *, Bndry **, int, int *&)
{
	fprintf(stderr, "Hex::split_element not implemented yet\n");
	return;
}
...
void Pyr::close_split(Element_List *, Bndry **, int, int *&)
{
	fprintf(stderr, "Hex::split_element not implemented yet\n");
	return;
}
...
void Prism::close_split(Element_List *, Bndry **, int, int *&)
{
	fprintf(stderr, "Hex::split_element not implemented yet\n");
	return;
}
...
void Hex::close_split(Element_List *, Bndry **, int, int *&)
{
	fprintf(stderr, "Hex::split_element not implemented yet\n");
	return;
}
...
void Element::close_split  (Element_List *, Bndry **, int, int *&)
{
	ERR;
}
...
void Tri::delete_element(Element_List *EL)
{
	Element *E;
...
void Quad::delete_element(Element_List *)
{
...
void Tet::delete_element(Element_List *)
{
...
void Pyr::delete_element(Element_List *)
{
...
void Prism::delete_element(Element_List *)
{
...
void Hex::delete_element(Element_List *)
{
...
void Element::delete_element(Element_List *)
{
	ERR;
}
