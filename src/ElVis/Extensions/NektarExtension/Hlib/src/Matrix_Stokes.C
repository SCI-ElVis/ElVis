                   ...
                   Bsystem *Ubsys, Bsystem *Pbsys, Metric *lambda)
{
	...
	for(E = U->fhead; E;E=E->next)
	{
		        ...
		        !Ubsys->Gmat->invc[E->geom->id])
		{
			...
			{ /* multiply by kinvis */
				dscal(helm->asize*helm->asize,Ubsys->lambda[E->id].d,
						*helm->a,1);

				if(helm->csize)
				{
					dscal(helm->asize*helm->csize,Ubsys->lambda[E->id].d,
							*helm->b,1);
					dscal(helm->csize*helm->csize,Ubsys->lambda[E->id].d,
							*helm->c,1);
			...
			E->condense_stokes(helm,div,Ubsys,Pbsys);
		}

		if(Ubsys->smeth == direct || Pbsys->rslv)
		{
			E->project_stokes(Pbsys,eDIM*div->bsize+1,
								P->flist[E->id]->geom->id);
		}
	...
	{ /* recursive solver setup */
		...
		for(i = 0; i < Pbsys->rslv->nrecur-1; ++i)
		{
			...
			for(j = 0; j < rdata->npatch; ++j)
				Project_Recur(j,rdata->patchlen_a[j],a[j],rdata->map[j],i+1,
							  Pbsys);
	...
	if(nsolve)
	{
		if(bsys->lambda->wave)
		{
			...
			{    /* banded */
				...
				error_msg(banded nonsymmetric solve is not implemented in
						  invert_a);
			}
			else
			{
		...
		else
		{
			...
			else
			{
				...
				bsys->Gmat->pivota = ivector(0,nsolve-1);
				dsptrf('L', nsolve,  *bsys->Gmat->inva, bsys->Gmat->pivota,
						 info);
			}
		}
	}
	...
	if(!Pbsys->rslv)
	{
		if(Pbsys->smeth == direct)
		{
			if(nsolve)
			{
				...
				if(Ubsys->lambda->wave)
				{ /* Oseen Solver */
					if(3*Pbsys->Gmat->bwidth_a  < nsolve)
					{  /* banded solver    */
						Pbsys->Gmat->inva = dmatrix(0,nsolve-1,0,
						                            3*Pbsys->Gmat->bwidth_a-3);
						dzero(nsolve*(3*Pbsys->Gmat->bwidth_a-2),
								*Pbsys->Gmat->inva,1);
					}
					else
					{      /* symmetric solver */
				...
				{
					if(2*Pbsys->Gmat->bwidth_a  < nsolve)
					{  /* banded solver    */
						Pbsys->Gmat->inva = 
								dmatrix(0,nsolve-1,0,Pbsys->Gmat->bwidth_a-1);
						dzero(nsolve*Pbsys->Gmat->bwidth_a,*Pbsys->Gmat->inva,
								1);
					}
					else
					{      /* symmetric solver */
...
static void MemPrecon(Element_List *U, Bsystem *B, Bsystem *BP)
{
	...
	switch(BP->Precon)
	{
		...
		case Pre_Block:
		{
			...
			M->info.block.iedge = (double **)calloc((nes) ? nes : 1,
													sizeof(double *));
		...
		{	
			...
			M->info.blockst.iedge = (double **)calloc((nes) ? nes : 1,
													  sizeof(double *));
				...
				for(d = 0; d < eDIM; ++d)
				{
					...
					for(i = 0; i < E->Nedges; ++i)
					{
						...
						if(gid < nedge)
						{
							...
							if(E->edge[i].con)
								for(j = 0,sign1=1;j<l;
										++j,cnt+=j+d*l,pos+=acnt++,sign1=-sign1)
									for(n = 0,sign=sign1; n < j+1;
											 ++n,sign=-sign)
					...
					/* note this is not set up for variable polynomial order */
							...
							if(E->edge[i].con)
							{
								...
								{
									if(d)
										for(n = 0,sign=sign1; n < l;
												 ++n,sign=-sign)
											s[cnt+n] += sign*A[pos-j-
											 	     (E->Nedges*l+E->Nverts)+n];
									for(n = 0,sign=sign1; n < j+1;
											 ++n,sign=-sign)
							...
							else
							{
								for(j = 0; j < l;++j,cnt+=j+d*l,pos+=acnt++)
								{
									if(d)
										dvadd(l,
											  A+pos-j-(E->Nedges*l+E->Nverts),1,
											  s+cnt,1,s+cnt,1);
									
									dvadd(j+1,A+pos-j,1,s+cnt+d*l,1,
										  s+cnt+d*l,1);
								}
							}
#endif
...
static void InvtPrecon(Element_List *U, Bsystem *B)
{
	switch(B->Precon)
	{
		...
		{			
			...
			if(info)
				fprintf(stderr,"Error inverting matrix in InvtPrecon "
						"(stokes)\n");
#else
		...
		{	
			...
			for(cnt = 0,k = 0; k < nel; ++k)
			{
				...
				for(i = 0; i < nedge; ++i)
				{
					...
					dpptrs('U',l,1,B->Pmat->info.blockst.iedge[i],tmp+cnt1,l,
							info);
					cnt1 += l;
				}
                              ...
                              Bsystem *B)
{
	...
	if(info)
		error_msg("error inverting C matrix");
...
void Element::DivMat  (LocMatDiv *, Element *)
{
	ERR;
}
...
void Tri::project_stokes(Bsystem *Pbsys, int asize, int geom_id)
{
	...
	if(Pbsys->rslv)
	{ /* project a to first recurrsion */
		Project_Recur(id, asize, a, list, 0, Pbsys);
		free(a);
	}
	else
	{           /* normal static condenstion */
		if((nsolve = Pbsys->nsolve) != 0)
		{
				...
				else
				{
					...
					for(i = 0; i < asize; ++i)
						if((pi=list[i]) < nsolve)
							for(j  = 0; j < asize; ++j)
								if((pj=list[j]) < nsolve)
									inva[0][pj*nsolve+pi]+=a[i*asize+j];
									/*pack in fortran form*/
				}
			}
			else
			{
...
void Element::project_stokes(Bsystem *, int, int)
{
	ERR;
}
                          ...
                          Bsystem *Pbsys)
{
	...
	for(i = 0,n=0; i < D->bsize; ++i)
		for(j = 0; j < csize; ++j)
			binvc[n++] = dxb[j+1][i] -
			             ddot(m->csize,Ubsys->Gmat->binvc[geom_id]+i*m->csize,1,
							  dxi[j+1],1);
	...
	if(Ubsys->lambda->wave)
	{
		...
		for(i = 0,n=0; i < D->bsize; ++i)
			for(j = 0; j < csize; ++j)
				invcd[n++] = dxb[j+1][i] -
				             ddot(m->csize,dxi[j+1],1,
								  Ubsys->Gmat->invcd[geom_id]+i*m->csize,1);

		for(i = 0; i < D->bsize; ++i)
			for(j = 0; j < csize; ++j)
				invcd[n++] = dyb[j+1][i] -
				             ddot(m->csize,dyi[j+1],1,
								  Ubsys->Gmat->invcd[geom_id]+i*m->csize,1);
	...
	if(Ubsys->lambda->wave)
	{
		...
		for(i = 0; i < D->bsize; ++i)
		{
			...
			atot[(i+1)*asize-1] = dxb[0][i] -
			                      ddot(m->csize,
							 Ubsys->Gmat->binvc[geom_id]+i*m->csize,1,dxi[0],1);
			
			atot[(asize-1)*asize+i] =  dxb[0][i] -
			                           ddot(m->csize,dxi[0],1,
									Ubsys->Gmat->invcd[geom_id] + i*m->csize,1);
			...
			atot[(D->bsize+i+1)*asize-1] = dyb[0][i] -
			            ddot(m->csize,
							 Ubsys->Gmat->binvc[geom_id]+i*m->csize,1,dyi[0],1);
			
			atot[(asize-1)*asize+D->bsize+i] =  dyb[0][i] -
			                   					ddot(m->csize,dyi[0],1,
									Ubsys->Gmat->invcd[geom_id] + i*m->csize,1);
		}
	}
	else
	{
		...
		for(i = 0; i < D->bsize; ++i)
		{
			...
			atot[(i+1)*asize-1] = atot[(asize-1)*asize+i] =  dxb[0][i] -
			                      ddot(m->csize,
									   Ubsys->Gmat->binvc[geom_id]+i*m->csize,1,
									   dxi[0],1);

			dcopy(D->bsize,a[i],1,atot + (D->bsize+i)*asize + D->bsize,1);
			atot[(D->bsize+i+1)*asize-1] = atot[(asize-1)*asize+D->bsize+i] = 
					dyb[0][i] - ddot(m->csize,
									 Ubsys->Gmat->binvc[geom_id]+i*m->csize, 1,
									 dyi[0],1);
		}
	}
