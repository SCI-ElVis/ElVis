                        ...
                        double *hr, double *hs, double *ht)
{
	switch (E->identify())
	{
		...
		case Nek_Hex:
			...
			break;
		case Nek_Pyr:
			fprintf(stderr,"get_point_shape_3d not implemented for Pyramids\n");
			break;
		default:
			fprintf(stderr,"Non-3d element type passed to "
						   "get_point_shape_3d\n");
			break;
	}
}
                        ...
                        double *hr, double *hs)
{
	switch (E->identify())
	{
		...
		case Nek_Quad:
			...
			break;
		default:
			fprintf(stderr,"Non-2d element type passed to "
						   "get_point_shape_2d\n");
			break;
	}
}
...
void get_point_shape(Element *E, Coord A, double **h)
{
	switch (E->identify())
	{
		...
		case Nek_Hex:
			...
			break;
		default:
			fprintf(stderr,"Element type not handled in get_point_shape\n");
			break;
	}
}


double eval_field_at_pt_2d(int qa, int qb, double *field, double *hr,
						   double *hs)
{
	int i;
	double d=0.;
...
double eval_field_at_pt_3d(int qa, int qb, int qc, double *field, double *hr,
						   double *hs, double *ht)
{
	int i,j;
	double d=0.;
                   ...
                   double *a, double *b, char trip)
{
	...
	else
	{
		...
		{// straight sided so use analytic form
			Vert *v[3];
			...
			aa = (fabs(1.-ss) > FPTOL)? (2.*(1.+rr)/(1.-ss))-1. :
										(fabs(1.+rr) > FPTOL) ? 2 : rr;
			bb = ss;
			fl = 1;
		}
	}
                   ...
                   double *a, double *b, double *c, char trip)
{
    ...
    case Nek_Tet:
        ...
        if(E->curvX)
        {
            ...
            while (!fl && (ic++ <= MAXIT))
            {
                ...
                aa = (fabs(ss+tt) > FPTOL) ? (-2.*(1.+rr)/(ss+tt)) - 1.:0.0;
                bb = (fabs(1.-tt) > FPTOL) ? ( 2.*(1.+ss)/(1.-tt)) - 1.:0.0;
                cc = tt;
        ...
        { // use analytic mapping
            ...
            aa = (fabs(ss+tt) > FPTOL)? (-2.*(1.+rr)/(ss+tt)) - 1.: rr;
            bb = (fabs(1.-tt) > FPTOL)? ( 2.*(1.+ss)/(1.-tt)) - 1.: ss;
            cc = tt;
    ...
    case Nek_Prism:
        ...
        while (!fl && (ic++ <= MAXIT))
        {
            ...
            aa = (fabs(1.-tt) > FPTOL)?( 2.*(1.+rr)/(1.-tt)) - 1.:0.0;
            bb = ss;
            cc = tt;
    ...
    default:
        fprintf(stderr,"Find_coord_3d: got non 3d element.\n");
        break;
...
int find_elmt_coords(Element_List *U, Coord X,int *eid, Coord *A, char trip)
{
    if(U->fhead->dim() == 2)
        ...
        return Find_elmt_coords_3d(U,*(X.x),*(X.y),*(X.z),eid,
                                   A->x,A->y,A->z,trip);
}
...
static int Find_elmt_coords_3d(Element_List *U, double xo, double yo, double zo,
                               int *eid, double *a, double *b, double *c,
                               char trip)
{
...
void Find_local_coords(Element_List *U, Coord *X, int Npts,
                       int **Eids, Coord **A, bool SetUpMem)
{
	...
	if(U[0]->fhead->dim() == 2)
	{
		for(k=0;k<Npts;++k)
		{
			if(Eids[k] != -1)
			{
				...
				for(j=0;j<Nfields;++j)
					(*data)[j][k] =
					    eval_field_at_pt_2d(E->qa,E->qb,
											U[j]->flist[E->id]->h[0], hr, hs);
			}
			else
			{
#if 1
			...
			if(Eids[k] != -1)
			{
				...
				for(j=0;j<Nfields;++j)
					(*data)[j][k] = eval_field_at_pt_3d(E->qa,E->qb,E->qc,
									 U[j]->flist[E->id]->h_3d[0][0],hr, hs, ht);
			}
			else
			{
				for(j=0;j<Nfields;++j)
