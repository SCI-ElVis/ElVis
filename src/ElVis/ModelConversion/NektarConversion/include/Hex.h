...
class Hex: public Element
{
	...
	public:
		...
		// Error functions
		void Set_field(const char *sHexng);           	// set field to function
		void Error(const char *sHexng);                   // compare with function
		double L2_error_elmt(const char *sHexng);
		double H1_error_elmt(const char *sHexng);
		void Verror(double *u, const char *sHexng);
		double Int_error(const char *sHexng);
		double Norm_li();
		double Norm_l2();
