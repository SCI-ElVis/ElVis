...
namespace MatSolve
{
	...
	The static condensation routine can also be applied in a multi-level
	approach. 
	...
	Which are  stored in  \e column major form, whilst the matrix 
	 - \a D is [m x n] 
	 is stored in \e row major form so that \f$D^T = C\f$ if the matrix is
	 symmetric
	\n
	*/
	class SCmat
	{
		...
		public:
			...
			/// Enforce a signchange on the entries of A depending on the value
			/// of sc
