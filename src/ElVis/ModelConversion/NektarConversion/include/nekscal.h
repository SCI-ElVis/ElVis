...
typedef struct dom
{
	int      nfields;     /* Number of fields to solve */
	char     *name;       /* name of run        */
	char     **soln;      /* solution           */
	char     fieldtypes[MAXFIELDS];  /* type flag for each field */
	...
	Bsystem *Vsys, *Wxsys, *Xsys, *Zsys, *Ysys, *Psys;
...
void      ReadFieldTypes(FILE *fp, Domain *omega);
Element_List  *ReadMesh      (FILE *,char*);
void      ReadICs       (FILE *, Domain *omega , Element_List *Mesh);
void      ReadAdv       (FILE *fp, Domain *omega, Element_List *Mesh);
void      summary       (void);
void      ReadSetLink   (FILE *fp, Element_List *U);
Bndry    *ReadBCs       (FILE *fp, Element *U, AdvVel *vel = (AdvVel *) NULL);
Bndry_List *ReadBCs_nlevels (int nfields, FILE *fp, Element *U, AdvVel *vel = (AdvVel *) NULL);
...
void      ReadDF(FILE *fp, int nforces, Element_List &U, Domain *omega);
...
void	  ReadReaction (FILE *fp, Domain *omega, Element_List *Mesh);



/* functions in convective.c */
void CdgradV(Domain *omega);
double full_cfl_checker(Element *U, Element *V, double dt, int& eid_max);
double full_cfl_checker_3D(Element *U, Element *V, Element *W,  double dt, int& eid_max);
