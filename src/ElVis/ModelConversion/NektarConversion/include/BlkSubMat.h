	...
	class BlkSubMat
	{
		public:
			...
			void AddMat(const int rows, const int cols, const double *mat)
			{
				...
				if((_rows != rows) || (_cols != cols))
					NekError::error(warning,"BlVeck::AddMat",
					             "Adding sub-matrices of different dimensions");
			           ...
			           const double& beta, double *y)
			{
				...
				{
					case RowMajor:
						blas::dgemv('T',_cols,_rows,alpha,_mat,_cols,v,1,beta,y,
						            1);
						break;
					case ColMajor:
						blas::dgemv('N',_cols,_rows,alpha,_mat,_cols,v,1,beta,y,
						            1);
						break;
					default:
			           ...
			           const BlkSubMat& A,const BlkSubMat& B,const double& beta)
			{
				...
				switch(formB)
				{
					case RowMajor:
						...
						{
							case RowMajor:
								blas::Cdgemm(A._rows,B._cols,A._cols,alpha,
								             A._mat,A._cols,B._mat,B._cols,beta,
								             _mat,_cols);
								break;
							case ColMajor:
								blas::dgemm('N','T',B._cols,A._cols,A._rows,
								            alpha,B._mat,B._cols,A._mat,A._cols,
								            beta,_mat,_cols);
								break;
							default:
					...
					case ColMajor:
						...
						{
							case RowMajor:
								blas::dgemm('T','N',B._rows,A._rows,A._cols,
								            alpha,B._mat,B._cols,A._mat,A._cols,
								            beta,_mat,_cols);
								break;
							case ColMajor:
								blas::dgemm('N','N',A._rows,B._cols,A._cols,
								            alpha,A._mat,A._cols,B._mat,B._cols,
								            beta,_mat,_cols);
								break;
							default:
								NekError::error(warning, "BlkMat::geMxM",
			...
			void invert(void)
			{
				...
				if(_rows != _cols)
					NekError::error(fatal,"BlkSubMat::invert()",
					                "Matrix not square");
				...
				lapack::dgetrf(_rows,_cols,_mat,_cols,ipiv,info);
				assert(info==0);
				lapack::dgetri(_rows,_mat,_cols,ipiv,Wk,_rows*_cols,info);
				assert(info==0);
			...
			{
				if(_rows != _cols)
					NekError::error(fatal,"BlkSubMat::rescale",
