...
namespace MatSolve
{
	...
	class DGMatrix
	{
		public:
			...
			void GenMat (Element_List *U, Element_List *Uf, Bndry *Ubc,
						 Bsystem *Ubsys, double lambda);
			void Solve (Element_List *U, Element_List *Uf, Bndry *Ubc,
						Bsystem *Ubsys);
			void SolveSC (double *f);

			void HelmHoltz(Element_List *U, Element_List *Uf);
			void Project_Helm(BlkMat& Helm, SClevel& SClev,
							  const Element_List& U);
		...
		private:
			...
			SClevel *SClev;
			Gmat    *Ainv;
			void set_gmap(const Element_List *U, const Bndry *Ubc,
						  const Bsystem *Ubsys);
	};
} // end of namespace
#endif
