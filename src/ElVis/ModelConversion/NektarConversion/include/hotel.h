...
#define _MAX_FIELDS    MAXFIELDS   // Maximum fields in a solution file
...
/* gettimeofday(&tvc,NULL); \
 fprintf(stdout,"%s Took %7ld microseconds\n",s, \
 				(tvc.tv_usec-tvp.tv_usec)+(tvc.tv_sec-tvp.tv_sec)*1000000); \
 fflush(stdout); \
...
// macros put into ifndef statement because incompatibility with std::max
// and std::min when using gcc 3.4.1
#ifndef _ALGOBASE_H
...
void  Add_robin_matrix(Element *E, LocMat *helm, Bndry *rbc, AdvVel *vel,Metric *lambda);
...
void       free_dtarray(double ***m3, int nwl, int nrl);
...
Element_List *LocalMesh(Element_List *EL, const char *name, int nlevels=1, int nz=1);
Element_List *LocalMesh(Element_List *EL, Bndry *Bc, const char *name,
                        int nlevels=1, int nz=1);
...
extern "C"
{
	...
	void   vector_def    (const char *vlist, const char *function);
	void   vector_set    (const int   vsize, ... /* v1, v2, ..., f(vn) */ );
	double scalar        (const char *function);
	double dparam_set    (const char *name, const double value);
	double dparam        (const char *name);
	int    option_set    (const char *name, const int value);
	int    option        (const char *name);
	int    iparam_set    (const char *name, const int value);
	int    iparam        (const char *name);
}

/* functions in Fieldfiles.C */
void  Writefield  (FILE *fp, const char *name, int step, double t,
...
Field *readFieldFiles(int nfiles, const char *name, Element_List *Mesh);
...
double FullEigenVec(double **a, double *v, int n, int lda);
...
void genRecon   (Curve *curve, double *va, double *vb, double *vc, double *x, double *y, double *z, int qa, int qb);
...
void Load_Felisa_Surface(const char *name);
void Free_Felisa_data   (void);


//Functions in HOSurf.C
void Load_HO_Surface (const char *name);
