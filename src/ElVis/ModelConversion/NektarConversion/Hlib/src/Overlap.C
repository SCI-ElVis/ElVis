...
static void APatch(Element_List *U, Element_List *Uf, Bsystem *B, double *p,
				   double *w, int patchid);
static void GathrBndryPatch(Element_List *U, double *u, Bsystem *B, 
							int patchid);
static void GenPatches(Element_List *U, Element_List *Uf, Bsystem *B);
                       ...
                       Bsystem *B)
{
					...
					for(i=0;i<OL->npatches-1;++i)
					{
						for(k=0;k<U->nel;++k)
						{
							...
							for(j=0;j<E->Nverts;++j)
								...
								else if(E->vert[j].gid ==
									    i+B->nsolve-B->nv_solve)
								{
									OL->maskflags[i][E->id] = 1;
								}
						}
	...
	for(i=0;i<OL->npatches;++i)
	{
		for(j=0;j<B->nsolve;++j)
			if(fabs(OL->solveflags[i][j]) > FPTOL)
				OL->patchex[j] = 0.;
	}
#if 1
	for(j=0;j<B->nsolve;++j)
		if(fabs(OL->patchex[j]) > FPTOL)
			for(k=0;k<U->nel;++k)
                    ...
                    Bsystem *B, double *pin, double *zout)
{
	...
	for(patchid=0;patchid<OL->npatches;++patchid)
	{
		...
		//    if(tol > dparam("TOLCG")){
		{
			...
			{
				printf("\tpatchid: %d Field %c: %3d iterations, error = "
					   "%#14.6g %lg %lg\n",patchid, U->fhead->type, iter,
						eps/epsfac, epsfac, tolcg);
...
static void GathrBndryPatch(Element_List *U, double *u, Bsystem *B, int patchid)
{
	register int i,k;
	int      *bmap, Nbmodes;
	double   *s;
			...
			if(OL->maskflags[patchid][k] == 0)
				if(dmin( Uf->flist[k]->Nbmodes, Uf->flist[k]->vert[0].hj, 1)!=0
			     || dmax(Uf->flist[k]->Nbmodes, Uf->flist[k]->vert[0].hj, 1)!=0)
					fprintf(stdout, "Non-zero after scatr in out-of-patch "
							"elmt\n");
		}
#endif
...
void Acolumn(Element_List *U, Element_List *Uf, Bsystem *B, int colid,
			 double *col)
{
	...
	for(j=0;j<OL->npatches;++j)
	{
		for(k=0;k<OL->patchlengths[j];++k)
			for(i=0;i<OL->patchlengths[j];++i)
				if(fabs(A[j][k][OL->patchlengths[j]-i-1]) > FPTOL)
				{
					OL->patchbw[j] = max(OL->patchlengths[j]-i-1-k,
										 OL->patchbw[j]);
					break;
				}
		OL->patchbw[j] +=1;
...
void PreconDirectOverlap(Bsystem *B, double *pin, double *zout)
{
	...
	for(i=0;i<OL->npatches;++i)
	{
		...
		if(L)
		{
			...
			if(L>2*OL->patchbw[i])
				dpbtrs('L',L,OL->patchbw[i]-1,1,OL->patchinvA[i],OL->patchbw[i],
					   tmp,L,info);
			else
