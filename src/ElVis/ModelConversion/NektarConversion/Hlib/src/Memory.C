...
void Tri::Mem_J(int *list, char )
{
	...
	for(i = 0; i < Nedges; ++i)
		lmax = max(lmax,edge[i].l+2);
	for(i = 0; i < Nfaces; ++i)
		if(face[i].l)
			lmax = max(lmax,face[i].l+3);
...
void Tri::Mem_shift(double *new_h, double *new_hj)
{
...
void Quad::Mem_shift(double *new_h, double *new_hj)
{
...
void Tet::Mem_shift(double *new_h, double *new_hj)
{
	...
	for(i = 0; i < Nfaces; ++i)
		if ((ll=face[i].l) != 0)
		{
			...
			face[i].hj = (double **)malloc(sizeof(double *));
...
void Pyr::Mem_shift(double *new_h, double *new_hj)
{
...
void Prism::Mem_shift(double *new_h, double *new_hj)
{
	...
	// interior modes
	if(interior_l>1)
	{ /* when interior_l==1 the triangular face
...
void Hex::Mem_shift(double *new_h, double *new_hj)
{
...
void Element::Mem_shift(double *, double *)
{
	ERR;
}
...
LocMatDiv *Element::divmat_mem(Element *)
{
	ERR;
	return NULL;
}
...
void Element::divmat_free(LocMatDiv *)
{
	ERR;
}
...
void Element::MemBndry(Bndry *, int , int )
{
	ERR;
}
