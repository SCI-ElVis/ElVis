                              ...
                              double *invnorm)
{
	...
	for(i = 0; i < q; ++i)
		dvmul(q, w, 1, h+i*q,1, wk+i*q,1);
	for(i = 0; i < q; ++i)
		dvmul(q, w, 1,  wk+i,q,   wk+i,q);
...
// assumes 'a'x'b'
// interior modes..
void tri_normalised_iprod_2d(int qa, char dira, char dirb,  int qb, double *h,
				 int L, double *ma, double *mb, double *res, double *invnorm)
{
	int i;
	...
	getzw(qa, &z, &w, dira);
	for(i = 0; i < qb; ++i)
		dvmul(qa,    w,1, h+i*qa, 1, wk+i*qa, 1);
	getzw(qb, &z, &w, dirb);
	for(i = 0; i < qb; ++i)
		dscal(qa, w[i], wk+i*qa, 1);
...
// At this moment, this function is never used. If you modify the code and
// include a call to this function, please remove this comment.
...
namespace
{
	...
	// Use this to free memory allocated to BasisB_G, BasisB_GR,
	// BasisC_GR, BasisC_G
...
Basis *Tri_addbase(int L, int qa, int qb, int qc)
{
	...
	if(option("Normalise"))	/* normalise basis so that L2 of each mode = 1 */
		Tri_normalise(b,qa,qb,qc); 
...
Basis *Hex_mem_base(int L, int qa, int qb, int qc)
{
	...
	b->vert = m = mvector(0,NHex_verts + NHex_edges*(L-2) + 
							NHex_faces*(L-2)*(L-2) + (L-2)*(L-2)*(L-2)-1);
...
void Hex_set_faces(Basis *, int, char dir)
{
	switch (dir)
	{
		case 'a':
...
static void Tet_normalise(Basis *, int , int , int )
{
