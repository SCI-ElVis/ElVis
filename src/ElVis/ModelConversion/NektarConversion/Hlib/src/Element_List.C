...
#define EFGloop(E,F,G) for(el=E, fl=F, gl=G; el&&fl&&gl; \
						   el=el->next,fl=fl->next,gl=gl->next)
#define EFGHloop(E,F,G,H) for(el=E,fl=F,gl=G,hl=H;el&&fl&&gl&&hl; \
							  el=el->next,fl=fl->next,gl=gl->next,hl=hl->next)
...
Element_List::Element_List(Element **hea, int n)
{
	...
	nztot   = 1;
	nlevels = 1;
	levels  = (Element_List**) malloc(nlevels*sizeof(Element_List*));
	levels[0] = this;
	flevels   = (Element_List**) malloc(nz*sizeof(Element_List*));
	flevels[0] = this;
}
...
void Element_List::Cat_mem()
{
	...
	if(base_h)
		free(base_h);
	base_h  = dvector(0, nlevels*nz*htot-1);
	if(base_hj)
		free(base_hj);
	base_hj = dvector(0, nlevels*nz*hjtot-1);

	double *tmp_base_h  = base_h;
	double *tmp_base_hj = base_hj;

	dzero(nlevels*nz*htot,  base_h, 1);
	dzero(nlevels*nz*hjtot, base_hj, 1);

	Eloop(fhead)
	{
		el->Mem_shift(tmp_base_h, tmp_base_hj);
		tmp_base_h  += el->qtot;
		tmp_base_hj += el->Nmodes;
	}

	// Not sure if necessary! If so, take the comments out and update cvs
	/*  for(int i = 0; i < nlevels; ++i)
	  {
	      for(int j = 0; j < nz; ++j)
	      {
	          levels[i]->flevels[j]->Mem_shift(tmp_base_h,tmp_base_hj);
	          tmp_base_h  += htot;
	          tmp_base_hj += hjtot;
	          levels[i]->flevels[j]->htot  = htot;
	          levels[i]->flevels[j]->hjtot = hjtot;
	      }
	  }*/
}
                        ...
                        Basis *ba, Basis *bb)
{
	...
	if(!Tri_mapping)
		Tri_mapping = Tri_get_global_mapping(E);
	...
	for(k = 0; k < nel; ++k)
	{
		...
		if(E->flist[k]->curvX)
			dvmul(qa*qb,E->flist[k]->geom->jac.p,1,*E->flist[k]->h,1,
				  *E->flist[k]->h,1);
		else
                         ...
                         Basis *ba, Basis *bb)
{
	...
	if(!Quad_mapping)
		Quad_mapping = Quad_get_global_mapping(E);
...
void Element_List::Iprod(Element_List *EL)
{
	...
	if(!option("variable")&&(fhead->dim() == 2))
...
void Element_List::Grad_d(double *AL, double *BL, double *CL, char)
{
	Element *A;
	double  *nul = (double*)0;
...
void Element_List::Grad_h(double *HL, double *AL, double *BL, double *CL,
						  char)
{
	Element *A;
	double  *nul = (double*)0;
	...
	if(AL && BL && CL)
		for(A=fhead; A; AL+=A->qtot, BL+=A->qtot, CL+= A->qtot, HL += A->qtot,
					    A=A->next)
...
void Element_List::GradT_h(double *HL, double *AL, double *BL, double *CL,
						   char Trip)
{
...
void Element_List::Set_field(const char *string)
{
	Eloop(fhead)
            el->Set_field(string);
}
...
void Element_List::reset_type(const char *types)
{
    int i;

    for(i = 0; i < nlevels; ++i)
    {
        Eloop(levels[i]->fhead)
            el->type = types[i];
    }
}

Element_List* Element_List::gen_aux_field_nlevels(char ty)
{
    return gen_aux_field_nz(ty);
}

Element_List* Element_List::gen_aux_field_nz(char ty)
{
    int i,j;
    Element_List *ee = (Element_List*) new Element_List();

    ee          = gen_aux_field(ty,false);
    ee->nlevels =  nlevels;
    ee->levels  = (Element_List**) malloc(nlevels*sizeof(Element_List*));
    ee->htot    = htot;
    ee->hjtot   = hjtot;
    ee->levels[0] = ee;

    //fill out Fourier levels
    ee->levels[0]->flevels  = (Element_List**) malloc(nz*sizeof(Element_List*));
    ee->levels[0]->flevels[0] = ee->levels[0];

    for(i = 1; i < nz; ++i)
    {
        ee->levels[0]->flevels[i] = gen_aux_field(ty,false);
    }

    // do other levels
    for(j = 1; j < nlevels; ++j)
    {
        ee->levels[j]        = gen_aux_field(ty,false);

        //fill out Fourier levels
        ee->levels[j]->flevels  = (Element_List**)
            malloc(nz*sizeof(Element_List*));
        ee->levels[j]->flevels[0] = ee->levels[j];
        for(i = 1; i < nz; ++i)
        {
            ee->levels[j]->flevels[i] = gen_aux_field(ty,false);
            ee->levels[j]->flevels[i]->htot  = htot;
            ee->levels[j]->flevels[i]->hjtot = hjtot;
        }
    }

    ee->Cat_mem();

    return (Element_List*) ee;
}

Element_List* Element_List::gen_aux_field(char ty, bool allocate_mem)
{
    int           eid, id, i,k;
    Edge         *edg;
    ...
    for(i=0;i<nel;++i)
    {
        ...
        case Nek_Pyr:
            ...
            break;
        case Nek_Element:
            fprintf(stderr,"Element_List::gen_aux_field copying from "
                    "Element\n");
            break;
        default:
            fprintf(stderr,"Element_List::gen_aux_field: incorrect element "
                    "type\n");
            break;
        }
        new_E[i]->type = ty;
    ...
    for(k = 0; k < nel; ++k)
    {
        ...
        if(new_E[0]->dim() == 2)
            ...
            for(i = 0; i < new_E[k]->Nedges; ++i)
                if((!new_E[k]->edge[i].base)&&flist[k]->edge[i].base)
                {
                    ...
                    {
                        new_E[edg->eid]->edge[edg->id].base =
                            new_E[eid]->edge+id;
        ...
        for(i = 0; i < new_E[k]->Nfaces; ++i)
            if(!new_E[k]->face[i].link&&flist[k]->face[i].link)
            {
                ...
                { // if link back does not exist then treat as dummy face
                    new_E[k  ]->face[i ].link = (Face *) calloc(1,sizeof(Face));
                    memcpy(new_E[k]->face[i].link,flist[k]->face[i].link,
                           sizeof(Face));
                }
            }
    }
    ...
    EL->nz      = nz;
    EL->nztot   = nztot;
    EL->levels  = (Element_List**) NULL;
    EL->flevels  = (Element_List**) NULL;

    if(allocate_mem)
    {
        EL->levels    = (Element_List**) malloc(sizeof(Element_List*));
        EL->levels[0] = EL;
        EL->flevels  = (Element_List**) malloc(sizeof(Element_List*));
        EL->levels[0]->flevels[0] = EL->levels[0];
        EL->Cat_mem();
    }
...
void Element_List::Terror(const char *string)
{
...
void DirBCs(Element_List *U, Bndry *Ubc, Bsystem *Ubsys, SolveType Stype)
{
    ...
    if(Ubsys->smeth == direct)
    {
        ...
        for(k = 0; k < Ubsys->nel; ++k)
        {
            ...
            if(N)
                dgemv('T',N, nbl, -1., binvc[U->flist[k]->geom->id], N,
...
void DirBCs_Stokes(Element_List **V, Bndry **Vbc, Bsystem **Vbsys)
{
    ...
    if(Vbsys[0]->smeth == direct)
    {
        ...
        {
            V[2]->Grad(0,V[0],0,'z');
            dvadd(V[2]->htot,V[eDIM]->base_h,1,V[0]->base_h,1,
                  V[eDIM]->base_h,1);
        }
                ...
                if(N)
                {
                    ...
                    if(B->lambda->wave)
                    {
                        ...
                        dgetrs('N',N,1,invc[E->geom->id],N,ipiv[E->geom->id],
                               tmp,N,info);
                        dgemv('T', N, E1->Nmodes, -1., dbinvc[i][E->geom->id],
                              N, tmp, 1, 1., E1->vert->hj,1);
                    }
                    else
                    {
                        dgemv('T', N, E1->Nmodes, -1., dbinvc[i][E->geom->id],
                              N, E->vert->hj+nbl, 1, 1., E1->vert->hj,1);
                    }
                }
            }
        }
...
{
    fprintf(stderr, "Element_List::Grad_z  -- not compiled for fourier "
            "modes\n");
}
...
void Element_List::H_Refine(int * to_split, int nfields, Bndry **Ubc, int *flag)
{
    ...
    for(i=0; i < orignel; i++)
    {
        if(to_split[i])
        {
            ...
            //upon returning, edges_to_split contains the number of unconnected
            //element and their element ids
...
void set_pllinfo(Element_List *EL, const char *name);
void set_pBCinfo(Element_List *EL, Bndry *Ubc);
void set_Pllinfo(Element_List *EL, Bndry *Ubc, const char *name);


static Element_List *Part_elmt_list(Element_List *EL, int nlevels, int nz);

/* given an element E this routine returns a sorted element list
   with a reduced number of elements in the parallel version */

Element_List *LocalMesh(Element_List *EL, const char *name, int nlevels, int nz)
{
    Element_List *new_EL;
    ...
    new_EL = Part_elmt_list(EL,nlevels, nz);
...
Element_List *LocalMesh(Element_List *EL, Bndry *Ubc, const char *name,
                        int nlevels, int nz)
{
    Element_List *new_EL;
...
void set_con_info(Element_List *U, int *partition)
{
...
void set_pllinfo(Element_List *EL, const char *name)
{
    ...
    DO_PARALLEL
    {
        ...
        /* sort element list so that the numbers of global elements are
           consecutive with lowest global id first */
...
static Element_List *Part_elmt_list(Element_List *GEL, int nlevels, int nz)
{
    int           i,k,l;
    ...
    for(i=0;i<lnel;++i)
    {
        ...
        case Nek_Element:
            ...
            break;
        default:
            fprintf(stderr,"Part_elmt_list: incorrect element type\n");
            break;
        }
            ...
            for(i = 0; i < new_E[k]->Nedges; ++i)
            {
                if( (base = GEL->flist[pllinfo.eloop[k]]->edge[i].base) )
                {
                    ...
                    while(base)
                    {
                        ...
                        {
                            new_E[k]->edge[i].base =
                                new_E[inc[base->eid]]->edge + base->id;
                            break;
                        }
                        else
                            base = base->link;
            ...
            for(i = 0; i < new_E[k]->Nfaces; ++i)
                if((face = new_E[k]->face+i)->link)
                {
                    if(inc[face->link->eid]+1)
                        new_E[k]->face[i].link =
                            new_E[inc[face->link->eid]]->face + face->link->id;
                    else
                    {							// reflexive link
                        new_E[k]->face[i].link = new_E[k]->face + i; 
    ...
    EL->nlevels = nlevels;
    EL->levels = (Element_List**) malloc(nlevels*sizeof(Element_List*));
    EL->levels[0]  = EL;
    EL->flevels = (Element_List**) malloc(nz*sizeof(Element_List*));
    EL->flevels[0] = EL;

    // need memory for geometric factors
    EL->nlevels = 1;
    EL->Cat_mem();
    EL->nlevels = nlevels;

    for(E=*new_E;E;E=E->next)
    {
        E->geom = (Geom*)0;
        E->set_geofac();
        if(E->geom->singular)
    ...
    for(k = 1; k < nz; ++k)
    {
        EL->flevels[k] = EL->gen_aux_field(EL->fhead->type,false);
    }
    ...
    if(nlevels > 1)
    {
        for(k = 1; k < nlevels; ++k)
        {
            EL->levels[k] = EL->gen_aux_field(EL->fhead->type,false);
            EL->levels[k]->flevels = (Element_List**)
                malloc(nz*sizeof(Element_List*));
            EL->levels[k]->flevels[0] = EL->levels[k];

            for(l = 1; l < nz; ++l)
            {
                EL->flevels[l] = EL->gen_aux_field(EL->fhead->type,false);
            }
        }
...
void AdvVel :: Get_vel_element(Element *E, double *u, double *v, double *w){
    Element_List *U = advU;
    Element_List *V = advV;
    Element_List *W = advW;
    ...
    if(U->fhead->dim() == 2){
        ...
        dcopy(E->qtot,U->flist[n]->h[0],1,u,1);
        dcopy(E->qtot,V->flist[n]->h[0],1,v,1);
    }
    ...
    if(U->fhead->dim() == 3){
        ...
        dcopy(E->qtot,U->flist[n]->h_3d[0][0],1,u,1);
        dcopy(E->qtot,V->flist[n]->h_3d[0][0],1,v,1);
        dcopy(E->qtot,W->flist[n]->h_3d[0][0],1,w,1);

    }

}

void AdvVel :: Get_inflow_element(Element *E, double *u){
    ...
    /*Select component of the velocity */
    switch (type1)
    {
...
void AdvVel :: Get_inflow_singular_points(Element *E, Bndry  *nbdry){
    ...
    int nvert=E->Nfverts(fac);

    for (i=0; i<nvert; i++){
	nbdry->bvert[i]=U->flist[n]->vert[E->vnum(fac,i)].hj[0];
    }
...
void AdvVel :: Get_inflow_singular_edge(Bndry  *nbdry, double *f){


    register int i;
    Element *E = nbdry->elmt;
    char type1= E->type;
    int n = E->id;
    int fac=nbdry->face;
    Element_List *U;
    ...
    /*Select which component of the velocity we want to read*/
    switch (type1)
    {
  ...
  /*Based on InterpFace1*/
	...
	if(fac > 1)
	{
		...
		for(i=0;i<qc;++i)
		{
			f[qb*qc+i] = ddot(qb,im[qb],1,f+i*qb,1);
		}
	}
...
void React :: Get_rate_element(Element *E, double *u)
{
	Element_List *R = rate;
	...
	if(R->fhead->dim() == 2)
	{
		...
		dcopy(E->qtot,R->flist[E->id]->h[0],1,u,1);
	}
	...
	if(R->fhead->dim() == 3)
	{
		...
		dcopy(E->qtot,R->flist[E->id]->h_3d[0][0],1,u,1);
	}
}
