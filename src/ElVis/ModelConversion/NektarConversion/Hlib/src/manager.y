...
static Symbol  *install(const char*, int, ...),    /* Table management (LOCAL) */
               *lookup (const char*);
static  double 
...
static struct {                  /* Parameters (double) */
  ...
  double  pval;
    ...
    "Womersley",           -0.25,    /* Default flag to identify womersley solution */ 
	 0,                0.
};
...
static struct {                /* Built-ins */
  ...
  PFD      func;               /* Pointer to the function */
     ...
     "shock", 3,  Shock,   /* shock(x,a,b) = a      (if x < 0), 
		 								    (a+b)/2 (if x==0) or b (if x > 0) */
     "jacobi", 4,  Jacobi,
...
%token	<val>	NUMBER
%token	<sym>	VAR BLTIN_UNARY BLTIN_BINARY BLTIN_TRINARY BLTIN_QUATERNARY 
				BLTIN_QUINTIC UNDEF DPARAM IPARAM OPTION
%type	<val>	expr asgn
%right	'='
...
void execerrnr(const char *s, const char *t)   /* run-time error, no recovery */
{
  warning(s,t);
...
double scalar_set (const char *name, const double val)
{
  Node   *np;
  Symbol *sp;
...
int iparam_set (const char *name, const int num)
{
  Node   *np;
  Symbol *sp;
...
int option_set (const char *name, const int status)
{
  Node *np;
...
static double Pow (double x, double y)
{
  ...
  if (yn >= 0 && fabs(yn-y) < FPTOL) { /*Do it inline if y is an integer power*/
      register int n = yn;
      while (n--) 
         px *= x;
  } else  
...
static double Womersley(double A,double B,double r,double R,double mu, 
  ...
  if(fabs(wnum) < FPTOL) /* return poseuille flow  with mean of 1.*/
    return 2*(1-r*r/R/R);
  else{
    int    ierr,nz;
    ...
    static double Jr,Ji,alpha,j0r,j0i;
    

    /* for case of repeated calls to with same parameters look to store 
       parameters independent of r. */
    if((fabs(R-R_str) > FPTOL) || fabs(wnum-wnum_str) > FPTOL ||
	   (fabs(mu-mu_str) > FPTOL))
	{
      ...
      Jr  = 1 + 2*fac/alpha*((rej*j0r+imj*j0i)*isqrt2 -
	  		(imj*j0r - rej*j0i)*isqrt2);
...
static double Womsin(double r, double R, double mu, double wnum, double t){
  if(fabs(wnum) < FPTOL) /* no sin term for zeroth mode */
    return 0;
  else
