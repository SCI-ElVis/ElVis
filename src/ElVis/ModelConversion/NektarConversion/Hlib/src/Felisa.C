...
static void Findmin1D(int np, double *u, double *zw, double **xg,
                      SplineDat *spdat, int eflag);
static void Findmin(int np, double *u, double *v, double *z, SplineDat *spdat,
                    int felsu);
static void Findmin2d(int n1, int n2, double **u, double **v, double *z1,
                      double *z2, SplineDat *spdat, int felsu);
...
void genFelFile(Element *E, double *x, double *y, double *z, Curve *curve)
{
	...
	double **xl,**xg,**u,**v,u1,u2,v1,v2,*za,*zb,*w,*zb1;
	Fcurve  *fc,*fc1;
	Fsurf   *fs;
	...
	if(cnt > 1)
	{
		...
		for(i = 0; i < nfface; ++i)
			...
			{ // just search vertices with same vertex id sum
				...
				{
					if((fvertid[1] == surfids[i][1]) ||
					   (fvertid[1] == surfids[i][2]))
					{
						felsu = surfids[i][3];
						break;
					}
		...
		if(eflag[i]+1)
		{
			...
			switch(i)
			{
				case 0:
					...
					{
						fgcurv(0,cdat[eflag[i]].nu[0],cdat[eflag[i]].r,utmp[j],
							   xg[j],err);
					...
					locuv(qa,*xg,*xl,sdat[felsu].nu[0],sdat[felsu].nu[1],
						  sdat[felsu].r);
				...
				case 1:
					...
					locuv(qb,*xg,*xl,sdat[felsu].nu[0],sdat[felsu].nu[1],
						  sdat[felsu].r);
		...
		else
		{
			...
			switch(i)
			{
				...
				case 1:
					...
					if(nonlinear)
					{
						dcopy(qb,u[0]+qa-1,qa,utmp,1);
						utmp[qb] = u2;
						dcopy(qb,v[0]+qa-1,qa,vtmp,1);
						vtmp[qb] = v2;
				...
				case 2:
					...
					if(nonlinear)
					{
						dcopy(qb,u[0],qa,utmp,1);
						utmp[qb] = u2;
						dcopy(qb,v[0],qa,vtmp,1);
						vtmp[qb] = v2;
						Findmin(qb+1, utmp, vtmp, zb1, sdat, felsu);
                      ...
                      SplineDat *spdat, int eflag)
{
	...
	for(it = 0; it < NITER; ++it)
	{
		...
		for(i = 1; i < np-1; ++i)
		{
			...
			fgcurv(0,spdat[eflag].nu[0],spdat[eflag].r,u[i-1],xg[0],err);
			fgcurv(0,spdat[eflag].nu[0],spdat[eflag].r,u[i+1],xg[2],err);
			...
			for(iu = 0; iu < 3; ++iu)
			{
				...
				fgcurv(0,spdat[eflag].nu[0],spdat[eflag].r,un,xg[1],err);
			...
			// Is the starting point is the minimum point?
                    ...
                    SplineDat *spdat, int felsu)
{
	...
	for(it = 0; it < NITER; ++it)
	{
		...
		{
			// Identify the neighbouring points
			fgsurf(0,spdat[felsu].nu[0],spdat[felsu].nu[1],spdat[felsu].r,
			       u[i-1],v[i-1],x1,err);
			fgsurf(0,spdat[felsu].nu[0],spdat[felsu].nu[1],spdat[felsu].r,
			       u[i+1],v[i+1],x3,err);
			...
			for(iu = 0; iu < 3; ++iu)
			{
				...
				for(iv = 0; iv < 3; ++iv)
				{
					...
					fgsurf(0,spdat[felsu].nu[0],spdat[felsu].nu[1],
						   spdat[felsu].r,un,vn,x2,err);

					if(err)
						alen[iv][iu] = almin;
					else
					{
						len1 = (x2[0]-x1[0])*(x2[0]-x1[0]) + 
							   (x2[1]-x1[1])*(x2[1]-x1[1]) +
						       (x2[2]-x1[2])*(x2[2]-x1[2]);
						len2 = (x2[0]-x3[0])*(x2[0]-x3[0]) + 
							   (x2[1]-x3[1])*(x2[1]-x3[1]) +
						       (x2[2]-x3[2])*(x2[2]-x3[2]);
	...
	for(it = 0; it < NITER; ++it)
	{
		...
		for(i = 1; i < n1-1; ++i)
		{
			for(j = 1; j < n2-1; ++j)
			{
				...
				totlen1 = (x3[0]-x1[0])*(x3[0]-x1[0]) + 
						  (x3[1]-x1[1])*(x3[1]-x1[1]) +
				          (x3[2]-x1[2])*(x3[2]-x1[2]);
				totlen2 = (x5[0]-x4[0])*(x5[0]-x4[0]) +
						  (x5[1]-x4[1])*(x5[1]-x4[1]) +
				          (x5[2]-x4[2])*(x5[2]-x4[2]);
					...
					for(iv = 0; iv < 3; ++iv)
					{
						...
						else
						{
							...
							len2 = (x2[0]-x3[0])*(x2[0]-x3[0]) +
								   (x2[1]-x3[1])*(x2[1]-x3[1]) +
							       (x2[2]-x3[2])*(x2[2]-x3[2]);
							len3 = (x2[0]-x4[0])*(x2[0]-x4[0]) +
								   (x2[1]-x4[1])*(x2[1]-x4[1]) +
							       (x2[2]-x4[2])*(x2[2]-x4[2]);
							len4 = (x2[0]-x5[0])*(x2[0]-x5[0]) + 
								   (x2[1]-x5[1])*(x2[1]-x5[1]) +
							       (x2[2]-x5[2])*(x2[2]-x5[2]);
							pen = len1 + len2 - totlen1 + len3 + len4 - totlen2;

							alen[iv][iu]  = A*len1 + B*len2 + C*len3 + D*len4;
						}
