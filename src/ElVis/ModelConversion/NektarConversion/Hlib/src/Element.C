...
Element::~Element()
{
	...
	// boost::shared_ptr, as multiple elements reference the same block of
	// memory.
	// Angus
	//    double  **h;
	//    double  ***h_3d;
...
void Element::set_curved(Curve*)
{
	ERR;
}              // fix curve sides

void Element::SetGlobalMapInfo(Element *E)		// mapping info
{
	int i;
	for(i = 0; i < Nverts; ++i)
	{
		vert[i].gid   = E->vert[i].gid;
		vert[i].solve = E->vert[i].solve;
	}
	for(i = 0; i < Nedges; ++i)
	{
		edge[i].gid = E->edge[i].gid;
		edge[i].con = E->edge[i].con;
	}
	for(i = 0; i < Nfaces; ++i)
	{
		face[i].gid = E->face[i].gid;
		face[i].con = E->face[i].con;
	}
}
