...
void Element::Trans(Element *, Nek_Trans_Type)
{
	ERR;
...
void Tri::Jbwd(Element *T, Basis *b)
{
	...
	*H = vert[1].hj[0];
	++H;
	memcpy(H,e[1].hj,e[1].l*d);
	H += lmax-2;
	*H = vert[2].hj[0];
	++H;
	*H = vert[0].hj[0];
	++H;
	memcpy(H,e[2].hj,e[2].l*d);
	H += lmax-2;
	...
	dgemv('N',Qb,lmax,1.0,b->vert[2].b,Qb,H,1,0.0,wk,  lmax);
	H+=lmax;
	dgemv('N',Qb,lmax,1.0,b->vert[2].b,Qb,H,1,0.0,wk+1,lmax);
	H+=lmax;
	...
	mxva(b->vert[2].b,1,Qb,H,1,wk  ,lmax,Qb,lmax);
	H+=lmax;
	mxva(b->vert[2].b,1,Qb,H,1,wk+1,lmax,Qb,lmax);
	H+=lmax;
	for(i = 0; i < lmax-2; H+=lmax-2-i, ++i)
...
void Quad::Jbwd(Element *T, Basis *b)
{
	...
	*H = vert[0].hj[0];
	++H;
	memcpy(H,e[0].hj,e[0].l*d);
	H += lmax-2;

	dcopy(e[1].l, e[1].hj, 1, H, lmax);
	++H;
	dcopy(e[3].l, e[3].hj, 1, H, lmax);
	++H;
...
void Tet::Jbwd(Element *E, Basis *b)
{
	...
	{ /* use first line of face 3 to initialise f2 */
		/* face 3 */
		for(i = 0,f2a = f2; i < f[2].l; ++i,f2a += qc)
	...
	{ /* use first line of face 4 to initialise f2 */
		/* face 4 */
		for(i = 0,f2a = f2; i < f[3].l; ++i,f2a += qc)
...
void Hex::Jbwd(Element *E, Basis *B)
{
	...
	for(i = 0; i < lmax; ++i)
		dgemm('N','T',qa,qb,lmax,1.0,(**E->h_3d)+i*qa*lmax,qa,bv->a,qb,0.0,
			  H+i*qa*qb,qa);
...
void Element::Jbwd(Element *, Basis *)
{
	ERR;
}
...
void Tri::Jfwd(Element *E)
{
	...
	if(curvX)
	{
		...
		if(info)
error_msg(Jtransfwd_Loc: info not zero curved);
	...
	else
	{
		...
		if(Tri_compare_L(this))
		{
			...
			if(info)
error_msg(Jtransfwd_Loc: info not zero straight );
...
void Quad::Jfwd(Element *E)
{
	...
	if(info)
error_msg(Jtransfwd_Loc: info not zero);
...
void Tet::Jfwd(Element *E)
{
	...
	dpptrf('L',N,invm,info);
	if(info)
		error_msg(Tet::Jfwd info not zero);
...
void Pyr::Jfwd(Element *E)
{
	...
	if(Pyr_Jfwd_E != this)
	{
		...
		dpptrf('L',N,invm,info);
		if(info)
			error_msg(Pyr::Jfwd info not zero);
...
void Prism::Jfwd(Element *E)
{
	...
	dpptrf('L',N,invm,info);
	if(info)
		error_msg(Prism::Jfwd info not zero);
...
void Hex::Jfwd(Element *E)
{
	...
	dpptrf('L',N,invm,info);
	if(info)
		error_msg(Hex::Jfwd info not zero);
...
void Element::Obwd(double*, double*, int)
{
	ERR;
}
...
void Tri::Ofwd(double *in, double *out, int L, int invjac)
{
	...
	mode = 0;
	for (i= 0; i < L; ++i)
	{ /* Inner product trans w.r.t. b modes */
...
void Quad::Ofwd(double *in, double *out, int L, int invjac)
{
	...
	for(i = 0; i < qb; ++i)
		dvmul(qa, wa, 1, in+qa*i,  1, in+i*qa,  1);
	for(i = 0; i < qa; ++i)
		dvmul(qb, wb, 1,    in+i, qa,    in+i, qa);
...
void Tet::Ofwd(double*, double*, int, int)
{
	fprintf(stderr, "Tet:Ofwd(double*,double*,int,int) not set up yet\n");
	exit(-1);
}
...
{
	fprintf(stderr, "Pyr::Ofwd NOT set up yet\n");
	exit(-1);
}


void Pyr::Ofwd(double*, double*, int, int)
{
	fprintf(stderr, "Pyr:Ofwd(double*,double*,int,int) not set up yet\n");
	exit(-1);
}
...
void Prism::Ofwd(double*, double*, int, int)
{
	fprintf(stderr, "Prism:Ofwd(double*,double*,int,int) not set up yet\n");
	exit(-1);
}
...
void Hex::Ofwd(double *in, double *out, int L)
{
	...
	for(i=0;i<L;++i)
		for(j=0;j<L;++j)
			for(k=0;k<L;++k)
				out[i*L*L+j*L+k] *=
						0.5*(2.0*i+1.0)*0.5*(2.0*j+1.)*0.5*(2.0*k+1.);
...
void Hex::Ofwd(double*, double*, int, int)
{
	fprintf(stderr, "Hex:Ofwd(double*,double*,int,int) not set up yet\n");
	exit(-1);
}


void Element::Ofwd(double*, double*, int, int)
{
	ERR;
}


void Element::Ofwd(double*, double*, int)
{
	ERR;
}
...
void Tri::JtransEdge(Bndry *B, int edge_id, int loc, double *f)
{
	...
	switch(edge_id)
	{
		...
		case 2:
			...
			dvmul(q,f+1,1,w+1,1,f+1,1);
			/*correct for (1-b)/2 factor in weights*/
			dvdiv(q,f+1,1,m[0].b+1,1,f+1,1);
...
void Tet::JtransEdge(Bndry *B, int edge_id, int loc, double *f)
{
	...
	switch(edge_id)
	{
		...
		case 5:
			...
			dvmul(q,f+1,1,w+1,1,f+1,1);
			/*correct for (1-c)^2/2 factor in weights*/
...
void Element::JtransEdge(Bndry *, int , int , double *)
{
	ERR;
}
...
void Tet::JtransFace(Bndry *B, double *f)
{
	...
	switch(fac)
	{
		case 0:
			...
			ll = E->edge[1].l;
			for(i=0;i<ll;++i)
				daxpy(q2-1,B->bedge[1][i],m[i].b+1,1,tmp,1);
			m = Base->vert+1;
			for(i=0;i<q2-1;++i)
				daxpy(q1-2,-tmp[i],m[0].a+1,1,f+q1*(i+1)+1,1);
			...
			ll = E->edge[2].l;
			for(i=0;i<ll;++i)
				daxpy(q2-1,B->bedge[2][i],m[i].b+1,1,tmp,1);
			m = Base->vert;
			for(i=0;i<q2-1;++i)
				daxpy(q1-2,-tmp[i],m[0].a+1,1,f+q1*(i+1)+1,1);
			...
			for(i = 0; i < L; ++i)
			{
				...
				for(j = 1; j < q2; ++j)
					tmp1[j-1] = ddot(q1-2,tmp,1,f+q1*j+1,1);
		...
		case 1:
			...
			ll = E->edge[4].l;
			for(i=0;i<ll;++i)
				daxpy(q2-1,B->bedge[1][i],m[i].c+1,1,tmp,1);
			m = Base->vert+1;
			for(i=0;i<q2-1;++i)
				daxpy(q1-2,-tmp[i],m[0].a+1,1,f+q1*(i+1)+1,1);
			...
			ll = E->edge[3].l;
			for(i=0;i<ll;++i)
				daxpy(q2-1,B->bedge[2][i],m[i].c+1,1,tmp,1);
			m = Base->vert;
			for(i=0;i<q2-1;++i)
				daxpy(q1-2,-tmp[i],m[0].a+1,1,f+q1*(i+1)+1,1);
		...
		case 2:
			...
			m = Base->vert+2;
			for(i=0;i<q2-1;++i)
				daxpy(q1-1,-tmp[i],m[0].b+1,1,f+q1*(i+1)+1,1);
			...
			for(i = 0; i < L; ++i)
			{
				...
				for(j = 1; j < q2; ++j)
					tmp1[j-1] = ddot(q1-1,tmp,1,f+q1*j+1,1);
	...
	if(L)
	{
		...
		for(i = 0; i < L; ++i)
			for(j = 0; j < L; ++j)
			{
				...
				for(k = 0; k < q2; ++k)
					dvmul(q1,w1,1, tmp+k*q1,1, tmp+k*q1,1);
				for(k = 0; k < q2; ++k)
					dscal(q1,w2[k],tmp+k*q1,1);
...
void Element::JtransFace(Bndry *, double *)
{
	ERR;
}
...
void Pyr_get_tri_mmat2d(double **mat, int L, Element *E)
{
	...
	for(Pyr_tri_m2inf = Pyr_tri_m2base; Pyr_tri_m2inf;
		Pyr_tri_m2inf = Pyr_tri_m2inf->next)
...
void Pyr_get_quad_mmat2d(double **mat, int L, Element *E)
{
	...
	for(Pyr_quad_m2inf = Pyr_quad_m2base; Pyr_quad_m2inf;
		Pyr_quad_m2inf = Pyr_quad_m2inf->next)
...
void Prism_get_tri_mmat2d(double **mat, int L, Element *E)
{
	...
	for(Prism_tri_m2inf = Prism_tri_m2base; Prism_tri_m2inf;
		Prism_tri_m2inf = Prism_tri_m2inf->next)
...
static MMinfo *Tet_addmmat2d(int L, Element *E)
{
	...
	if(info)
error_msg(Tet_addmmat2d: info not zero);
...
static MMinfo *Pyr_tri_addmmat2d(int L, Element *E)
{
	...
	if(info)
error_msg(Pyr_tri_addmmat2d: info not zero);
...
static MMinfo *Pyr_quad_addmmat2d(int L, Element *E)
{
	...
	if(info)
error_msg(addmmat2d: info not zero);
...
void Pyr_JtransFace_Tri(Bndry *B, double *f)
{
	...
	for(i=0;i<qc;++i)
		dvmul(qa, wa, 1, f+i*qa, 1, f+i*qa, 1);
	for(i=0;i<qa;++i)
		dvmul(qc, wc, 1, f+i, qa, f+i, qa);
...
void Pyr_JtransFace_Quad(Bndry *B, double *f)
{
	...
	if(L)
	{
		...
		for(k = 0; k < qb; ++k)
			dvmul(qa,w1,1, f+k*qa,1, f+k*qa,1);
		for(k = 0; k < qb; ++k)
			dscal(qa,w2[k],f+k*qa,1);
...
void Prism_JtransFace_Tri(Bndry *B, double *f)
{
	...
	if(fabs(f[0] - B->bvert[0]) > FPTOL){
		fprintf(stderr,"Warning: Prism::JtransFace vertices not set \n");
	if(E->curve->type==T_Recon)
		fprintf(stderr,"Warning: Prism::JtransFace vertices ARE set (approx)\n");
		}
...
static MMinfo *Prism_tri_addmmat2d(int L, Element *E)
{
	...
	if(info)
error_msg(Prism_tri_addmmat2d: info not zero);
...
void Element::Jbwdfac1(int, double *, double *)
{
	ERR;
}
...
void JbwdTri(int qa, int qb, int lmax, int *lm, double *vj, double *v)
{
	...
	*H = vj[1];
	++H;
	dcopy(lm[1],vj+3+lm[0],1,H,1);
	H += lmax-2;
	*H = vj[2];
	++H;
	*H = vj[0];
	++H;
	dcopy(lm[2],vj+3+lm[0]+lm[1],1,H,1);
	H += lmax-2;
	...
	dgemv('n',qb,lmax,1.0,baseB[0][0],qb,H,1,0.0,wk,  lmax);
	H+=lmax;
	dgemv('n',qb,lmax,1.0,baseB[0][0],qb,H,1,0.0,wk+1,lmax);
	H+=lmax;
	for(i = 0; i < lmax-2; H+=lmax-2-i, ++i)
	...
	mxva(baseB[0][0],1,qb,H,1,wk  ,lmax,qb,lmax);
	H+=lmax;
	mxva(baseB[0][0],1,qb,H,1,wk+1,lmax,qb,lmax);
	H+=lmax;
	for(i = 0; i < lmax-2; H+=lmax-2-i, ++i)
...
void JbwdQuad(int qa, int qb, int lmax, int *lm, double *vj, double *v)
{
	...
	*H = vj[3];
	++H;
	dcopy(lm[2],vj+4+lm[0]+lm[1],1,H,1);
	H += lmax-2;
	...
	*H = vj[0];
	++H;
	dcopy(lm[0],vj+4,1,H,1);
	H += lmax-2;

	dcopy(lm[1], vj+4+lm[0], 1, H, lmax);
	++H;
	dcopy(lm[3], vj+4+lm[0]+lm[1]+lm[2], 1, H, lmax);
	++H;
