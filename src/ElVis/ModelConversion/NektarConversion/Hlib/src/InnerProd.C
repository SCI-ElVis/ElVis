...
void Element::Iprod(Element *)
{
	ERR;
}                // Inner product to Elmt
...
void Tri::Iprod_d(Element *T, Basis *ba, Basis *bb)
{
	...
	for(i = 0; i < qb; ++i)
		dvmul(qa,wa,1, T->h[i],1, T->h[i],1);
	for(i = 0; i < qb; ++i)
		dscal(qa,wb[i],T->h[i],1);
	...
	T->vert[1].hj[0] = *H;
	H++;
	memcpy(e[1].hj,H,e[1].l*d);
	H += lmax-2;
	T->vert[2].hj[0] += *H;
	H++;
	T->vert[0].hj[0] = *H;
	H++ ;
	memcpy(e[2].hj,H,e[2].l*d);
	H += lmax-2;
...
void Quad::Iprod_d(Element *T, Basis *ba, Basis *bb)
{
	...
	T->vert[0].hj[0] = *H;
	++H;
	memcpy(e[0].hj,H,e[0].l*d);
	H += lmax-2;

	dcopy(e[1].l, H, lmax, e[1].hj, 1);
	++H;
	dcopy(e[3].l, H, lmax, e[3].hj, 1);
	++H;
...
void Pyr::Iprod_3d(Element *E, Basis *ba, Basis *bb, Basis *bc)
{
	...
	for(i=0;i<qc;++i)
		dgemm('N','N',lmax,lmax,qb,1.,H+i*lmax*qb,lmax, bb->vert[0].b, qb,0.0,
				 (**E->h_3d)+i*lmax*lmax,lmax);
...
void Hex::Iprod_3d(Element *E, Basis *ba, Basis *bb, Basis *bc)
{
	...
	dgemm('N','N',lmax*lmax,lmax,qc,1.0, **E->h_3d, lmax*lmax, bc->vert[0].c,
			 qc, 0.0, H, lmax*lmax);
...
double Element::iprod(Mode *, Mode *)
{
	return 0.0;
}
...
double Element::iprodlap(Mode *, Mode *, Mode *)
{
	return 0.0;
}
...
Function Purpose: Calculate elemental Helmholtz matrix: L + lambda*M
...
{
    // code removed due to compile errors.
    // should not be necessary for Nektar model conversion.
    assert(false);
...
void Tri::form_diprod(double *u1, double *u2, double *, Mode *m)
{
	...
	{
		if(fabs(G->rx.d) > FPTOL)
			dsmul(qt,G->rx.d,u1,1,Ur,1);
		else
			dzero(qt,Ur,1);
		if(fabs(G->ry.d) > FPTOL)
			daxpy(qt,G->ry.d,u2,1,Ur,1);

		if(fabs(G->sy.d) > FPTOL)
			dscal(qt,G->sy.d,u2,1);
		else
			dzero(qt,u2,1);
		if(fabs(G->sx.d) > FPTOL)
			daxpy(qt,G->sx.d,u1,1,u2,1);
	}
...
void Tet::form_diprod(double *u1, double *u2, double *u3, Mode *m)
{
	...
	{
		if(fabs(G->rx.d) > FPTOL)
			dsmul(qtot,G->rx.d,u1,1,Ur,1);
		else
			dzero(qtot,Ur,1);
		if(fabs(G->ry.d) > FPTOL)
			daxpy(qtot,G->ry.d,u2,1,Ur,1);
		if(fabs(G->rz.d) > FPTOL)
			daxpy(qtot,G->rz.d,u3,1,Ur,1);

		if(fabs(G->sx.d) > FPTOL)
			dsmul(qtot,G->sx.d,u1,1,Us,1);
		else
			dzero(qtot,Us,1);
		if(fabs(G->sy.d) > FPTOL)
			daxpy(qtot,G->sy.d,u2,1,Us,1);
		if(fabs(G->sz.d) > FPTOL)
			daxpy(qtot,G->sz.d,u3,1,Us,1);

		if(fabs(G->tz.d) > FPTOL)
			dsmul(qtot,G->tz.d,u3,1,u3,1);
		else
			dzero(qtot,u3,1);
		if(fabs(G->ty.d) > FPTOL)
			daxpy(qtot,G->ty.d,u2,1,u3,1);
		if(fabs(G->tx.d) > FPTOL)
			daxpy(qtot,G->tx.d,u1,1,u3,1);
	}
...
void Element::form_diprod(double *, double *, double *, Mode *)
{
	ERR;
}
