...
static void solve_boundary(Element_List **, double *, double *, Bsystem **);
static void solve_pressure(Element_List **, double *, Bsystem **);
...
void solve_boundary(Element_List **V, double *rhs, double *u0, Bsystem **Vbsys)
{
	int    eDIM = V[0]->flist[0]->dim();
			...
			if(B->smeth == direct)
			{
				if(aslv)
					...
					{       /* banded matrix */
						...
						dsptrs('L', aslv, 1, R->A.inva, R->A.pivota, rhs, aslv,
							   info);
			}
			else
			{
			...
			else
			{
				...
				else
				{
					if(2*bwidth < nsolve) /* banded matrix */
						dpbtrs('L', nsolve, bwidth-1, 1, *PB->Gmat->inva,
								bwidth,rhs, nsolve, info);
					else                  /* symmetric matrix */
						dsptrs('L',nsolve,1,*PB->Gmat->inva,PB->Gmat->pivota,
								rhs,nsolve,info);
				}
			}
		}
	...
	for(i = 0; i < eDIM; ++i)
		for(E=V[i]->fhead;E;E=E->next)
		{
			...
			if(Vbsys[0]->lambda->wave)
			{
				...
				if(N > 3*bw[id])
					dgbtrs('N',N,bw[id]-1,bw[id]-1,1,invc[id],3*bw[id]-2,
							ipiv[id],hj, N, info);
				else
