...
void Tri_Face_JacProj(Bndry *B)
{
	...
	for(i = 0; i < qc; ++i)
		dsmul(qa,1/v->c[i],xr+i*qa,1,xr+i*qa,1);
	...
	for(i = 0; i < qc; ++i)
		dsmul(qa,1/v->c[i],yr+i*qa,1,yr+i*qa,1);
	...
	for(i = 0; i < qc; ++i)
		dsmul(qa,1/v->c[i],zr+i*qa,1,zr+i*qa,1);
...
void gen_face_normals(Element *E)
{
	...
	for(i=0;i<E->Nfaces;++i)
	{
		...
		if(Bc->sjac.p)
		{
			...
			E->InterpToGaussFace(0, Bc->sjac.p, E->lmax, E->lmax,
								 E->face[i].sjac.p);
			free(Bc->sjac.p);
			Bc->sjac.p = NULL;

			E->face[i].nx.p   = dvector(0, E->lmax*E->lmax-1);
			E->InterpToGaussFace(0, Bc->nx.p, E->lmax, E->lmax,
								 E->face[i].nx.p);
			free(Bc->nx.p);
			Bc->nx.p = NULL;

			E->face[i].ny.p   = dvector(0, E->lmax*E->lmax-1);
			E->InterpToGaussFace(0, Bc->ny.p, E->lmax, E->lmax, 
								 E->face[i].ny.p);
			free(Bc->ny.p);
			Bc->ny.p = NULL;
...
void superblend(double *ri, double **Q, double *P, double *r_blend ){
   ...
   rn = 1.0/(r_blend[0] + r_blend[1] + r_blend[2]);
   ...
   for (i=0; i<3; ++i)
   {
	 if (ri[i] >= (1-TOL_BLEND)){r_blend[i] = 1.0;}
	 if (ri[i] <= TOL_BLEND){r_blend[i] = 0.0;}     
   }
 
}


void genRecon(Curve *curve, double *va, double *vb, double *vc, double *x, double *y, double *z, int qaa, int qbb){

  int i;
  int q=qaa*qbb;
  double ra, rb, rc, *vna, *vnb, *vnc,*dx,*dy,*dz, ta[3], tb[3], tc[3], a[3],b[3],c[3];
 ...
 //  return;
  ...
  //Normalised Phong Normals
        double N_mod;
  	N_mod = sqrt(Nx*Nx +Ny*Ny + Nz*Nz);
  	Nx=Nx/N_mod;
  	Ny=Ny/N_mod;
  	Nz=Nz/N_mod;   
...
//    z[i] = r_blend[0]*Q[0][2] + r_blend[1]*Q[1][2] + r_blend[2]*Q[2][2];
  ...
  file = fopen("Spherigon_quad_pts.dat","a+");
  
  if (init){
  fprintf(file,"%s\n", "VARIABLES = x_sph, y_sph, z_sph");
  init = 0;
  }
  
   fprintf(file,"ZONE T=\"Elmt %d Face %d\",I=%d, J=%d, F=POINT\n",
	      elmt,curve->face+1,qaa,qbb);
		  
	for(i = 0; i < q; ++i)
	fprintf(file,"%16.12lg %16.12lg %16.12lg\n", x[i], y[i], z[i]);
