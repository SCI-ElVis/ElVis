...
Bndry_List::Bndry_List():
		nlevels(0)
{
	levels = (Bndry **)NULL;
}

Bndry_List::Bndry_List(int nlev):
		nlevels(nlev)
{
	levels = (Bndry **)calloc(nlevels,sizeof(Bndry *));
}
...
void Bndry_List::Set_Bndry(int loc, Bndry *Ubc)
{
	if(loc < nlevels)
	{
		levels[loc] = Ubc;
	...
	{
		fprintf(stderr,"Bndry_List::Set_Bndry: loc is larger than nlevels");
		exit(1);
	}
}
/*
...
Bndry *Quad::gen_bndry(char bc, int fac, ...)
{
	...
	if(OpSplit&&(bc != 'X'))
	{ // don't use this when defining boundaries
		nbdry->phys_val_g = dvector(0,QGmax-1);
		Quad_Fill_Phys_Bound (nbdry,f);
	}
...
Bndry *Prism::gen_bndry(char bc, int fac, ...)
{
	...
	else
	{
		...
		if (bc =='N') //Calculate wall normal velocity
		{
				nbdry->type = 'V';
				nbdry->face = fac;
				nbdry->elmt = this;
			...
			if (option("NORMAL0")){
				dsmul (qt, bv, nbdry->nx.p, 1, f, 1);
				//Get singular points
					if(curve){
						if(curve->type != T_Recon)
						error_msg(Normal Velocity BC :: only implemented for flat bndrys and Recon curves);
		
					nbdry->bvert[0] = bv*curve->info.recon.vn0[0];
					nbdry->bvert[1] = bv*curve->info.recon.vn1[0];
					nbdry->bvert[2] = bv*curve->info.recon.vn2[0];
					}
					
					else if (!curve) //If Element is not curve---> normal in the normals = normal in the face
					for(i = 0; i < Nfverts(fac); ++i)
					nbdry->bvert[i] = bv*nbdry->nx.p[0];
				}
				
			else if (option("NORMAL1")){
				dsmul (qt, bv, nbdry->ny.p, 1, f, 1);
				//Get singular points
					if(curve){
						if(curve->type != T_Recon)
						error_msg(Normal Velocity BC :: only implemented for flat bndrys and Recon curves);
		
					nbdry->bvert[0] = bv*curve->info.recon.vn0[1];
					nbdry->bvert[1] = bv*curve->info.recon.vn1[1];
					nbdry->bvert[2] = bv*curve->info.recon.vn2[1];
					}
					if (!curve) //If Element is not curve---> normal in the normals = normal in the face
					for(i = 0; i < Nfverts(fac); ++i)
					nbdry->bvert[i] = bv*nbdry->ny.p[0];
				}
				
			else if (option("NORMAL2")){
				dsmul (qt, bv, nbdry->nz.p, 1, f, 1);
				//Get singular points
					if(curve){
						if(curve->type != T_Recon)
						error_msg(Normal Velocity BC :: only implemented for flat bndrys and Recon curves);
		
					nbdry->bvert[0] = bv*curve->info.recon.vn0[2];
					nbdry->bvert[1] = bv*curve->info.recon.vn1[2];
					nbdry->bvert[2] = bv*curve->info.recon.vn2[2];
					}

					if (!curve) //If Element is not curve---> normal in the normals = normal in the face
					for(i = 0; i < Nfverts(fac); ++i)
					nbdry->bvert[i] = bv*nbdry->nz.p[0];
			}
...
Bndry *Hex::gen_bndry(char bc, int fac, ...)
{
	...
	printf("bf=%c\n",bc);
		...
		if (bc =='N') //Calculate wall normal velocity
		{
			...
			if (option("NORMAL0")){
				...
				dsmul (qt, bv, nbdry->nz.p, 1, f, 1);
				//dfill(qt, bv*nbdry->nz.d, f, 1);
				}
		}
			
			
			
		else if (!(bc=='N')) //Check only else
			dfill(qt,bv,f,1);
	...
	switch(bc)
	{
		...
		case 'N':
		 nbdry->type = 'V';//N==Dirichlet boudnary condition
		case 'v':
		case 'm':			// ALE modification
...
Bndry *Element::gen_bndry(char , int , ...)
{
	ERR;
	return (Bndry*)NULL;
}
...
/***************************************************************/
/* New definiton of the Element::gen_bundary */
/*An extra argument is used to access the velocity field*/
...
Argument 1: AdvVel *vel 
Purpose: Implement velocity dependent boundary conditions
...
static void Tri_Fill_Phys_Bound (Bndry *Ubc, double *f);
Bndry *Tri::gen_bndry(AdvVel *vel, char bc, int fac, ...)
{
	register int i;
	int     qt,Je;
	char   *bf;
...
Bndry *Quad::gen_bndry(AdvVel *vel, char bc, int fac, ...)
{
	register int i;
	int     qt,Je;
	char   *bf;
...
Bndry *Tet::gen_bndry(AdvVel *vel, char bc, int fac, ...)
{
	int     i,qt,Je;
	char   *bf;
	double  bv,*f;
	...
	switch(bc)
	{
		case 'n':
		case 'v':
			/*Read Inflow Boundary Condition from file*/
			if(iparam("INFLOW") || iparam("INFLOW2D")){  
			
			double *tmp1 = dvector(0, QGmax*QGmax*QGmax-1);
			vel->Get_inflow_element(this, tmp1);
			this->GetFace(tmp1,fac,f);
		
									
			/* set singular point */
			vel->Get_inflow_singular_points(nbdry->elmt, nbdry);
		
			/*get singular edge for faces 2, 3 */
			vel->Get_inflow_singular_edge(nbdry, f);
...
Bndry *Pyr::gen_bndry(AdvVel *vel, char bc, int fac, ...)
{
	register int i;
	int     qt,q1,q2,Je;
	char   *bf;
...
static void Prism_Fill_Phys_Bound (Bndry *Ubc, double *f);
Bndry *Prism::gen_bndry(AdvVel *vel, char bc, int fac, ...)
{
	register int i;
	int     qt,Je;
	char   *bf;
...
Bndry *Hex::gen_bndry(AdvVel *vel, char bc, int fac, ...)
{
	...
	switch(bc)
	{
	    ...
	    double *tmp1 = dvector(0,QGmax*QGmax*QGmax-1);
	    vel->Get_inflow_element(nbdry->elmt, tmp1);
		this->GetFace(tmp1,nbdry->face,f);
		free (tmp1);
		}
...
Bndry *Element::gen_bndry(AdvVel *,char , int , ...)
{
	ERR;
	return (Bndry*)NULL;
}
...
void Element::update_bndry(Bndry *, int)
{
	ERR;
}
...
void Tri::setbcs(Bndry *Ubc,double *bc)
{
	if(Ubc->type == 'W' || Ubc->type == 'V' || Ubc->type == 'M' ||
	        Ubc->type == 'n'|| Ubc->type == 'v' || Ubc->type == 'o' || Ubc->type == 'm' ||
...
void Element::setbcs(Bndry *,double *)
{
	ERR;
}
...
void Element::Add_flux_terms(Bndry *)
{
	ERR;
}
...
Argument 3: AdvVel *vel 
Purpose: Access the velocity field. Default argument NULL
 
Function Notes:
 
*/

void Tri::MakeFlux(Bndry *B, double *f, AdvVel *vel)
{
	register int i,j;
	...
	/*read Wall Shear Stress (WSS) along the edge*/
	if(dparam("WSS_FLUX")){ 
	...
	int qb = E->qb;
	double **Dt, **D, **ws, *tmp1;
	double vis = dparam("VIS");     /* VIS = 1/Re */
	...
	printf("\tViscosity coeff. (vis) = %lf\n", dparam("VIS")); 
		
	D  = dmatrix(0,2,0,qa*qb-1);
	tmp1 = dvector(0, qa-1);
	Dt = dmatrix(0,3,0,qa-1); 
//	dzero(6*qa,Dt[0],1);
	ws = dmatrix(0,1,0,qa-1);
	WSS = dvector(0,qa-1);

				
		/*Differentiating x component velocity field */ 
		E->Grad_h(u, D[0], D[1], D[2], 'a'); 
		
		/* Extract faces and interp to face 1 */
			for(j = 0; j < 2; ++j){ 
			E->GetFace (D[j],B->face,tmp1); 
			E->InterpToFace1(B->face,tmp1,Dt[j]); 
          }
        /*Differentiating y component velocity field */ 
		E->Grad_h(v, D[0], D[1], D[2], 'a'); 
		
		/* Extract faces and interp to face 1 */
			for(j = 0; j < 2; ++j){ 
			E->GetFace (D[j],B->face,tmp1); 
			E->InterpToFace1(B->face,tmp1,Dt[2+j]); 
          }
 ...
 /* calculate wall shear stress */ 
	 ...
	 /* 
	 ws[0] = Sx = VIS*[2*ux*nx + (uy+vx)*ny + (uz + wx)*nz]
	 ws[1] = Sy = ...
	 */
	...
	if(E->curvX){
		...
		ws[0][i] = vis*(2.0*Dt[0][i]*B->nx.p[i] + 
				   (Dt[1][i] + Dt[2][i])*B->ny.p[i]);
				   
		ws[1][i] = vis*(2.0*Dt[3][i]*B->ny.p[i] + 
				   (Dt[1][i] + Dt[2][i])*B->nx.p[i]); 
	    ...
	    ws[0][i] = vis*(2.0*Dt[0][i]*B->nx.d + 
				   (Dt[1][i] + Dt[2][i])*B->ny.d);
				   
		ws[1][i] = vis*(2.0*Dt[3][i]*B->ny.d + 
				   (Dt[1][i] + Dt[2][i])*B->nx.d); 
		}
	}
		...
		for(i=0; i< qa; i++){
		WSS[i]=sqrt(ws[0][i]*ws[0][i]+ws[1][i]*ws[1][i]);
	...
	InterpToFace1(fac, f, fi);

	
	if(dparam("WSS_FLUX")){
		dvmul(qa,WSS,1,fi,1,fi,1); 
		}	
...
void Quad::MakeFlux(Bndry *B, double *f,AdvVel *vel)
{

	register int i,j;
	const    int fac = B->face;
	const    int L = edge[fac].l;
	double   *wa,*tmp,*fi,*WSS,*Rno;
...
/*Parabolic flow. Specific case*/
if(dparam("WSS_PARABOLIC")){ 
	...
	int qb = E->qb;
	double *tmp1;
	double Pe    = 1/dparam("KINVIS");     /* Pe = 1/Kinvis */
    double delta = dparam("DELTA");
	double gamma = dparam("GAMMA");
	double Wo    = dparam("Wo");
	...
	Rno[i]=Pe+sqrt(pow(delta,2.0) + pow(Pe,2.0)) -delta;
	Rno[i]=Wo*pow(Rno[i],2.0)/(Pe + sqrt(pow(delta,2.0) +pow(Pe,2.0)));
	Rno[i] = 1/(1 + gamma*exp(-Rno[i])); 
	}
...
void Tet::MakeFlux(Bndry *B, double *f,AdvVel *vel)
{
	register  int i,j,k;
	const     int fac = B->face;
	int       l;
	...
	for(i = 0; i < l; ++i)
	{
		...
		for(j = 1; j < qb; ++j)
			tmp1[j-1] = ddot(qa-2,tmp,1,f+qa*j+1,1);
...
void Pyr::MakeFlux(Bndry *B, double *f,AdvVel *vel)
{
	register  int i,j,k;
	const     int fac = B->face;
	int       l;
	...
	if(Nfverts(fac) == 4)
	{
		...
		for(j = 0; j < qb; ++j)
			dvmul(qa, wa, 1, f+j*qa, 1, f+j*qa, 1);
		for(j = 0; j < qb; ++j)
			dscal(qa, wb[j], f+j*qa, 1);
		...
		for(i = 0; i < Nfverts(fac); ++i)
		{
			...
			{
				for(k = 0; k < qb; ++k)
					tmp1[k] = ddot(qa,m[j].a,1,f+qa*k,1);
				B->bedge[i][j] = ddot(qb,m[j].b,1,tmp1,1);
			}
		}
		...
		for(i = 0; i < l; ++i)
			...
			{
				for(k = 0; k < qb; ++k)
					tmp1[k] = ddot(qa,m1[i][j].a,1,f+qa*k,1);
	...
	else
	{
		...
		for(i = 0; i < Nfverts(fac); ++i)
		{
			...
			{
				for(k = 0; k < qc; ++k)
					tmp1[k] = ddot(qa,m[j].a,1,f+k*qa,1);
...
void Prism::MakeFlux(Bndry *B, double *f,AdvVel *vel)
{
	register  int i,j,k;
	const     int fac = B->face;
	int       l, nfv;
...
void Hex::MakeFlux(Bndry *B, double *f,AdvVel *vel)
{
	register  int i,j,k;
	const     int fac = B->face;
	int       l;
...
void Element::MakeFlux(Bndry *, double *,AdvVel *)
{
	ERR;
}
...
void Tri::Add_Surface_Contrib(Element *Ef, double *in, char dir, int)
{
	Tri::Add_Surface_Contrib(Ef,in,dir,-1,1);
}
...
void Quad::Add_Surface_Contrib(Element *Ef, double *in, char dir, int)
{
	Add_Surface_Contrib(Ef,in,dir,-1,1);
}
...
void Tet::Add_Surface_Contrib(Element *, double *, char, int)
{
	fprintf(stderr,"Tet::Add_Surface_Contrib(Element*,double*,char,int) not "
				   "setup yet\n");
	exit(-1);
}


void Tet::Add_Surface_Contrib(Element *, double *, char, int, int)
{
	fprintf(stderr,"Tet::Add_Surface_Contrib(Element*,double*,char,int,int) "
				   "not setup yet\n");
	exit(-1);
}


void Tet::Add_Surface_Contrib(double *, char, int)
{
	fprintf(stderr,"Tet::Add_Surface_Contrib(double*,char,int) not setup "
				   "yet\n");
	exit(-1);
}


void Tet::Add_Surface_Contrib(double *, char, int, int)
{
	fprintf(stderr,"Tet::Add_Surface_Contrib(double*,char,int,int) not setup "
				   "yet\n");
	exit(-1);
}
...
void Pyr::Add_Surface_Contrib(Element *, double *, char, int)
{
	fprintf(stderr,"Pyr::Add_Surface_Contrib(Element*,double*,char,int) not "
				   "setup yet\n");
	exit(-1);
}


void Pyr::Add_Surface_Contrib(Element *, double *, char, int, int)
{
	fprintf(stderr,"Pyr::Add_Surface_Contrib(Element*,double*,char,int,int) "
				   "not setup yet\n");
	exit(-1);
}


void Pyr::Add_Surface_Contrib(double *, char, int)
{
	fprintf(stderr,"Pyr::Add_Surface_Contrib(double*,char,int) not setup "
				   "yet\n");
	exit(-1);
}


void Pyr::Add_Surface_Contrib(double *, char, int, int)
{
	fprintf(stderr,"Pyr::Add_Surface_Contrib(double*,char,int,int) not setup "
				   "yet\n");
	exit(-1);
}


void Pyr::Add_Surface_Contrib(Element *, double *, char)
{
	fprintf(stderr,"Pyr::Add_Surface_Contrib(Element*,double*,char) not "
				   "setup yet\n");
	exit(-1);
}


void Prism::Add_Surface_Contrib(Element *, double *, char, int)
{
	fprintf(stderr,"Prism::Add_Surface_Contrib(Element*,double*,char,int) not "
				   "setup yet\n");
	exit(-1);
}


void Prism::Add_Surface_Contrib(Element *, double *, char, int, int)
{
	fprintf(stderr,"Prism::Add_Surface_Contrib(Element*,double*,char,int,int) "
				   "not setup yet\n");
	exit(-1);
}


void Prism::Add_Surface_Contrib(double *, char, int)
{
	fprintf(stderr,"Prism::Add_Surface_Contrib(double*,char,int) not setup "
				   "yet\n");
	exit(-1);
}


void Prism::Add_Surface_Contrib(double *, char, int, int)
{
	fprintf(stderr,"Prism::Add_Surface_Contrib(double*,char,int,int) not setup "
				   "yet\n");
	exit(-1);
}
...
void Hex::Add_Surface_Contrib(Element *, double *, char, int)
{
	fprintf(stderr,"Hex::Add_Surface_Contrib(Element*,double*,char,int) not "
				   "setup yet\n");
	exit(-1);
}


void Hex::Add_Surface_Contrib(Element *, double *, char, int, int)
{
	fprintf(stderr,"Hex::Add_Surface_Contrib(Element*,double*,char,int,int) "
				   "not setup yet\n");
	exit(-1);
}


void Hex::Add_Surface_Contrib(double *, char, int)
{
	fprintf(stderr,"Hex::Add_Surface_Contrib(double*,char,int) not setup "
				   "yet\n");
	exit(-1);
}


void Hex::Add_Surface_Contrib(double *, char, int, int)
{
	fprintf(stderr,"Hex::Add_Surface_Contrib(double*,char,int,int) not setup "
				   "yet\n");
	exit(-1);
}
...
void Element::Add_Surface_Contrib(Element *, double *, char, int, int)
{
	ERR;
}
void Element::Add_Surface_Contrib(Element *, double *, char, int)
{
	ERR;
}
void Element::Add_Surface_Contrib( double *, char, int)
{
	ERR;
}
void Element::Add_Surface_Contrib( double *, char, int, int)
{
	ERR;
}
void Element::Add_Surface_Contrib(Element *, double *, char )
{
	ERR;
}
...
void Tet::fill_edges(double *ux, double *uy, double *uz)
{
	...
	if(!uy)
	{
		...
		{
			GetFace(ux,i,Tet_wk);
			InterpToGaussFace(i, Tet_wk, face[i].qface, face[i].qface,
							  face[i].h);
		}
	}
	else
...
void Pyr::fill_edges(double *, double *, double *)
{}
...
void Hex::fill_edges(double *ux, double *uy, double *uz)
{
	...
	if(!uy)
	{
		for(i=0;i<NHex_faces;++i)
		{
			...
			GetFace(ux,i,Hex_wk);
			Interp2d(*ima,*ima,Hex_wk,qa,qb,face[i].h,face[i].qface,
					 face[i].qface);
		}
	}
	else
...
void Element::fill_edges(double *, double *, double *)
{
	ERR;
}
...
void Element::get_mmat1d(double **, int)
{
	ERR;
}
...
static MMinfo *Tri_addmmat1d(int L, Tri *E)
{
	...
	if(info)
error_msg(Tri addmmat1d: info not zero);
...
static MMinfo *Quad_addmmat1d(int L, Quad *E)
{
	...
	if(info)
error_msg(Quad addmmat1d: info not zero);
...
static MMinfo *Tet_addmmat1d(int L, Tet *E)
{
	...
	if(info)
error_msg(addmmat1d: info not zero);
#if 0
...
void Element::set_solve(int , int )
{
	ERR;
}
...
/*
   This function interpolates the value in in->[Bc->id].link at the
...
void Element::SubtractBC(double, double, int,  Element *)
{
	ERR;
}
...
void Pyr::SubtractBC(double, double, int, Element *)
{
