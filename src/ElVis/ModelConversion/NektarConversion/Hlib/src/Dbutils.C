...
void showfield(Element *E)
{
	...
	{
		case Nek_Tri:
			fprintf(debug_out,"Triangle");
			break;
		case Nek_Quad:
			fprintf(debug_out,"Quadrilateral");
			break;
		case Nek_Nodal_Quad:
			fprintf(debug_out,"Nodal Quadrilateral");
			break;
		case Nek_Nodal_Tri:
			fprintf(debug_out,"Nodal Triangle");
			break;
		case Nek_Seg:
			fprintf(debug_out,"Segment");
			break;
		default:
			break;
	}
...
void showbc(Bndry *Ubc)
{
	...
	if(E->dim() == 2)
	{
		if((l=Ebc->elmt->edge[Ebc->face].l) != 0)
		{
	...
	{
		for(i = 0; i < nfv; ++i)
			if((l = E->edge[E->ednum(Ebc->face,i)].l) != 0)
			{
...
void shownormals(Element_List *EL)
{
	...
	for(E=EL->fhead;E;E=E->next)
	{
		...
		{
			fprintf(debug_out,"x\n");
			showdvector(E->face[i].n->x, 0,
						E->face[i].qface*E->face[i].qface-1);
			fprintf(debug_out,"y\n");
			showdvector(E->face[i].n->y, 0,
						E->face[i].qface*E->face[i].qface-1);
			fprintf(debug_out,"z\n");
			showdvector(E->face[i].n->z, 0,
						E->face[i].qface*E->face[i].qface-1);
		}
	}
}
