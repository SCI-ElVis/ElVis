		...
		switch(cur->type)
		{
			...
			case T_Naca2d:
				...
				if(fabs(x[0]-vert[vnum(f,0)].x)>1e-8)
					fprintf(stderr, "Quad: %d ::Naca is      %lf away from "
...
TransformFace:
			...
			for(i = 0; i < Nfverts(fac); ++i)
				for(e = edge[ednum(fac,i)].base; e; e = e->link)
				{
					...
					dcopy(e->l,curvX[0].Cedge[ednum(fac,i)], 1,
						  E->curvX[0].Cedge[e->id],1);
					dcopy(e->l,curvX[1].Cedge[ednum(fac,i)], 1,
  						  E->curvX[1].Cedge[e->id],1);
					dcopy(e->l,curvX[2].Cedge[ednum(fac,i)], 1,
						  E->curvX[2].Cedge[e->id],1);
  		...
  		else
		{
			...
			switch(cur->type)
			{
				...
				case T_File:
					...
					Load_HO_Surface(cur->info.file.name);
					genSurfFile (this, x,y,z,cur); // returns point in face 1
												   // format
#else
...
void Element::set_curved_elmt(Element_List*)
{
	ERR;
}
...
static Cmodes *SetCXMem(Element *E)
{
	...
	for(i = 0; i < E->dim(); ++i)
	{
		for(j = 0; j < E->Nedges; ++j)
			cx[i].Cedge[j] = (double *)
							 calloc(max(E->edge[j].l,1),sizeof(double));
		if(E->dim() == 3)
		{
...
void Element::coord(Coord *)
{
	ERR;
}                 // get quadrature coords
...
void Element::straight_elmt(Coord *)
{
	ERR;
}         //
...
void Element::curved_elmt(Coord *)
{
	ERR;
}           //
...
void Hex::straight_edge(Coord *, int)
{
...
void Element::straight_edge(Coord *, int )
{
	ERR;
}   //
...
void Element::CoordTransEdge(double *, double *, int )
{
	ERR;
}
...
void Element::GetFaceCoord(int , Coord *)
{
	ERR;
}    //
	...
	/* calculate the surface jacobian as sjac = sqrt((dx/da)^2 + (dy/da)^2) */
		...
		for(i=0;i<qa;++i)
		{
			...
			if(d<1e-5 && B->K.p[i] >1e10)
				fprintf(stderr, "Tri::Surface_geofac.. singular curvature "
						"k=%lf\n", B->K.p[i]/d );
...
void Tet::Surface_geofac(Bndry *B)
{
	...
	switch(fac)
	{
		...
		case 1:
			...
			{
				for(i = 0; i < qc; ++i)
					dcopy(qa,G->sx.p+i*qa*qb,1,f+i*qa,1);
				InterpToFace1(fac,f,B->nx.p);
				dvneg(qa*qb,B->nx.p,1,B->nx.p,1);

				for(i = 0; i < qc; ++i)
					dcopy(qa,G->sy.p+i*qa*qb,1,f+i*qa,1);
				InterpToFace1(fac,f,B->ny.p);
				dvneg(qa*qb,B->ny.p,1,B->ny.p,1);

				for(i = 0; i < qc; ++i)
					dcopy(qa,G->sz.p+i*qa*qb,1,f+i*qa,1);
				InterpToFace1(fac,f,B->nz.p);
				dvneg(qa*qb,B->nz.p,1,B->nz.p,1);

				for(i = 0; i < qc; ++i)
					dcopy(qa,S.x+i*qa*qb,1,f+i*qa,1);
				InterpToFace1(fac,f,S.x);
				for(i = 0; i < qc; ++i)
					dcopy(qa,S.y+i*qa*qb,1,f+i*qa,1);
				InterpToFace1(fac,f,S.y);
				for(i = 0; i < qc; ++i)
					dcopy(qa,S.z+i*qa*qb,1,f+i*qa,1);
				InterpToFace1(fac,f,S.z);
		...
		case 2:
			...
			if(E->curvX)
			{
				...
				for(i = 0; i < qc; ++i)
					dcopy(qb,S.x+qa*(i*qb+1)-1,qa,f+i*qb,1);
				InterpToFace1(fac,f,S.x);
				for(i = 0; i < qc; ++i)
					dcopy(qb,S.y+qa*(i*qb+1)-1,qa,f+i*qb,1);
				InterpToFace1(fac,f,S.y);
				for(i = 0; i < qc; ++i)
					dcopy(qb,S.z+qa*(i*qb+1)-1,qa,f+i*qb,1);
				InterpToFace1(fac,f,S.z);
		...
		case 3:
			...
			{
				for(i = 0; i < qc; ++i)
					dcopy(qb,G->rx.p+i*qa*qb,qa,f+i*qb,1);
				InterpToFace1(fac,f,B->nx.p);
				dvneg(qa*qb,B->nx.p,1,B->nx.p,1);

				for(i = 0; i < qc; ++i)
					dcopy(qb,G->ry.p+i*qa*qb,qa,f+i*qb,1);
				InterpToFace1(fac,f,B->ny.p);
				dvneg(qa*qb,B->ny.p,1,B->ny.p,1);

				for(i = 0; i < qc; ++i)
					dcopy(qb,G->rz.p+i*qa*qb,qa,f+i*qb,1);
				InterpToFace1(fac,f,B->nz.p);
				dvneg(qa*qb,B->nz.p,1,B->nz.p,1);

				for(i = 0; i < qc; ++i)
					dcopy(qb,S.x+i*qa*qb,qa,f+i*qb,1);
				InterpToFace1(fac,f,S.x);
				for(i = 0; i < qc; ++i)
					dcopy(qb,S.y+i*qa*qb,qa,f+i*qb,1);
				InterpToFace1(fac,f,S.y);
				for(i = 0; i < qc; ++i)
					dcopy(qb,S.z+i*qa*qb,qa,f+i*qb,1);
				InterpToFace1(fac,f,S.z);
...
void Element::Surface_geofac(Bndry *)
{
	ERR;
}
...
static void Tet_JacProj(Bndry *B, Coord *S, double nx, double ny, double nz)
{
	...
	for(i = 0; i < qb; ++i)
		dsmul(qa,1/v->b[i],xr+i*qa,1,xr+i*qa,1);

	/* calculate dx/ds */
	for(i = 0; i < qb; ++i)
		dvmul(qa,v[1].a,1,xr+i*qa,1,xs+i*qa,1);
	dgemm('N','N',qa,qb,qb,1.0,x,qa,*db,qb,1.0,xs,qa);

	/* calculate dy/dr */
	dgemm('T','N',qa,qb,qa,1.0,*da,qa,y,qa,0.0,yr,qa);
	for(i = 0; i < qb; ++i)
		dsmul(qa,1/v->b[i],yr+i*qa,1,yr+i*qa,1);

	/* calculate dy/ds */
	for(i = 0; i < qb; ++i)
		dvmul(qa,v[1].a,1,yr+i*qa,1,ys+i*qa,1);
...
static void Tet_JacProj(Bndry *B)
{
	...
	for(i = 0; i < qb; ++i)
		dsmul(qa,1/v->b[i],zr+i*qa,1,zr+i*qa,1);

	/* calculate dz/ds */
	for(i = 0; i < qb; ++i)
		dvmul(qa,v[1].a,1,zr+i*qa,1,zs+i*qa,1);
...
void Hex::straight_face(Coord *X, int fac, int)
{
	register int i;
	int      q,vn;
	double   *f = dvector(0,QGmax*QGmax-1);
...
void Tri::genArc(Curve *cur, double *x, double *y)
{
	...
	p1.x = vert[fac].x;
	p1.y = vert[fac].y;
	p2.x = vert[(fac+1)%Nverts].x;
	p2.y = vert[(fac+1)%Nverts].y;
	...
	for(i = 0; i < q; ++i)
	{/* calulate points based on gll distribution */
		alpha = theta * z[i];
...
void Tri::genNaca4(Curve *cur, double *x, double *y)
{
	...
	for(i = 0; i < q; ++i)
	{
		...
		if(x0 < a)
		{ /* do polar coord search around leading edge */
			...
			if((fabs(x0) > FPTOL) && (fabs(y0) > FPTOL)) //(x0!=0.0)&&(y0!=0.0)
			{
				for(j = 0; j < ITERNACA; ++j)
				{
					...
					n0 = 5*t*(c1*sqrt(x0) +
							  x0*(c2 + x0*(c3 + x0*(c4 + c5*x0))));
					n1 = 5*t*(0.5*c1/sqrt(x0) +
							  c2 + x0*(2*c3 + x0*(3*c4 + 4*c5*x0)))*ct1;
				...
				if(j == ITERNACA)
					fprintf(stderr,"genNaca4: didn't converge\n");
				x[i] = r*ct1 + c;
				y[i] = sign*r*st1;
			}
		}
		else
		{ /* else find point on surface which so that x0,y0 lies along normal
				     to surface */
			for(j = 0; j < ITERNACA; ++j)
			{
				if(fabs(x[i]) < TOLNACA)
				...
				n0 = 5*t*(c1*sqrt(x[i]) +
					 x[i]*(c2 + x[i]*(c3 + x[i]*(c4 + c5*x[i]))));
				n0 *= sign;
				n1 = 5*t*(0.5*c1/sqrt(x[i]) +
					 c2+x[i]*(2*c3 + x[i]*(3*c4 + 4*c5*x[i])));
				n1 *= sign;
				n2 = 5*t*(-0.25*c1/(x[i]*sqrt(x[i]))+2*c3 +
						  x[i]*(6*c4 + 12*c5*x[i]));
				n2 *= sign;
		...
		{ /* do polar coord search around leading edge */
			...
			if((fabs(x0) > FPTOL) && (fabs(y0) > FPTOL)) //(x0!=0.0)&&(y0!=0.0)
			{
				for(j = 0; j < ITERNACA; ++j)
				{
					...
					n0 = 5*t*(c1*sqrt(x0) +
						      x0*(c2 + x0*(c3 + x0*(c4 + c5*x0))));
					n1 = 5*t*(0.5*c1/sqrt(x0)+ c2 +
							  x0*(2*c3 + x0*(3*c4 + 4*c5*x0)))*ct1;
...
typedef struct geomf
{    /* Curve defined in a file */
...
void Tri::genFile (Curve *cur, double *x, double *y)
{
	...
	p1.x = vert[vnum(fac,0)].x;
	p1.y = vert[vnum(fac,0)].y;
	p2.x = vert[vnum(fac,1)].x;
	p2.y = vert[vnum(fac,1)].y;
	...
	if((fabs(xoff) > FPTOL) || (fabs(yoff) > FPTOL)) //(xoff!=0.0 || yoff!=0.0)
	{
	...
	if 	(fabs(xoff) > FPTOL)	// (xoff != 0.0)
		dvsub (g->npts, g->x, 1, &xoff, 0, g->x, 1);
	if 	(fabs(yoff) > FPTOL)	// (yoff != 0.0)
...
static Geometry *loadGeom (char *name)
{
	...
	if(fabs(xmove) > FPTOL)
		dsadd(g->npts,xmove,g->x,1,g->x,1);
	if(fabs(ymove) > FPTOL)
		dsadd(g->npts,ymove,g->y,1,g->y,1);
	if(fabs(xscal) > FPTOL)
		dscal(g->npts,xscal,g->x,1);
	if(fabs(yscal) > FPTOL)
		dscal(g->npts,yscal,g->y,1);

	if (i < 2 )
		error_msg (geometry file does not have enough points);

	if (i > _MAX_NC)
		error_msg (geometry file has too many points);

	if (verbose > 1)
		printf ("%d points", g->npts);
...
static void bracket (double s[], double f[], Geometry *g, Point a, Vector ap)
{
	...
	while (fb > fc)
	{
		...
		if ((xb - u)*(u - xc) > 0.)
		{      /* Parabolic u is bewteen b and c */
		...
		else if ((xc-u)*(u-ulim) > 0.)
		{   /* Parabolic fit is bewteen c   */
		...
		else if ((u-ulim)*(ulim-xc) >= 0.)
		{  /* Limit parabolic u to the  */
...
static double brent (double s[], Geometry *g, Point ap, Vector app, double tol)
{
	...
	{    /* ....... Main Loop ...... */
		...
		else
		{
			...
			else if (fu <= fv || fabs(v-x) < FPTOL || fabs(v-w) < FPTOL)
			{
				v  = u;
				fv = fu;
			}
...
void Quad::genFile (Curve *cur, double *x, double *y)
{
	...
	if (fabs(xoff) > FPTOL || fabs(yoff) > FPTOL)
	{
		dsadd (g->npts, xoff, g->x, 1, g->x, 1);
		dsadd (g->npts, yoff, g->y, 1, g->y, 1);
		if (option("verbose") > 1)
