 ...
 *                        RCS Information                                    *
 *                                                                           *
 * $Source: /homedir/cvs/Nektar/Nektar3d/src/mlevel.C,v $  
 * $Revision: 1.4 $
 * $Date: 2008-10-28 08:32:17 $    
 * $Author: bscarmo $  
 * $State: Exp $   
 *---------------------------------------------------------------------------*/
#include "nektar.h"
	...
	for(i = 0,cnt1 = 0; i < ml[nlevel-1].npatch; ++i)
	{
		...
		for(j = 0; j < ginf[i].nedge; ++j)
		{
			if(edge[ginf[i].egid[j]] == -1)
			{
				...
				edgmap[ginf[i].egid[j]] = cnt1++; // keep reverse mapping for
			}									  // block precon	
		}
	}
...
static int Edge_connection(Patch *p)
{
	...
	for(i = 0; i < p->npatch; ++i)
		if(flag[i] == -1)
		{
			...
			if(trip)
			{ /* join unassigned patches into a new patch      */
				/* put all element of patch into new elmtid list */
				xelmt[npatch+1] = xelmt[npatch];
				nadj = p->xelmt[i+1] - p->xelmt[i];
				icopy(nadj,p->elmtid + p->xelmt[i],1,
					  elmtid + xelmt[npatch+1],1);
				xelmt[npatch+1] += nadj;
				flag[i] = npatch;
			...
			{ /* put patch in neighbouring patch with lowest number of elmts */

				/* find neighbouring patch with lowest number of elements */
				start = p->nel;
				for(j = p->xadj[i]; j < p->xadj[i+1]; ++j)
				{
					nadj = xelmt[flag[p->adjncy[j]]+1] -
						   xelmt[flag[p->adjncy[j]]];
					if(nadj < start)
					{
						start = nadj;
...
static void Fill_Mlevel(Mlevelinfo *m, Patch *p, Element_List *E, Bsystem *B)
{
	...
	DO_PARALLEL
	{
	    if(eDIM == 2)
	    {
		    ...
		    {
			    eid = pllinfo.npfeid[i];
			    for(j = 0; j < 2; ++j)
			    {
				    gid = E->flist[eid]->
				          vert[E->flist[eid]->vnum(pllinfo.npfside[i],j)].gid;
				    if(gid < nvs)
					    vflag[gid] = -2;
			    }
			    eflag[E->flist[eid]->edge[pllinfo.npfside[i]].gid] = -2;
		    }
	    }
	    else
	    {
		    for(i = 0; i < pllinfo.npface; ++i)
		    {
			    ...
			    {
				    gid = E->flist[eid]->
				          edge[E->flist[eid]->ednum(pllinfo.npfside[i],j)].gid;
				    if(gid < nes)
					    eflag[gid] = -2;
			    }

			    fflag[E->flist[eid]->face[pllinfo.npfside[i]].gid] = -2;
		    }
	    }
	}
	...
	while(p)
	{
		...
		for(i = 0; i < p->nel; ++i)
		{
				...
				/* check: vflag to see if vertex belongs to the interior of a
				   ...
				   check vmask to stop vertex being visited more than once */
					if(vmask[gid]&&(vflag[gid] < 0))
					{
						...
						{
							case 1:
				/* if only one surrounding patch then point is interior */	
								vflag[gid] = nlevel;
								cnt = patch[v2e[gid].next->eid];
								m[nlevel].Int[cnt].nvert++;
								m[nlevel].Int[cnt].vgid = (int *)
										  realloc(m[nlevel].Int[cnt].vgid,
							...
							default: // else point is on the boundary of patches
								...
								for(k = 0; k < p->npatch; ++k)
									if(ptchid[k])
									{
										...
										m[nlevel].Bdy[k].vgid = (int *)
												realloc(m[nlevel].Bdy[k].vgid,
			...
			for(j = 0; j < U->Nedges; ++j)
				...
				if((gid = U->edge[j].gid) < nes)
				{
					...
					if(emask[gid]&&(eflag[gid] < 0))
					{
						...
						else if(!U->edge[j].base)
						{ /* then it is just a single edge */
							cnt = 1;
						}
						else
						{
					...
					/* if only one surrounding patch then point is interior */
								...
								m[nlevel].Int[cnt].egid = (int *)
												realloc(m[nlevel].Int[cnt].egid,
							...
							default: // else point is on the boundary of patches
								...
								for(k = 0; k < p->npatch; ++k)
									if(ptchid[k])
									{
										...
										m[nlevel].Bdy[k].egid = (int *)
												realloc(m[nlevel].Bdy[k].egid,
			...
			{
				for(j = 0; j < U->Nfaces; ++j)
					if((gid = U->face[j].gid) < nfs)
					{ /* is face to be solved for? */
					/* check: fflag to see if face belongs to the interior of a
					   ...
					   check fmask to stop face being visited more than once */
						if(fmask[gid]&&(fflag[gid] < 0))
						{
							...
							{
								/* determine which patches this point is in */
								if(patch[U->face[j].eid] ==
								   patch[U->face[j].link->eid])
									cnt = 1;
								else
									cnt = 2;
							}
					...
					/* if only one surrounding patch then point is interior */
									...
									m[nlevel].Int[cnt].fgid = (int*) 
											realloc(m[nlevel].Int[cnt].fgid,
										  m[nlevel].Int[cnt].nface*sizeof(int));
					  m[nlevel].Int[cnt].fgid[m[nlevel].Int[cnt].nface-1] = gid;
									break;
								default: 
								/* else point is on the boundary of patches */
									fmask[gid] = 0;
									k = patch[i];
									m[nlevel].Bdy[k].nface++;
									m[nlevel].Bdy[k].fgid = (int*)
											realloc(m[nlevel].Bdy[k].fgid,
...
static void Print_Patch(Element_List *E, Patch *p, FILE *out)
{
	...
	for(i = 0; i < p->npatch; ++i)
	{
		...
		nadj = p->xelmt[i+1] - p->xelmt[i];
		if(eDIM == 3)
			fprintf(out,"ZONE T=\"zone %d\", N=%d, E=%d, F=FEPOINT, "
					"ET=TETRAHEDRON\n", cnt++,4*nadj,nadj);
		else
			fprintf(out,"ZONE T=\"zone %d\", N=%d, E=%d, F=FEPOINT, "
					"ET=QUADRILATERAL\n", cnt++,
					 E->flist[p->elmtid[j]]->Nverts*nadj,nadj);
		...
		for(j = p->xelmt[i]; j < p->xelmt[i+1]; ++j)
		{
			...
			else
			{
				fprintf(out,"%lf %lf %lf %d\n", U->vert[0].x+Xshift[0], 
						U->vert[0].y+Xshift[1], U->vert[0].z+Xshift[2],i+1);
				fprintf(out,"%lf %lf %lf %d\n", U->vert[1].x+Xshift[0], 
						U->vert[1].y+Xshift[1], U->vert[1].z+Xshift[2],i+1);
				fprintf(out,"%lf %lf %lf %d\n", U->vert[2].x+Xshift[0], 
						U->vert[2].y+Xshift[1], U->vert[2].z+Xshift[2],i+1);
				fprintf(out,"%lf %lf %lf %d\n", U->vert[3].x+Xshift[0], 
						U->vert[3].y+Xshift[1], U->vert[3].z+Xshift[2],i+1);
			}
		}
		...
		for(k = p->xelmt[i],j=0; k < p->xelmt[i+1]; ++k,++j)
		{
			...
			{
				fprintf(out,"%d %d %d %d\n", U->Nvert*j+1, U->Nvert*j+2, 
						U->Nvert*j+3, U->Nvert+j*4);
			}
		}
	}
}
