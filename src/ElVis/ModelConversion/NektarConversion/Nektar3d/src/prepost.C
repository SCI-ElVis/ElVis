...
static struct {              /* Default options for Nekscal         */
  ...
  "SurInflow", 0, "-infl  ... Dump inflow BC surface .fld file", 
...
void parse_args(int argc, char *argv[])
{
	...
	while (--argc && (*++argv)[0] == '-')
	{
		...
		else if (strcmp (*argv+1, "infl") == 0) 
		{
			option_set("SurInflow",1);
			continue;
		}
...
Domain *PreProcess(int argc, char **argv)
{
	...
	{
		init_ortho_basis();
		for(E = omega->U->fhead;E; E = E->next)
			E->dgL = E->lmax;
...
//	omega->Ubc  = ReadBCs (rea_file,omega->U->fhead); // Read BCs after setting velocity system
	...
	if(slvtype == SubStep)
		set_elmt_sides(omega->V);
	//omega->Vbc = ReadBCs       (rea_file,omega->V->fhead);  //Read BCs after setting velocity system
	...
	if(slvtype == SubStep)
		set_elmt_sides(omega->W);
	//omega->Wbc = ReadBCs(rea_file,omega->W->fhead);  //Read BCs after setting velocity system
	if(option("REFLECT2")||
   ...
   /* Memory for Inflow Boundary Conditions from file*/ 
    omega->Usys->vel = (AdvVel*) calloc(omega->U->nel, sizeof(AdvVel));
    
	omega->Usys->vel->advU = omega->U->gen_aux_field_nlevels('u');
	omega->Usys->vel->advV = omega->U->gen_aux_field_nlevels('v');
    omega->Usys->vel->advW = omega->U->gen_aux_field_nlevels('w');
	
	/* Read Boundary Conditions*/ 
    
	if(iparam("INFLOW2D")){
	//Use Meshbc to read inflow BC from a 2d .fld file
	ReadInflow2D(rea_file, omega, Meshbc);
	
	omega->Ubc = ReadBCs (rea_file,omega->U->fhead, omega->Usys->vel);
	omega->Vbc = ReadBCs (rea_file,omega->V->fhead, omega->Usys->vel);
	omega->Wbc = ReadBCs (rea_file,omega->W->fhead, omega->Usys->vel);
	}

	else if(iparam("INFLOW")){
	ReadInflow(rea_file, omega, Mesh);
	...
	omega->Pressure_sys->lambda = (Metric*)calloc(omega->P->nel,
												  sizeof(Metric));
	...
	ReadDF      (rea_file, omega->U->fhead->dim());
...
static void Summary (void)
{
	...
	printf ("Re (U=%lg, L=%lg)   : %g\n",dparam("Re_Uinf"),dparam("Re_Len"),
			 dparam("Re_Uinf")*dparam("Re_Len")/dparam("KINVIS"));
#ifdef OSSCYL
...
Gmap *GlobalNumScheme(Element_List *UL, Bndry *Ebc)
{
	...
	for(E=U; E; E = E->next)   // note vert[i].solve can have mag of 1 or 2
            for(i = 0; i < E->Nverts; ++i) 
	...
	if(option("iterative")&&0)
	{ // turn off form moment since going to use static
...
void LocalNumScheme(Element_List *EL, Bsystem *Bsys, Gmap *gmap)
{
	...
	if(Bsys->nglobal - Bsys->nsolve)
	{
		...
		for(i = gmap->nes; i < gmap->neg; ++i)
			if(emap[i]+1)
				for(j=0; j < elen[i]; ++j)
					p->knownmap[Bsys->edge[emap[i]]-nloc+j] = 
														  gmap->egid[i]+j-nglo;

		for(i = gmap->nfs; i < gmap->nfg; ++i)
			if(fmap[i]+1)
				for(j=0; j < flen[i]; ++j)
					p->knownmap[Bsys->face[fmap[i]]-nloc+j] =
														  gmap->fgid[i]+j-nglo;
			...
			for(i = 0; i < E->Nedges; ++i)
			{
				for(j = 0; j < E->dim(); ++j)
				{
					...
					if(E->edge[i].base)
					{
						...
						vb = V[eid*vertmax +
							   UL->flist[eid]->fnum1(face,j)].base;
			...
			for(i = 0; i < E->Nfaces; ++i)
			{
				for(j = 0; j < E->Nfverts(i); ++j)
				{
					...
					if(E->face[i].link)
					{
						...
						for(k=0;k<nfv;++k)
						{
							cx += F->vert[F->vnum(face,k)].x -
								  E->vert[E->vnum(i,k)].x;
							cy += F->vert[F->vnum(face,k)].y -
								  E->vert[E->vnum(i,k)].y;
							cz += F->vert[F->vnum(face,k)].z -
								  E->vert[E->vnum(i,k)].z;
						}
						cx /= 1.*nfv;
						cy /= 1.*nfv;
						...
						if(flag)
							fprintf(stderr, "Error in SetGid  Elmt:%d Face:%d "
									"Vertex:%d\n", E->id, i, j);
			...
			for(k = 0; k < E->Nfaces; ++k)
			{
				...
				{  // square face
					if(E->face[k].con < 4)
					{
						for(i=0;i<l;++i)
							for(j=0;j<l;++j)
								bmap[E->id][n+j+i*l] = B->face[E->face[k].gid]+
													   j+i*l;
					}
					else
					{ // transpose modes
						for(i=0;i<l;++i)
							for(j=0;j<l;++j)
								bmap[E->id][n+i+j*l] = B->face[E->face[k].gid]+
													   j+i*l;
						//	    E->face[k].con -= 4;
					}
					n += l*l;
