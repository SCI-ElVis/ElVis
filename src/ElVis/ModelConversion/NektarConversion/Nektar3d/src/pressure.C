...
Bndry *BuildPBCs(Element_List *P, Bndry *temp)
{
  ...
  for(i = 0; i < nbcs; ++i) {
	...
	Pbc[i].bvert[j] = pval;
      ...
      break;     	  
    case 'N': case 'V': case 'v':          /* allocate additional multi-step storage */
    case 'W': case 'F':
      Pbc[i].type = 'F';
...
static void AddAcc(Bndry *B, Domain *Omega){
  ...
  if((type  == 'V')||(type == 'M') ||(type == 'N')){ // might need other types here
    register int i,j,n;
